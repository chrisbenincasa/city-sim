    ...
    public override void _Ready()
    {
        ...
        AddChild(new WorldEnvironment
        {
            Environment = new Godot.Environment
            {
                ...
                BackgroundColor = new Color("7b8586"),
                AmbientLightSource = Godot.Environment.AmbientSource.Color,
                AmbientLightColor = new Color("d4e0ed"),
                AmbientLightEnergy = .5f
            }
        });
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                Mesh = new BoxMesh { Size = new Vector3(12, 8, 18) },
            ...
            {
                Mesh = new CylinderMesh { TopRadius = .17f, BottomRadius = .14f, Height = 1.4f },
                Position = new Vector3((i - 1) * 24 + 4, .7f, 10),
                MaterialOverride = personMaterial
            });
            AddChild(new MeshInstance3D
            {
                Mesh = new SphereMesh { Radius = .13f, Height = .26f },
                Position = new Vector3((i - 1) * 24 + 4, 1.62f, 10),
                MaterialOverride = personMaterial
            });
        }
