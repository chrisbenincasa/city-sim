...
public static class CellGrid
{
    ...
    /// <remarks>
    /// <para>
    /// <b>Open, and stated here rather than settled here.</b> <c>CLAUDE.md</c>'s constants table lists
    /// 4096² with a documented 2048² fallback and marks the choice <em>open</em>; the board files it
    /// under session J with save/load's other two. Nothing in this slice depends on which it is — the
    /// grid arithmetic is a shift either way — so it is a named constant with one reader rather than a
    /// number scattered through the diffusion.
    /// </para>
    /// <para>
    ...
    /// <summary>The map's edge, in Cells. 128, which is a 4096² Tile map.</summary>
