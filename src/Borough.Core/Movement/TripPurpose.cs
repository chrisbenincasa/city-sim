...
/// Most still lack a mechanism, so under <c>adr/0070</c> each is <em>unbuilt</em> rather than
...
public enum TripPurpose : byte
{
    ...
    /// <summary>
    /// A child travelling from home to a school, or anybody to a clinic (<c>adr/0032</c>).
    /// </summary>
    /// <remarks>
    /// <para>
    /// <b>The Attended half of the Service model, and the second GENERATED purpose in the
    /// project.</b> <c>adr/0032</c>: <em>"A Service reaches people by someone making a journey"</em>
    /// — so a school's reach is a property of the Road Graph and never of a radius. The single case
    /// that settled it is worth carrying: ***a school across an uncrossable Arterial is 200 m away
    /// and there is no route***, which a distance field calls excellent.
    /// </para>
    /// <para>
    /// <b>ONE value for schools and clinics, and it is not a shortcut.</b> The taxonomy that matters
    /// is <em>who moves</em>: both are Attended, both are generated by <c>ServiceEngine</c> in one
    /// pass, and nothing downstream distinguishes them — the Need they feed is a property of the
    /// destination's kind, which the Trip already names. A second value would be a distinction the
    /// mechanism does not make.
    /// </para>
    /// <para>
    /// ⚠ <b>The RETURN journey does not exist</b>, which is where <see cref="Commute"/> stood at
    /// 5b-bis and is why <c>CitizenActivity</c> did not grow with this. A child goes to school and is
    /// at home at the end of the Day because nothing moved them.
    /// </para>
    /// <para>
    /// 🔴 <b>A DROP-OFF is not modelled and <c>adr/0032</c> specifies one.</b> <em>"A drop-off is a
    /// waypoint on the parent's commute, not a Trip of its own … The child walks if the walk Leg fits
    /// their own budget; otherwise the school attaches to a parent's commute."</em> Only the first
    /// clause is built: a child who cannot walk to a school is recorded as unreached rather than
    /// driven by a parent. ***So a car-owning city is under-served here in a way the design says it
    /// should not be***, and that is <c>adr/0070</c> <em>unbuilt</em> rather than a decision.
    /// </para>
    /// </remarks>
    School = 5,
}
