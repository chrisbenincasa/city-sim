...
internal static class Session
{
    ...
    /// <summary>Runs the Life Stage dump, to a file or to the console.</summary>
    internal static int DumpStages(Options options)
    {
        ArgumentNullException.ThrowIfNull(options);

        if (options.OutPath is null)
        {
            return StageDump.Run(options, Console.Out);
        }
    ...
    /// <summary>Runs the flood dump, to a file or to the console.</summary>
    internal static int DumpFlood(Options options)
    {
        ArgumentNullException.ThrowIfNull(options);

        if (options.OutPath is null)
        {
            return FloodDump.Run(options, Console.Out);
        }

        using var writer = new StreamWriter(options.OutPath);
        return FloodDump.Run(options, writer);
    }
