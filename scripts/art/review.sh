...
case "${1:-capture}" in
  ...
  neighbourhood)
    ...
    ;;
  test-street)
    mkdir -p artifacts/test-street
    "$blender_bin" --background --factory-startup --python-exit-code 1 --python scripts/art/test-street.py > artifacts/test-street/export.log 2>&1
    "$godot_bin" --headless --path src/Borough.Godot --editor --import > artifacts/test-street/import.log 2>&1
    ...
    ;;
  exact-bodies)
    mkdir -p artifacts/exact-bodies/capture
  ...
  open)
    ...
    ;;
  *) echo 'Usage: scripts/art/review.sh [rebuild|verify|capture|open|material-rich|open-rich|kit|open-kit|extremes|open-extremes|styles|open-styles|models|open-models|palettes|open-palettes|expanded|open-expanded|construction|fidelity|open-fidelity|brick-scale|neighbourhood|open-neighbourhood|test-street|exact-bodies|open-test-street|open-test-street-variants]' >&2; exit 2 ;;
esac
