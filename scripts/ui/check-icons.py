...
quiet()
state = read()
mark = next(m for m in state['SupplyMarks'] if m['Visible'])
move(mark['Rect'])
after = read()
assert any(b['Icon'] == 'trouble' and b['Filled'] for b in after['Buttons'])
command(f'shoot {out}/icons-hover.png')
quiet()
assert all(not b['Filled'] for b in read()['Buttons'] if b['Icon'] == 'trouble')
command('ui building 2')
command('ui theme dark')
quiet()
command(f'shoot {out}/icons-dark.png')
assert read()['Hash'] == initial['Hash']
print('PASS: real outline/filled textures, stable hover geometry, selected outline, both themes, 100/150% sizing, trouble hover and unchanged paused State Hash.')
