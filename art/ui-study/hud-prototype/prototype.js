...
const families = [['Connections','↗'],['Zoning','▦'],['Municipal','⌂'],['Utilities','≈'],['Demolish','⌫']];
...
dataViews.unshift(['Government','M3 9l9-6 9 6H3zM5 11v7m7-7v7m7-7v7M3 21h18']);
...
add('Municipal','Education','School','','Place an education service');
add('Municipal','Education','Further education');
add('Municipal','Health','Clinic','','Place a health service');
add('Municipal','Emergency services','Fire service');
add('Utilities','Waste','Waste service');
add('Utilities','Networks','Power facility|Water facility|Sewage facility','Unresolved','Utility transport and facility catalogue need scoping');
...
add('Municipal','Health','Hospital');
add('Municipal','Parks and recreation','Park|Recreation centre');
add('Municipal','Emergency services','Police');
let state;
const instructions = {
  ...
  transit:'Open Line operation from Government on the bottom console → Transit operation. Change vehicle provision and apply. Inspect a stop, then use Back to return to the line. Pin the line and find it with the Pins button.'
};
function initial(scenario) {
  return {scenario,family:scenario==='zone'?'Zoning':scenario==='transit'?'Connections':null,dataView:null,lastChooser:'tools',mapGroup:'Environment',governmentGroup:'Finance',query:'',filter:'All',tool:null,
...
function renderData(){
  ...
  $('#data-launchers').innerHTML=dataViews.map(([name,path])=>button(`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${path}"/></svg><span>${name}</span>`,`data-data="${name}" title="${name==='Government'?'Government: policies, funding and programmes':name==='Maps'?'Data maps':name==='Pins'?'Pinned people, places and metrics':name}" aria-expanded="${state.dataView===name}" class="${state.dataView===name?'selected':''}"`)).join('');
  if(focused)$(`[data-data="${focused}"]`)?.focus({preventScroll:true});
  const p=$('#data-panel');p.hidden=!state.dataView;if(!state.dataView)return;
  let content='';
  if(state.dataView==='Government'){
    const policies=catalogue.filter(t=>t.family==='Government'),groups=[...new Set(policies.map(t=>t.group))];
    content=`<div class="map-groups">${groups.map(g=>button(g,`data-government-group="${g}" aria-pressed="${state.governmentGroup===g}"`)).join('')}</div><div class="data-choices government-choices">${policies.filter(t=>t.group===state.governmentGroup).map(t=>button(`${t.name}${badge(t)}`,`data-tool="${t.id}" title="${t.detail}"`)).join('')}</div>`;
  }else if(state.dataView==='Maps'){
...
function renderCatalogue() {
  ...
  const compact=families.some(([name])=>name===state.family);panel.classList.toggle('compact',compact);panel.classList.toggle('zoning',state.family==='Zoning');
  if(compact){
    const familyItems=catalogue.filter(t=>t.family===state.family),groups=[...new Set(familyItems.map(t=>t.group))];
    const items=groups.flatMap(group=>familyItems.filter(t=>t.group===group));let last='';
...
function choose(t) {
  if(t.family==='Government'){inspect(t.name==='Line operation'?'line':'sketch:'+t.id);closeData();tell(`${t.name} opened. Your map tool and active layer are preserved.`);return;}
...
document.addEventListener('click',e=>{
 ...
 if(b.dataset.governmentGroup){state.governmentGroup=b.dataset.governmentGroup;renderData();$(`[data-government-group="${state.governmentGroup}"]`).focus({preventScroll:true});}
