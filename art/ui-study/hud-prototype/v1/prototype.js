...
const families = [['Connections','↗'],['Zoning','▦'],['Services','⌂'],['Government','≡'],['Demolish','⌫'],['Layers','◫'],['Following','☆']];
...
let state;
const instructions = {
  zone:'Choose a permission, close the catalogue, and paint parcels. Change to Whole blocks, then try Erase zoning. Open a layer while a tool is held; close its picker and keep the legend.',
  ...
  transit:'Open Line operation from Connections → Transit line, or Government. Change vehicle provision and apply. Inspect a stop, then use Back to return to the line. Pin the line and find it under Following.'
};
function initial(scenario) {
  return {scenario,family:scenario==='zone'?'Zoning':scenario==='transit'?'Connections':null,query:'',filter:'All',tool:null,
    selected:scenario==='supply'?'shop':scenario==='transit'?'line':'parcel',history:[],parcels:{},parcel:22,
...
function openFamily(name) {state.family=state.family===name?null:name;state.query='';state.filter='All';renderRail();renderCatalogue();}
function renderRail() {
...
function renderList() {
  ...
  const q=state.query.toLowerCase();let items=catalogue.filter(t=>(q?`${t.name} ${t.family} ${t.group}`.toLowerCase().includes(q):state.filter==='Favourites'||t.family===state.family));
...
function choose(t) {
  if(t.family==='Layers'){state.layer=t.name;renderMap();renderLegend();renderList();completed('layer selected');tell(`${t.name} layer is on. Your held tool is unchanged.`);return;}
  if(t.name==='Supply diagnosis'){inspect('shop');return;}
  if(t.name==='Transit line'||t.name==='Line operation'){inspect('line');return;}
  if(t.family==='Zoning'||t.name==='Street'||t.name==='Abandoned Building'||t.name==='School'||t.name==='Clinic') {
    state.tool=t;renderConsole();renderList();tell(`${t.name} selected. Close the catalogue to make more room; Cancel disarms the tool.`);return;
  }
...
function fit(){ $('#city').style.setProperty('--console-clear',($('#console').offsetHeight+32)+'px');$('#city').style.setProperty('--legend-clear',state?.layer?($('#legend').offsetHeight+12)+'px':'0px'); }
...
function inspect(id) {if(state.selected!==id)state.history.push(state.selected);state.selected=id;renderInspector();renderMap();}
...
function render(){renderRail();renderCatalogue();renderInspector();renderConsole();renderLegend();renderMap();$('#progress').textContent='';}
...
document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(stroke){stroke=null;suppressMapClick=true;renderMap();tell('Stroke cancelled before applying.');}else if(!$('#camera').hidden){$('#camera').hidden=true;$('#camera-toggle').setAttribute('aria-expanded','false');$('#camera-toggle').focus();}else if(state.family){const family=state.family;state.family=null;renderRail();renderCatalogue();$(`[data-family="${family}"]`).focus();tell('Catalogue closed; held tool and layer preserved.');}else if(state.tool)cancelTool();});
document.addEventListener('click',e=>{
 const b=e.target.closest('button');if(!b||b.disabled)return;
 if(b.dataset.family)openFamily(b.dataset.family);
 if(b.dataset.open){state.family=b.dataset.open;state.query='';state.filter='All';renderRail();renderCatalogue();}
 if(b.dataset.close==='catalog'){const family=state.family;state.family=null;renderRail();renderCatalogue();$(`[data-family="${family}"]`).focus();completed('close catalogue, keep tool');}
 if(b.dataset.close==='inspector'){state.selected=null;$('#inspector').hidden=true;}
 ...
 case 'back':state.selected=state.history.pop();renderInspector();renderMap();completed('inspection history');break;
 case 'pin':state.pinned=state.pinned.includes(state.selected)?state.pinned.filter(x=>x!==state.selected):[...state.pinned,state.selected];renderInspector();if(state.family)renderList();completed('Pin');break;
 case 'route':state.route=true;inspect('route');completed('follow Evidence');break;
 case 'repair':state.tool=catalogue.find(t=>t.name==='Street');state.family='Connections';state.query='';state.filter='All';renderRail();renderCatalogue();renderConsole();tell('Street held. Click the dashed missing connection on the map.');break;
 ...
 case 'layer-off':state.layer=null;renderLegend();renderMap();if(state.family)renderList();tell('Layer off. Tool and selection preserved.');break;
 }
