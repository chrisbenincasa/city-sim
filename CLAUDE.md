...
| Constant | Value | Kind |
|---|---|---|
| `TICKS_PER_DAY` | ~~8192~~ **2048 — ⚠ DECIDED 2026-08-13, NOT YET FLIPPED** | world-creation, baked into the save, not hot-reloadable. `Ticks.PerDay` — a `const`, where `adr/0015` says it should be Ruleset data; see `plans/0012`. **The code still says 8192.** [`adr/0094`](docs/adr/0094-a-day-is-2048-ticks-because-ticks-per-day-is-a-sampling-rate-and-not-a-length-of-life.md): a Tick is **42.1875 s** of in-world time, a Day is **2m08s at 1×**, and a twenty-hour campaign holds **562 Days** against 140 — which is what `01 §4`'s Replacement Rate marker needs and what the old constant could not supply at any Life Stage length. **`adr/0019`'s prohibition falls and its premise is why**: it forbade this on the ground that a commute is a fixed number of Ticks, which `adr/0082` made false by deriving a Tick's duration *from* this constant — a commute is **1.39% of a Day at both values**, so no balance moves. **What rescales is decided by one question — *what is the quantity denominated in?*** Ticks (kept at its number), in-world time (unchanged, ×4 more of it per real second), Days (unchanged, ×4 faster in real time). **Only Goods quantities rescale, ×4.** ⚠ **One cost**: routing ×4, taking `plans/0013` to ~305% at 4× and ~76% at 1×. The sub-step band goes 21–45 → **84–180** and a second claimed cost — a Microscopic Cap collapse — was **withdrawn the same day by [`adr/0096`](docs/adr/0096-the-microscopic-cap-derives-from-the-design-speeds-budget-and-not-from-the-top-rungs.md)**: priced at the design speed the Cap's supply side is *unchanged* from the old clock's top rung, and the gap it was quoted against had no denominator. **UNRATIFIED**; ratifier is **playtest**, refuting numbers named in both directions, and **lower is the expected direction** |
| `WHEEL_SIZE` | ~~8192~~ **2048 Ticks — ⚠ DECIDED, NOT YET FLIPPED** | world-creation. Set by the longest routine sleep, which is bounded by one Day — so it moves with `TICKS_PER_DAY` and `adr/0019`'s note that their equality is *"a coincidence worth stating"* still holds |
...
| Microscopic Cap | **unset** | fixed world constant, still open. **It counts *Vehicles*, not Segments** (`adr/0062`) — the unit was wrong in three places and is the family of `adr/0053` and `adr/0059`. Its value is a **ratio nobody has both halves of**: Vehicles affordable in a Tick (**S5**) against Vehicles a real city stresses at once (`06` **5b-bis**, read 5b before 2026-08-12). ⚠ **The budget it is priced against is now the *design speed's* 62.5 ms, not 15.6 ms** ([`adr/0096`](docs/adr/0096-the-microscopic-cap-derives-from-the-design-speeds-budget-and-not-from-the-top-rungs.md)) — **~25,400 Vehicles, one core**. Pricing at the top rung chooses `03 §3.9`'s *simulation* degradation for every player permanently in order to avoid its *hardware* one on one machine, which is that table read backwards, and `01 §1` makes 4× the rung a large city withdraws anyway. ⚠ **Do not quote a gap against 186,624**: that is S2 R2's synthetic **fleet**, labelled *not a stressed count* by `plans/0013` in the cell itself, and a ratio was published against it on 2026-08-13 and withdrawn the same day — ***a caveat attached to a number does not travel with it***. **Two things owed**: a **2- and 4-thread** Lane kernel measurement (S5), the largest unclaimed multiple on the supply side; and a **fallback tier below Microscopic**, recorded as foreseen and deliberately **undesigned** under `adr/0070` until the demand side exists |
