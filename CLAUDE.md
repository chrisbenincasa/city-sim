...
| `plans/0026` | **Milestone 5c — statistical resolution and the travel-time matrix.** ⚠ **Scoped 2026-08-14, not started, and it is the current code row.** Eight tasks: the routing partition (`adr/0040`'s cluster **in Cells**, whose size is `0002` §D2's measurable-and-unset number and whose Chunk-divides-it correction `adr/0040` is still owed), the travel-time matrix (first consumer already in the tree — `EmploymentEngine` pays **~32.5 µs a walk search** per candidate today), the path source (**amends [`adr/0075`](docs/adr/0075-a-leg-is-a-plan-and-a-traveller-is-a-cursor.md)**, which gave a Leg a cost and no path), the route cache (`adr/0012`'s contract, and the **first consumer the per-Segment Epoch has ever had**), the **vehicular Leg at Statistical resolution**, the volume-delay function (`CapacityPerDay`'s first reader), a runner mode and the long run. ⚠ **Scoping it corrected `06`**: that document said volume *"waits on a path source — which is 5c, **not on vehicles**"*, and `03 §3.3` plus `WorldInvariants.cs:103` make volume **vehicular**, so the path source is necessary and **not sufficient**. ***A precondition stated in the singular is a precondition nobody has finished counting*** — third sighting in three milestones, after 5b task 4's missing destination set and 5b's missing path. **Three decisions are owed before the tasks needing them**, and the sharpest is session F's placeholder trap: a drive Leg with no Parking Shed must not get a zero-length parking walk, because that makes a **full car park cost less than an empty one** |
