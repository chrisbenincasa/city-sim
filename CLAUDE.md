...
| The market's damping | **`decay_percent` 50, `move_cap_percent` 10** — `[market]`, `rulesets/twinned.toml` only | tuning, hash-bearing ([`adr/0135`](docs/adr/0135-a-market-needs-two-sides-so-twelve-ships-a-provider-and-the-price-moves.md)). **Neither key is a price** — a Pool opens at the ceiling and moves from there, so no seed exists and none is needed. **Absent means every trade clears at the ceiling for ever**, which is the city the other ten shipped files have. ⚠ **`decay_percent` 0 is ALLOWED and means no smoothing; 100 is refused** as a rate that never moves. 🔴 ⚠ **THE MECHANISM IS STILL INERT AND THE REASON CHANGED ON 2026-08-26.** It was *`Scope.Pool` throws*; that scope has resolved since milestone 26 task 4 and **the price has still never moved on any world** — `World.RepriceDistrictPools` measures cover from the market row's own Bin, which [`adr/0139`](docs/adr/0139-a-district-pool-is-a-market-and-not-a-store-so-stock-stays-with-the-seller.md) emptied. Measured at eight rows and **zero** price changes over 24,576 Ticks. `plans/0003` queue item 21 |
