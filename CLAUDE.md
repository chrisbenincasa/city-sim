...
| A `[[kind]]`'s employment | **unset** — no shipped Ruleset declares one | tuning, hot-reloadable, **hash-bearing** — `[[building]] jobs` (5b-bis task 2). `adr/0068`'s rule on a second axis, and it transplants because the *reason* does: a Bin over its ceiling drains because a Bin has a consumer, and a job has a holder and none, so a lowered ceiling **dismisses** the overflow. It counts **Citizens**, never Households. **A gap rather than a debt** (`0002` §D2) — the key exists and nothing states a value, so nothing accretes on it. Ratifier is the first Ruleset with a workplace kind in it, which is 5b-bis task 4's. **Its arrival deleted `SyntheticCity`'s workplace stride**: the populator was handing out employment no declaration granted, which was expressible only once a ceiling could contradict it |
...
| Commute Budget | **unset, and the omission is now stated in a file** | tuning, hot-reloadable, **hash-bearing** — `[trips]` Ruleset data. The line between a Trip that completes and one whose Fate is *exceeded commute budget*, so it decides which Buildings decline. **Clock minutes, one currency across modes, and there is no per-mode weight** (`adr/0008`, session F): a weight would make the scored quantity differ from the **displayed** one, which is the SC4 failure the Budget exists to prevent, and distaste for walking belongs to the choice model instead. It is a **percentile of a Trip-cost distribution** and is meaningless before one exists. **`commute_budget_minutes` is `[trips]`'s one optional key** (5b-bis task 3), against `[placement]`'s every-key-required rule: **an omitted key states a city with no ceiling** — nothing is refused for its length and `TripFate.ExceededCommuteBudget` is reachable and unreached. It cannot be a default, because every minute count is a legitimate Budget and so no minute count can mean *unset*; and it is a **precondition of the ratifier** rather than a convenience, since a Budget in force censors the distribution it is a percentile of. **UNRATIFIED**; ratifier is the first **5b-bis** run long enough to produce that distribution *(read 5b before 2026-08-12)* |
