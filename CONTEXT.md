...
**Shopping cargo**
One Good bought by a Household and held by its shopping outing until delivered home. It is separate from a Citizen inventory and from the transient Traveller. `ShoppingTable.Cargo` owns it; `ShoppingEngine` transfers or explicitly records its loss.

**Visiting a provider is a Trip, and a Household normally visits one per shopping occasion** (`adr/0067`). Severe Sustenance shortage permits one additional known shop after buying nothing; any useful partial purchase ends the search. Finding nothing is recorded on the Household — consecutive failed occasions, and a refusal reason, since a provider can fail on price or distance as well as on stock — never as a `Trip Fate`, whose four outcomes are all properties of the journey. A cursor advances on failure and resets on success, so a provider that failed is skipped for exactly one occasion: a deprioritisation that is a **duration**, with its window derived from the mechanism rather than chosen.
...
**Taste**
A Household's private position on each preference axis: how much it weighs quiet against variety, space against centrality, rent against commute. Drawn from the Household's own seeded stream and **persistent for life** — Life Stage supplies a base and a *range*, and Taste picks the point within it, so someone who always valued quiet still values quiet once they have children. ⚠ **Nothing is stored**: `Ruleset.CentralityTaste` derives the position on read from a `Ticks.Zero` draw against the Household's monotonic id, so persistence is a property of the key rather than of a column somebody has to remember to copy forward. **One of the three axes is built** — space against centrality, read by `PlacementEngine.TryHouse`; quiet and rent are *unbuilt* (`adr/0070`).
