...
**It is the cursor over a Trip's Legs, and it holds nothing else**: which Citizen, which Trip, which Leg it is on, when it arrives, and — at **Microscopic** resolution only — **how far along that Leg's route it has got**. That last field was added 2026-08-14 (`adr/0075`, amended): a Statistical Traveller between two endpoints is nowhere in particular, so the arrival Tick is its whole position, and a Microscopic one must name a next Segment every Tick. It is the field Demotion discards. The **plan** is the Leg's. Every durable thing therefore sits on a row that outlives the journey and every transient thing on a row that is released — which is what makes *a Traveller is a view, not an owner* hold by construction. See [`docs/adr/0075`](docs/adr/0075-a-leg-is-a-plan-and-a-traveller-is-a-cursor.md).
