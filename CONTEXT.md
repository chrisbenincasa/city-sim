...
**Bin**
An integer store of one resource with a capacity. Its level is a `long`, and so is every quantity on the path that writes it (`adr/0065`) — a Household's money is a `long` and money is a Resource held in a Bin, so a narrower Bin would narrow a payment. No floats, no continuous flows. Bins live on Buildings, on **Households and Businesses** (their balance), on Districts (as Pools), and globally (the treasury).
