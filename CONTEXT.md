...
The count is therefore **physics rather than a design choice**: the early city has one District because the city *is* one neighbourhood, and more appear as ~~it outgrows the pooling radius~~ **it grows a second centre**.

🔴 ⚠ **The extent is a CURVE'S SCALE and no longer a maximum — amended 2026-08-22 by `docs/adr/0134-a-district-is-a-centre-and-its-basin-so-the-count-follows-centres-and-not-a-ceiling.md`.** A District is **a concentration of activity and the ground that drains to it**: a watershed over a Building-density field, clipped to a road component, seeded only where a concentration's **prominence** clears a threshold. **Nothing forces a split.** A large District is bounded **economically**, by `adr/0133`'s haulage charge scaling with extent, rather than geometrically — so the **128 Cells** above is the scale at which carriage starts to bite, not a line anything may not cross. ⚠ **Its ratification obligation did not soften, it changed shape**: a curve's parameter rather than a ceiling.

✅ **THE DERIVATION EXISTS AS OF 2026-08-22** — milestone 12 task 3. `Space.DistrictWatershed` is the operator, `DistrictTable` holds a centre apiece, `DistrictCellTable` holds the extent one row per built Cell, and `[districts] prominence_percent` is the one key. ⚠ **Read the symbol rather than this sentence for what it does** ([`docs/adr/0093`](docs/adr/0093-a-description-of-the-build-is-where-to-look-and-never-what-you-found.md)); what this line is for is the **trigger**, which is `World.EvaluateDistricts` — called once at world creation, and thereafter from the **end of phase 6** on `[districts] revisit_ticks`. ⚠ **It reconciles rather than replaces**: a District keeps its row across a re-evaluation, a Cell stays put unless the field is decisive, and a boundary moves by at most `migrate_cells` a time — which is what makes the extent *state* rather than a label. ⚠ **Nothing reads a District yet** — `Scope.Pool` still throws, and the Pool Bins are milestone 12 task 5. ⚠ **The 128 Cells above is still unmeasured against anything**, and the threshold is unratified with it: `plans/0002` §D1 carries the row and names milestone 15.

⚠ **The struck words are where the ceiling came from.** *"Outgrows the pooling radius"* reads as *a District splits when it gets big*, and what a city actually does when it outgrows a neighbourhood is **grow another one**. ***No algorithm can find a meaningful boundary in a featureless city, because there is not one to find*** — a monocentric city has no natural internal line, so a hard radius does not discover a boundary, it *manufactures* one somewhere on a featureless ring. And `adr/0013` requires the player be able to see a boundary and understand what it means. **A District is `(saved AND hashed)`**, not derived state: the persistence, hysteresis and damping that keep the boundary from flickering all consult the previous extent, so it is updated rather than recomputed.
...
⚠ **It said *Business types* until 2026-08-23 and that was stale by an ADR** (`adr/0032`, `adr/0151`). `adr/0032` widened it from *Business* to **destination** so that a park could be an entry, and this entry was never amended to carry it — which is why three documents disagreed about whether a park counted and why `adr/0123` recorded the blocker as *a `kind` column on `BusinessTable`*, a column that could never have enumerated one. **The key is the kind's name in the Ruleset**, so a shop, a school, a clinic and a park all count, one each; ***which* destinations exist is Ruleset content and not architecture.** Nothing is weighted by size, stock or quality.

⚠ **The count is the PLACE's and the set is the HOUSEHOLD's, and the word names both** (`adr/0151`). *How much variety stands within a walk of this address* is a standing property of the address with no mover in it, and is what land value reads. *Which bakery this Household actually uses* is the **Provider List** — sticky, chosen by something that moves, and the thing *Terms we deliberately do not use* → **no proximity scope** governs. ***That rule forbids a Building's Rule from selecting among nearby options; it does not forbid a place from having a property***, and reading it as though it did is what made Amenity look like it needed a mover before land value could read it.
...
**A Settlement forms where an edge is *missing*, and distance is one of the things that removes one** (`adr/0085`, `adr/0089`). Connectivity is transitive, so a contiguously-developed lattice is one Settlement however large the graph — what fragments it is unbuilt ground with no road across it, a Severance barrier, a mode nobody there has, congestion, or **a gap wider than the Commute Budget**.

⚠ **The last of those does not work on the shipped map and will.** At 4096² a corner-to-corner drive is under forty clock minutes, so no pair of Districts is out of range of any other and a connected city is **one** Settlement at any Budget that also bounds a Trip — S2 R1.5 measures exactly that. `adr/0089` takes the map to **512 Cells, 65.5 km**, which is 3.7–5.2 Commute Budgets across. ✅ **THE CONSTANT HAS MOVED** — `CellGrid.WorldCells` is **512** (`CellGrid.cs:135`); the road-generation gate it waited on was `plans/0003` queue **item 6**, shipped 2026-08-13, and the flip went in the same day. *(This paragraph said **has not moved yet** until 2026-08-20, and told the reader to treat every distance-dependent statement here as describing the intended map rather than the built one. They are now the same map. Found while scoping milestone 11 by opening `CellGrid`, which is [`adr/0093`](docs/adr/0093-a-description-of-the-build-is-where-to-look-and-never-what-you-found.md)'s rule doing its job: the sentence was wrong about the **trigger**, and the trigger had fired a week earlier.)* ⚠ **What has NOT been re-read is everything that was written while it was true** — `adr/0089` says so in its own Consequences: *every distance-dependent claim is re-opened favourably, and none of them may be quietly updated.*

**Mode is load-bearing rather than decorative**, and it works on both maps: a crossing is 27.3% of a Day on foot against 1.4% by car at 4096², and more than a full Day on foot at 512 Cells. A Household without a car inhabits a materially smaller map on the same ground.
...
save** (`docs/adr/0154-base-fertility-is-ruleset-data-keyed-by-terrain-type-and-the-old-name-invented-a-field.md`),
...
⚠ **AMENDED 2026-08-23 by [`adr/0141`](docs/adr/0141-a-tenant-owns-what-leaves-with-it-and-the-premises-own-the-capacity.md): it is a property of the SUBJECT, and there are two of them.** ~~a property of the Building and of nothing else.~~ A tenant runs its own Rules over its own Bins, so **a tenant accumulates its own pressure and it produces a different verdict**: the premises' pressure **condemns the Building**; a tenant's **ends the tenancy** and leaves the premises standing. ⚠ **Everything else in this entry is untouched** — the duration, the reset, the *missed firings* denomination and the argument against tallying all apply per subject exactly as they applied per Building. ***What moved is whose pressure it is, not what pressure is.*** 🔴 **The sentence above is what made one starving tenant condemn the other's shop**, which is the defect `adr/0141` names and milestone 25 task 4 removed. **Both subjects are measured against the same `condemn_after`**, which is the premises' kind's, because a tenant has no kind to declare its own.
...
**An abandoned Building is emptied and left standing on its Lot** (`adr/0091`). It is not demolished, and its Lot does not return to vacant: the standing Building is what **carries** the contagion — an abandoned Building feeds the desirability composition negatively, and bare ground has no term to feed it with — and it is what the retained condition above is retained *on*. It leaves by redevelopment when land value falls far enough to pencil, by the player's `Demolish`, or by a `Govern` clearance programme, so the stock is bounded by the Lot count rather than by elapsed time (`adr/0006`). `02 §5.9` asserted both readings and the build took the wrong one.

**Derelict**
A Building the **Ruleset in force cannot describe** — its kind is not declared, so it holds whatever Bins survived, houses whoever lived there, and runs nothing at all. It **still stands, still holds its Occupants and still occupies its Lot**; what it does not do is run Rules, so it has no failures of its own to die of and nothing about it decays. It is **derived, never recorded**: there is no derelict flag, the state is `Kind == 0` — the row naming nothing — and it is read off the kind (`adr/0057`). **That is load-bearing rather than stylistic.** The only actor who removes a kind from a running city is a designer balancing, and a designer's commonest move is *undo*; a mark that recorded the removal and never cleared would leave them a city of permanently inert Buildings, which is the failure `adr/0015` exists to prevent arriving through the mechanism written to serve it. A derelict Building is recovered by a reload that describes its kind again. `HONEST DEGRADATION`
...
⚠ **The gradient is produced by the willingness ordering and by nothing else** (`adr/0131`). *Raises its rate* and *skews its mix* are both consequences of taking the most willing first, so a stock that decrements where nothing orders willingness can express only **availability** — arrivals, then none — which is the population ceiling this entry refuses, wearing the name of the mechanism meant to replace it. ***A stock without an ordering is a wall, whatever the design calls it.*** Depth and recovery are therefore authored alongside the comparison that orders willingness, and not before it.

**A Hinterland field is authored in the milestone that reads it** (`adr/0131`), which is why the fields above do not all appear at once: what its emigrants carry arrives with the gate, because a Household crossing into the city brings its balance with it and that money's origin is this object rather than the city's founding settings. The rest — a price per Good, a wage, a rent, service levels, a commute figure — arrive with the surfaces that read them. ***An anchor that does not reach the thing it anchors is decoration.***
...
**Bin**
An integer store of one resource with a capacity. Its level is a `long`, and so is every quantity on the path that writes it (`adr/0065`) — a Household's money is a `long` and money is a Resource held in a Bin, so a narrower Bin would narrow a payment. No floats, no continuous flows. Bins live on Buildings, on **Households and Businesses** (their balance), and globally (the treasury). ⚠ **A District holds no Bin** — it holds a **market** rather than a store ([`adr/0139`](docs/adr/0139-a-district-pool-is-a-market-and-not-a-store-so-stock-stays-with-the-seller.md)); this line read *on Districts (as Pools)* until 2026-08-22.

⚠ **A Bin's owner is discriminated and the discriminator is what makes a balance possible** ([`docs/adr/0114`](docs/adr/0114-a-balance-a-rule-can-fail-on-is-a-bin-and-a-bins-owner-is-discriminated.md), built milestone 10 tasks 1 and 4c). `BinTable.OwnerKind` names one of Building, Household, Business or treasury. ***A balance a Rule can fail on is a Bin, because the failure surface is what a Bin is for*** — a Rule short of money held in a *column* cannot say what stopped it, has no wait list to join, is woken by nothing and reports no Blocking, so `adr/0050`'s bankruptcy-against-input-starvation diagnosis could not be written at all. `HouseholdTable.Money` and `.Savings` were columns until task 4c and are both deleted.

⚠ **The link runs from the actor to its Bin, not the other way**, and the asymmetry with a Building has a reason: a Building holds *many* Bins because its kind declares many Resources, so it carries a list and each Bin names it back; an actor holds *one*, because money is one Resource, so a single saved handle is the whole relationship. A second conserved Resource throws by name rather than defaulting.

⚠ **AMENDED 2026-08-23 by [`adr/0143`](docs/adr/0143-a-bin-hangs-off-its-owner-and-the-polymorphic-column-stays-unbuilt.md): AN ACTOR HOLDS SEVERAL, so the single handle became a LIST.** ~~an actor holds *one*, because money is one Resource, so a single saved handle is the whole relationship.~~ [`adr/0141`](docs/adr/0141-a-tenant-owns-what-leaves-with-it-and-the-premises-own-the-capacity.md) gave the tenant its stock as well as its money, so a Household and a Business each carry a saved `BinHead`/`BinTail` and `BinTable` carries a saved `OwnerNext`. ⚠ **The direction of the link is what did NOT change**, which is the sentence above still doing its work: the owner names its Bins and a Bin still names no owner. ***So the shape is owner-side ownership at a third cardinality*** — many per owner, one owner per Bin — and the polymorphic `BinTable.Owner` that would answer *Bin → owner* is **refused and unbuilt**, because its price falls on `Column.Fold` and `SaveHash.TargetsOf`, which are the instrument that proves the simulation deterministic. ⚠ **A tenant's Bin list is SAVED where a Building's is DERIVED**, and the asymmetry is forced: a derived list is only derivable when the element names its owner. ⚠ **A tenant's Bins live exactly as long as the TENANCY** — the premises declare every ceiling, so a Bin cannot outlive the premises that gave it one. The balance is the exception and stays, because money is unbounded and names no premises at all.

⚠ **A balance exists only if the Ruleset in force names money.** A Bin is created for a declared Resource, so a world on a file with no `family = "money"` block has no treasury Bin and no actor balances — where a column held money whatever the file said. ***Making a quantity conditional on the Ruleset turns every fixture's Ruleset into a statement about what that fixture can test.***

**A Bin's level never leaves `[0, capacity]` by any write**, and it can be **above** the ceiling all the same: a capacity is not a property of the Bin but of the Ruleset in force (`adr/0064`), so lowering one in a reload leaves standing Bins holding more than the new ceiling. Such a Bin **drains rather than clamps** — the stock stays, producers stop because Space is negative, consumers are untouched, and it comes back into range by being spent. Clamping would destroy Goods on a keystroke and break conservation, which is the one property the Bin exists to give.
...
**Supply** and **Space**
The two ways a Bin can stop a Rule, and **they are one quantity read from two ends**. A Bin holds a `level` against a `capacity`; **Supply** is that quantity read from the bottom — *there is not enough here to take* — and **Space** is `capacity − level`, the same quantity read from the top — *there is not enough room here to put*. A Rule drawing from a Bin is stopped on Supply; a Rule depositing into one is stopped on Space. Which end stops you is not a property of the Bin but of what your Rule was trying to do, so *empty* and *full* are **one condition seen from two sides** rather than two conditions.

**They are bounds, and a bound is what must be named.** A Bin has exactly two wait lists because the pair does not commute: a deposit can never satisfy a waiter that needs Space, and a withdrawal can never satisfy one that needs Supply, so one list would deadlock in whichever direction it was drained. The two were called *Level* and *Headroom* until 2026-08-14: exact against the code and poor against the city, since *level* does not say which side of the level and *headroom* is also an ordinary word for numeric range, which this codebase uses in that sense elsewhere. ***Name a bound, never a level*** — a name that does not say which end it means will eventually be quoted meaning the other one, which is what `adr/0097` did when it wrote *a stock failure* for a **Space** refusal while the rest of the corpus uses *stock* for the contents.

**Supply is a property of one named Bin and never of the city.** There is no aggregate supply anywhere in this design, for the same reason there is no **Demand** — see *Terms we deliberately do not use*. A sentence about *supply* that does not name the Bin it is about is the banned term wearing its twin's clothes.

**A money Bin's ceiling is unbounded and *derived*, never set once.** `04 §2` is the source and `World.RebuildCapacities` writes it on every rebuild — the column is `(derived AND rebuilt)`, so a value written only at creation does not survive a load, and a reloaded treasury would come back holding a ceiling of **zero**: a treasury every transfer into it fails against. Found in milestone 10 task 4c, live since task 1, and invisible until a fixture had a treasury Bin in it at all.
...
⚠ **AMENDED 2026-08-23 by [`adr/0141`](docs/adr/0141-a-tenant-owns-what-leaves-with-it-and-the-premises-own-the-capacity.md): the subject is the `Building` for a Rule the premises run and the `Occupant` for a Rule the tenant runs**, and which it is was decided by the Rule's own `local` terms when the Ruleset was read — *a Rule whose Bins all belong to a tenant is a tenant's Rule*. ⚠ **It still names its premises whoever runs it**, because a Rule emits under a footprint and reads Readouts scoped to a Building; what moved is *whose Bins its `local` terms address*. **This entry said *one Bin Rule on one Building*, which is the sentence that made three families in one dwelling share a larder.**

**Nothing is allocated when a Rule subscribes and nothing is freed when it wakes.** A Rule Instance's whole life is its **subject's**, so shortage *churn* — which `02 §4.1` identifies as the real cost driver, ahead of chain depth and ahead of how broken the city is — costs no rows at all. ⚠ **For a tenant's Rule that life is the TENANCY** — created when the Household moves in and freed when it moves out — for the reason its Bins have the same life: a ceiling is a function of the premises' kind (`adr/0141`), and an unhoused Household has no kind to read one from. ***A row whose ceiling nothing can derive is a row that must not outlive the thing that gave it one.***
...
**Pool**
A District’s **market** in one Good — ⚠ **not a store, and it holds no stock** ([`adr/0139`](docs/adr/0139-a-district-pool-is-a-market-and-not-a-store-so-stock-stays-with-the-seller.md), 2026-08-22). Goods offered for sale stay in the selling Building’s own Bin; the Pool is the **price** they clear at, the thing a blocked buyer **waits on**, and the set of sellers a District’s connectivity makes reachable. A buyer’s `pool` term resolves to **one seller’s Bin**, chosen when the term is resolved. Goods moving between Buildings within a District pass through instantly, subject to connectivity, and **no Vehicle is simulated** — which is `adr/0013` unchanged and is what reserves the expensive transport simulation for movements the player is meant to optimise. ⚠ **It said *abstract Goods store* until 2026-08-22**, and that sentence is what made the Pool the counterparty to every trade — a party that owns nothing, cannot hold money, and forced a question (`plans/0037` decision 10) that turned out to be the shape of a missing seller.
...
**A tier is earned by attendance, and the city produces its own** (`adr/0104`). When a Household's Life Stage advances and a new Young Household forms, its adults' tier is read off the **Education** Need that Household accumulated while it had children — so a school place actually taken is what raises a tier, and **Sorting is the fast channel rather than the only one**. Schooling therefore *influences* both boundaries and *gates* only the top: a Citizen who missed it still climbs to 2 by working, and takes **longer** over it.

**The segments remain three and there is no tier 0.** A fourth for the never-schooled was considered and refused: the state it names is already *stuck at tier 1, climbing slowly*, nothing in the design would read it differently, and a permanent underclass would be a **second** wall in a design that permits one. It becomes worth a segment the day a mechanism must treat *never-schooled* differently from *unskilled* — a Policy targeting it, an employer refusing it, a Departure reason naming it.
...
**Premises**, and **Unpremised** / **Unhoused**
The **Building an Occupant occupies**, named from the tenant's side. A Building *is* premises; the Household or Business in it is the tenant ([`adr/0141`](docs/adr/0141-a-tenant-owns-what-leaves-with-it-and-the-premises-own-the-capacity.md)). **Unpremised** is a Business with no premises; **unhoused** is a Household with no dwelling. Same state, two actors, and the pair of words exists only because *homeless* means something about a person and these are states of a **tenancy**.

⚠ **Both were terms of art before they were terms.** Two ADR filenames leaned on *unpremised* and the vocabulary defined it in a subordinate clause inside **Business** — which is `CONTEXT.md`'s own rule failing on the word for the thing two milestones are about. ***A word carried by a filename is not owned by anything***, and this entry is where it now lives.

**Neither is a failure state and that is the whole reason they are named.** An unhoused Household waits in the **Unplaced Pool**, and it got there by arriving, by being born, by choosing to look, or by **eviction** — three of the four are ordinary. An unpremised Business waits in a pool of its own on the same terms. ⚠ **Both are BOUNDED states rather than permanent ones**: each waits under a give-up bound and then becomes a **Departure**, which is what keeps either pool inside [`adr/0006`](docs/adr/0006-no-collection-grows-with-elapsed-time.md). ***A pool with an inflow and no sink is the thing that ADR forbids***, so the bound ships on the day the pool does.

⚠ **What the tenant keeps across the gap is decided by whether the premises declared it.** Money is **kept** — it is conserved, it is unbounded, and it names no premises, so losing a Building is not losing what you own (`adr/0054`, and `Invariant.MoneyIsConserved` holds it). **Stock is not**: a Bin's capacity is the premises' kind's ([`adr/0064`](docs/adr/0064-a-bins-capacity-is-a-property-of-the-ruleset-in-force-and-an-over-full-bin-drains-rather-than-clamps.md), `adr/0141`), so a tenant with no premises has no kind to read a ceiling from, and ***a row whose ceiling nothing can derive must not outlive the thing that gave it one.*** **A Household's stock is destroyed at the moment it is evicted**, which is what `World.UnfitOccupant` does today, and a Business's is destroyed on the same terms — it searches holding its till and nothing else ([`adr/0144`](docs/adr/0144-a-tenant-that-loses-its-premises-keeps-only-its-money-and-waits-a-households-wait.md)). ⚠ **Forced for the Household and CHOSEN for the Business**, which could have been left unbounded while unpremised and was not: ***the word Occupant is only worth having if it names one thing.*** **And a Business waits exactly as long** — it shares `[placement] gives_up_after_days` as a declared **stand-in**, because a second bound would be hash-bearing and no world contains a Business to ratify it against.
...
**An immigrant waits at the gate it arrived by, and the move-in is a Trip** (`adr/0129`). Arrival is an entry event at one Outside Connection — that is where the gate's throughput binds — and the journey into the city happens when placement finds the Household a dwelling, because until then there is nowhere for it to go. A Household waiting in the Pool is therefore somewhere: at a gate if it arrived, in the city if it was evicted or born here.
...
The Pool is bounded — a Household that has been looking for longer than it will keep looking gives up and becomes a Departure. `LEGIBLE CAUSE` *(This read **after a limited number of failed attempts** until 2026-08-20. The bound is a **duration**, and the count is recorded beside it rather than being the bound: `adr/0059`'s rule is that authoring a count where the felt quantity is time makes the felt quantity move whenever a cadence is retuned — and a count of dwellings **considered** cannot fire at all in a city with no vacancies, which is the one failure the unhoused channel exists to diagnose. A second bound on **refusals** joins it when acceptance exists, because nothing refuses anything until then — `adr/0130`.)*
...
**The two are Occupants in the same sense and are evicted in different ones.** A Household leaves a demolished Building for the Unplaced Pool with its money intact (`adr/0054`); a Business has no pool, so its row survives holding a **severed** premises handle on `CitizenTable.Workplace`'s precedent — *the premises stopped existing*. ⚠ **Both hold their balance in a Bin and both spell it the same way** (`adr/0114`, task 4c): `adr/0024`'s *"one integer per Household and per Business"* is trivial in the way that sentence claims only if they do, and task 4b shipped a **column** on the Business for a day on the strength of a precedent the same document had already deleted. ***An argument for symmetry does not name which side to move.*** ⚠ **What becomes of a Business with no premises is undesigned** (`adr/0070`) and is filed in [`plans/0002`](plans/0002-open-questions.md) §C. Neither route destroys money, which is what `Invariant.MoneyIsConserved` holds them to.
...
**It is the cursor over a Trip's Legs, and it holds nothing else**: which Citizen, which Trip, which Leg it is on, when it arrives, and — at **Microscopic** resolution only — **how far along that Leg's route it has got**. That last field was added 2026-08-14 (`adr/0075`, amended) and **widened to every resolution the same day** (`adr/0099`): a Traveller at **any** resolution names the Segment it is on, because `adr/0041` attributes volume on Segment entry and somebody has to know which. ⚠ **What a Statistical Traveller still lacks is a position *within* that Segment** — no offset, no lane, no headway — and that, rather than *nowhere in particular*, is the whole distinction from Microscopic and the thing `adr/0007`'s promotion is defined against. It is the field Demotion discards. The **plan** is the Leg's. Every durable thing therefore sits on a row that outlives the journey and every transient thing on a row that is released — which is what makes *a Traveller is a view, not an owner* hold by construction. See [`docs/adr/0075`](docs/adr/0075-a-leg-is-a-plan-and-a-traveller-is-a-cursor.md).
...
**A Node is where a choice exists, and that is what it is for.** It is the unit the routing machinery is keyed in: a route cache entry is `(origin node, destination node, variant)`, a **Rejoin Target** is the Node a diverting Traveller declined to enter, the **Sight Horizon**'s floor is the distance to the next *branching* Node, and a **routing partition** is a set of them.
...
**Routing partition**
**A square tile of the map, sized in Cells, over which the travel-time matrix is keyed and the hierarchical router clusters.** `adr/0040` calls it the *pathfinding cluster*; `adr/0047` gave it this name when it took the role off the District. A partition holds every Node standing on its ground, and one of them is its **access node** — the node nearest the partition's centre, which is where a matrix row is measured from.

**It is not the District, and the whole of `adr/0047` is keeping them apart.** A District is sized by what pools convincingly, which is a Goods playtesting question; a partition is sized by how wrong a matrix entry may be, which is a routing measurement. They were one object because `adr/0014` asserted the identity instead of arguing it, and *a constant welded to two decisions is governed by whichever of them is louder*. Redrawing a District now changes what pools and never what a Traveller drives.

**It is a multiple of the Cell and the Chunk divides it** — `adr/0040`'s owed correction, paid by milestone 5c task 1. That ADR sized it in Chunks, and the Chunk is declared *tuning, hash-preserving* (`05 §4`), so a partition measured in Chunks would let a profiler change the city by turning a knob documented as unable to. The arrow is reversed: the Cell is frozen, the partition is a whole number of Cells, and the Chunk must divide it.

**It covers the developed city, never the map.** Partitions holding no Node are not numbered, because the matrix is quadratic in the count — 16,384 partitions across a 512-Cell map would be a 1.07 GB matrix where the ~1,400 a million-Citizen city occupies is 8.3 MB. `adr/0021`, *scale with developed area, not map area*.

**A matrix entry's error is a fixed distance and therefore a mode-dependent time**, which is why a partition size argued from car speeds is not thereby right for a walk. Half a partition diagonal is a few hundred metres; that is under a minute by car and several minutes on foot, against a Commute Budget denominated in clock minutes for both.

*Avoid*: "District" (it is not one, and `adr/0047` exists because they were confused), "zone" (`plans/0010` and S2's harness use it for this, and a Zone here is a permission set over land), "cluster" unqualified.
...
~~The Tick is fine-grained **because of traffic and nothing else**. Every other process in the simulation is a discrete event that the Event Wheel can schedule at any granularity; car-following is the only continuous one, and it is what sets the resolution everything else inherits.~~ **⚠ INVERTED 2026-08-12 by `adr/0082`.**

**A Tick is a *behavioural* unit, and traffic is the one process that does not inherit it.** A Day is 24 in-world hours over `Ticks.PerDay`, so a Tick is **42.1875 s** of in-world time — a **derived** quantity rather than a free one, because the Ruleset authors speeds in km/h and `05 §26` fixes a Tile at ~4 m. *(⚠ This paragraph read **10.546875 s** and **~45×** until 2026-08-14. `adr/0094` took `Ticks.PerDay` from 8192 to 2048 on 2026-08-13 and the closure reached the ADRs, `CLAUDE.md` and the code and not this entry — `plans/0012` **Cause 2**, in the file every cold start reads for its vocabulary. Both figures are pure restatements of the constant, which is why they could rot without failing anything.)* What sets `TICKS_PER_DAY` is the finest **behavioural** event worth scheduling distinctly; every other process is a discrete event the Event Wheel can place at any granularity. **Car-following is the only continuous coupling and it needs ~180× finer**, so it takes a **sub-step ratio** inside Tick phase 4 rather than dragging the whole simulation down with it — which the arithmetic refuses outright, at 108× the measured Tick budget. Walking is *not* a continuous coupling (`03 §3.8`), so a walk Leg is a departure, an arrival and a cost, and needs nothing from this.
...
**A Day is 24 in-world hours** (`adr/0082`), which is what makes a Commute Budget stated in clock minutes mean anything: at `Ticks.PerDay = 2048` one clock minute is **1.4222 Ticks**, so a 30-minute budget is **43 Ticks**. *(⚠ Read `8192`, **5.6889** and **171** until 2026-08-14, for the reason the Tick entry above now carries. The 171 figure is the one S2 R1.5 measured its Settlement counts at, so **do not read this correction back onto that spike** — its numbers were taken under the old clock and are correct there.)* ***The Lane kernel's sub-step is not a second clock and does not touch the one-clock rule above.*** `adr/0010` forbids a second **slower** time base — a calendar for growth running beside a clock for behaviour — because a conversion factor between two scales breaks the literal truth of *this shop closed because its customers' commutes got too long*. A sub-step is **faster, internal to one Tick phase, and never escapes it**: no simulation quantity is denominated in sub-steps, a Traveller's arrival is still an arrival **Tick**, and nothing outside the Lane kernel can observe that it happened. **One clock survives; what it gained is a finer gear inside a single phase, not a second hand.**
