...
**A Tick is a *behavioural* unit, and traffic is the one process that does not inherit it.** A Day is 24 in-world hours over `Ticks.PerDay`, so a Tick is **10.546875 s** of in-world time — a **derived** quantity rather than a free one, because the Ruleset authors speeds in km/h and `05 §26` fixes a Tile at ~4 m. What sets `TICKS_PER_DAY` is the finest **behavioural** event worth scheduling distinctly; every other process is a discrete event the Event Wheel can place at any granularity. **Car-following is the only continuous coupling and it needs ~45× finer**, so it takes a **sub-step ratio** inside Tick phase 4 rather than dragging the whole simulation down with it — which the arithmetic refuses outright, at 108× the measured Tick budget. Walking is *not* a continuous coupling (`03 §3.8`), so a walk Leg is a departure, an arrival and a cost, and needs nothing from this.
