...
⚠ **The last of those does not work on the shipped map and will.** At 4096² a corner-to-corner drive is under forty clock minutes, so no pair of Districts is out of range of any other and a connected city is **one** Settlement at any Budget that also bounds a Trip — S2 R1.5 measures exactly that. `adr/0089` takes the map to **512 Cells, 65.5 km**, which is 3.7–5.2 Commute Budgets across; the constant is **gated on road generation being scoped to developed land** and has not moved yet. Until it does, read every distance-dependent statement in this file as describing the intended map rather than the built one.
