...
**A fourth arrives at milestone 25, and it does not fail the rule above — it finds the condition the rule assumes.** An Occupant's own Bins — a Household's, a Business's — are `(saved AND hashed)` ([`adr/0143`](adr/0143-a-bin-hangs-off-its-owner-and-the-polymorphic-column-stays-unbuilt.md)), threaded through `BinTable.OwnerNext` off a head and tail on the **owner**. ⚠ **It is saved for a reason neither bullet above gives.** The rule asks whether a list's *order* is recoverable and says membership *"could be rebuilt"* — and for a Building's Bins it can, because a Building-owned Bin names its Building. ***A tenant-owned Bin names nobody***, so its membership is not recoverable either, and the prior condition is the one that binds:

> **A list is `(derived AND rebuilt)` only if the element names its owner. Order is the second question and it is only reached when the first is answered.**

`DistrictPoolTable` reached the same wall one milestone earlier and stated it as *"a derived list is only derivable when the element names its owner"* — a Pool Bin's row does not name its District either. ⚠ **The alternative was to make the element name its owner** — a polymorphic owner column on the Bin, which [`adr/0114`](adr/0114-a-balance-a-rule-can-fail-on-is-a-bin-and-a-bins-owner-is-discriminated.md) gestured at — and `adr/0143` refuses it on **where** the cost lands rather than how large it is: a dispatching fold and dangling check would be paid inside `Column.Fold`, `TargetIds` and `SaveHash.TargetsOf`, which are the machinery [`adr/0112`](adr/0112-the-saved-set-is-the-hashed-set-so-a-save-can-compute-its-own-state-hash.md) rests on and lints **5** and **6** prove determinism with. ***The safest place to not invent machinery is the instrument that detects divergence.*** ⚠ **Its links are handles rather than slot indices**, because a saved list is folded and a slot index is identity — `IndexList`'s `int` form stays right for a derived list and may not be borrowed by a saved one.
...
4. **Thread-count equivalence:** `run(log, threads=1).hash() == run(log, threads=8).hash()`. If we cannot run single-threaded on demand, we cannot debug determinism at all. `RouteWorkerTests` exercises `Simulation.RouteWorkerCount`, including actual worker-result consumption; `plans/0067` owns the measured scope.
...
**Implementation reference:** `Main.Threading`, `SimulationThread` and `RouteBatch` own the live
handoff and route-worker paths. `plans/0067` owns their evidence and remaining snapshot work. The
build-order discussion below remains unratified.
...
- **The failure mode inverts, and the new one is silent.** Polling is self-healing — miss a wake and a Building is merely slow. Subscription is not: a Bin written without draining its wait list leaves that Building asleep forever, with no error and no timer to rescue it. The mitigation is structural, not disciplinary: **Bins are not public fields.** Every write goes through one function that drains the list, so *"every mutation site knows its observers"* is satisfied by there being exactly one. Backed by a sweep invariant — *no Rule is asleep with all its inputs satisfiable* — which is unaffordable per Tick and trivial at the end of a headless run. **It was specified here, in `adr/0033` and in `02 §10`, and built in none of them for the life of Phase 1**; it is now `WaiterIsBlockedByTheBinItNames`, registered in `WorldInvariants` rather than left to a caller, and it fired on the committed golden session within minutes of existing ([`adr/0063`](adr/0063-a-wait-list-wakes-on-the-bins-state-and-a-shortfall-is-derived-rather-than-stored.md)). ⚠ **The built check asks the *head* of the wait list and not every waiter on it**, because the drain stops rather than skips — the sentence in italics above is the specification and is stronger than the mechanism delivers ([`adr/0063`](adr/0063-a-wait-list-wakes-on-the-bins-state-and-a-shortfall-is-derived-rather-than-stored.md), amended 2026-08-22). Same species of bug, and same shape of cheap assertion, as the save/reload test in §7.
...
2. ~~**Tick rate, and how many Ticks make a Day.**~~ **Settled** — ~~`TICKS_PER_DAY = 8192`~~ **`TICKS_PER_DAY = 2048`**, reference rate 16 Ticks/s, world-creation constant. See [`02-simulation-model.md` §1.2](02-simulation-model.md), [`adr/0019`](adr/0019-ticks-per-day-is-a-balance-constant-not-a-pacing-knob.md) and its second amendment, [`adr/0094`](adr/0094-a-day-is-2048-ticks-because-ticks-per-day-is-a-sampling-rate-and-not-a-length-of-life.md), which **withdrew `adr/0019`'s title claim**. ⚠ **CORRECTED 2026-08-19**, and the row below it moved too. Three corrections this produced that matter here specifically: **tick rate is not a simulation parameter** — it is the same action as the speed multiplier, costs CPU linearly and buys no car-following fidelity, since that constraint is expressed in Tiles per Tick rather than in Hertz. **`WHEEL_SIZE` is set by the longest common event horizon rather than by the Day** — which lands on the same number, ~~8192~~ **2048**, because the longest routinely-scheduled event is a work shift. Sizing the wheel *below* a Day to save memory would make sleeping entities wrap and be touched repeatedly, discarding the wheel's entire benefit for 64 KB. And **the Tick loop must dilate wall-clock time rather than skip Ticks** when it saturates, or replay and the State Hash break.
