...
- **Cached Parking Shed membership per Building — `(derived AND rebuilt)`.** The ordered set of **Car Parks** (~~parking Bins~~ — `adr/0113`) within walking distance of a pedestrian Access Point, ~~invalidated by the **Road Graph Epoch**~~ **invalidated per-Segment, witnessed by the walk paths to the Car Parks the shed kept** ([`adr/0083`](adr/0083-a-sheds-use-is-the-arrival-query-and-a-stale-shed-is-wrong-by-a-bounded-walk.md)).
