...
**One assertion in this section is inherited and has never been argued.** *3b must not slip behind 3c.* It is kept because other documents act on it, and labelled because it cannot support them: this document gives no reasoning for it, and [`adr/0015`](adr/0015-all-tuning-data-is-hot-reloadable.md) grounds itself by citing this document straight back — *"the roadmap therefore places the Rule engine third in Phase 1"* — so the two are not independent corroboration, they are one unargued claim counted twice. It has also **already slipped**: slice 6 is 3c's Layers half and it is the current work. Owed to the `adr/0015` session, which [`the board`](../plans/0000-board.md) lists as **A**.
...
| **8** | Parking | That parking is abstracted into a District average, losing the specific diagnosis — and that occupancy leaks, which is an [`adr/0006`](adr/0006-no-collection-grows-with-elapsed-time.md)-class permanent capacity loss |
...
| **10** | Save/load | **Unsaved state**, which the save→reload→compare-hash test is the only reliable way to find |
...
| Terraforming, procedural generation guarantees, the three version numbers in the save header | [`adr/0021`](adr/0021-the-map-is-bounded-procedural-and-terrain-never-enters-a-tick.md) | a milestone, and milestone 10 owes the *three* version numbers where it currently implies one |
