...
| **Decline, abandonment and contagion** | `02 §5.9`, [`adr/0053`](adr/0053-failure-pressure-is-a-duration-not-a-tally.md), [`adr/0091`](adr/0091-clearing-land-is-bought-rather-than-taken-and-demolish-is-the-sixth-verb.md) | a milestone. `01 §6`'s sustained-detection duration is *derived from the time contagion takes to reach a neighbour*, so an authored number is waiting on a mechanism nobody schedules |
| **Private capital — the fourth pacing mechanism** | `02 §5.7`, [`adr/0059`](adr/0059-a-zone-rules-sample-is-a-revisit-period-so-the-ruleset-states-a-duration.md) | a milestone. `adr/0059` states the split: *"sampling paces cost, and **capital** paces growth. Until capital exists, growth is bounded by the Unplaced Pool"* — which is what the city does today, and it is a stand-in rather than the design |
| **Commercial and industrial placement — growth-loop steps 1 and 3** | `02 §5.2`, `02 §5.5` | a milestone, and **distinct from the residential choice model above**: *unserved need in reach*, reachable inputs and buyers, Extraction pinned to the ground. `04 §1`'s Materials-gate-construction argument runs entirely through it |
| **Agglomeration — Productivity, Amenity, the congestion cost** | `02 §5.5` | a milestone. It is a **termination condition rather than a feature** — `02 §5.5`: *"With only the first two, cities grow without bound"* |
| **Shipments — freight Vehicles between Districts and to the gate** | [`04 §3`](04-economy-and-goods.md), `02 §6` | a milestone. ⚠ `CONTEXT.md` → Good makes freight **the only family that contributes congestion**, so every congestion figure taken before it exists is taken on commuters alone |
| **Household finances — the savings reserve, discretionary spend, destitution as an absorbing state** | `CONTEXT.md` → Money and Unemployment, `01 §6` | a milestone. The *conserved Money* row above supplies the **treasury**; this is the **balance sheet**, which is what the Destitute Departure channel and `04 §5`'s tax-as-velocity argument both read |
| **Needs, their decay, and the housed Departure comparison** | [`adr/0102`](adr/0102-a-housed-departure-is-a-comparison-the-household-re-runs-not-a-threshold-it-crosses.md), [`adr/0103`](adr/0103-a-need-is-where-a-frequent-private-failure-accumulates.md), [`adr/0104`](adr/0104-a-skill-tier-is-earned-by-attendance-and-the-credential-stays-a-wall.md) | a milestone, **owed on the day they were written (2026-08-15)**. 9a schedules the Pool and the *unhoused* Departure; the accumulator, its decay, the re-evaluation countdown and the Education-Need-to-Skill-Tier step are in no row. All three are downstream of the Provider List |
...
| **`Demolish`, and the four routes to cleared land** | [`01 §2`](01-player-experience.md), [`adr/0091`](adr/0091-clearing-land-is-bought-rather-than-taken-and-demolish-is-the-sixth-verb.md) | a milestone. 5a-bis built the **road** bulldoze only. `CONTEXT.md` → Derelict makes `Demolish` a derelict Building's **sole** exit, so this is also [`adr/0006`](adr/0006-no-collection-grows-with-elapsed-time.md)'s sink for standing abandoned stock |
| **Shocks, Disasters and Hazard Regions** | `01 §5.2`, `01 §5.3` | a milestone, and **not** the Terraforming row. `01 §5.2` names a disaster as *"the only instrument that can measure redundancy"*, and the Intensity Dial's third sub-dial has nothing to scale without it |
| **The Intensity Dial, the Modes and the lock policy** | `01 §5.4`–`01 §5.6` | a milestone, **and it is core work rather than shell** — `01 §5.7` puts the dial in the **Input Log**, so it is simulation input and subject to replay |
| **Trajectory detection and notification** | `01 §6` | a milestone. Nine named syndromes, and the indicators behind most of them are produced by nothing: Replacement Rate, Retention, Destitute Departures, the underemployment share, the money supply. It also owns the checkable *no orphan figures* test over the Evidence graph |
| **Overlays as a primary view, and the *modelled* marking** | `01 §7`, [`03 §3.9`](03-agent-architecture.md) | a milestone. ⚠ The marking rule is the **entire `HONEST DEGRADATION` discharge for the Microscopic Cap** — `01 §6` deleted the Cap's failure-mode row on the strength of it — and `CONTEXT.md` → Service demotes coverage to this overlay, so the Services row above depends on it too |
| **`Diagnose`'s second half — re-running the Zone Rule predicate on an empty Lot** | `01 §1`, `CONTEXT.md` → Frontage | a milestone. It is the design's answer to its **most common signal, the non-event**, which Evidence structurally cannot explain because nothing happened. It needs one stored field — *when was this Lot last looked at* — and that is the whole retrofit cost |
| **Pin** | `01 §7`, `CONTEXT.md` → Pin | a milestone. `01 §6` makes Pin the **third detection path**, and it is the honest limit on a notification system that can only ever warn about failures somebody thought of in advance |
| **A second Trip generator** | `plans/0021` §A, [`adr/0032`](adr/0032-services-are-delivered-by-trips-not-by-coverage.md) | a milestone. 5b-bis discharged **one of seven**. ⚠ Every Commute Budget rung and every congestion figure in the corpus is calibrated against that one flow, so this is an **ordering** question rather than a scheduling one: a second generator arriving after 6 invalidates 6's and 7a's baselines |
...
| **`05 §6`'s threading policy** | `plans/0002` §C; `plans/0024` | **An argument.** ⚠ Both documents say it *"gates milestones 10 and 11"* — **and there is no milestone 11**; Phase 2 ends at 10. One fact, two homes, and the milestone it half-names has never existed. It also holds `plans/0013` lever 2, the largest unclaimed multiple in the ledger |
