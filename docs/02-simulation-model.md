...
**⚠ What milestone 9 actually composes is two of those terms — `− w₂·pollution − w₃·noise` — and amenity is absent from it** ([`adr/0123`](adr/0123-desirability-ships-without-its-only-positive-term-and-a-caveat-that-must-travel-gets-a-test.md)). Amenity needs a **kind** on a Business, which milestone **15** supplies; shoreline needs Water Bodies, which nothing has built. Both are **absent from the formula rather than defaulted to zero**, and the two absences are not alike: shoreline is zero and *that is true of every world that exists*, while **amenity is the only positive term in the composition**. So until 15, desirability is **bounded above by zero** and land value is a **disamenity field** whose maximum is clean, quiet, empty ground. ***That is not a hole that fails loudly — it is a working mechanism that says something false about cities***, which is the opposite failure from the one *leave named holes rather than placeholders* was written for, and it is why this paragraph exists and why a corpus check refuses any document to say desirability is composed without saying amenity is missing from it.
