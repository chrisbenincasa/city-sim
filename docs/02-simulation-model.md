...
> ⚠ **Corrected 2026-08-22: the matrix role is struck, and [`adr/0047`](adr/0047-routing-never-keys-on-the-district.md) removed it.** Routing never keys on the District, so **redrawing a boundary changes what pools and never what a Traveller drives** — which is the whole point of that ADR, and `CONTEXT.md` → District has carried the corrected wording since. This paragraph did not, and it is the one a reader arrives at first. ***A role that was deliberately detached is re-attached by whichever document still lists it***, and the paragraph below inherits the error where it says extent decides *"Goods pooling and matrix granularity"* — the Chunk argument stands on **pooling alone**. Filed to [`plans/0012`](../plans/0012-corpus-audit.md).
...
> 🔴 ⚠ **SUPERSEDED IN HALF 2026-08-22 by [`adr/0132`](adr/0132-the-district-is-derived-and-a-ward-is-what-the-player-draws.md): automatic stands, and the player arm is withdrawn.** The pen moved to a separate object, the **Ward** — a named set of Cells the player draws that Policy is scoped to and that has no logistics consequence. **The two roles this section states four lines apart want opposite things from one line**: extent is *physics*, and a Policy scope is *nothing but a choice*. Welding them let a player redraw the logistics abstraction while believing they were drawing an administrative boundary, and [`adr/0013`](adr/0013-goods-are-pooled-within-a-district-and-shipped-between.md) had the conclusion in its own trigger list — *"a boundary the player draws to switch off a subsystem is a boundary that is not a gameplay concept."* ⚠ **The physics half below is not merely intact, it is the half that forced the split** — read it as the reason the pen had to go, not as surviving argument for it. The original text follows.
>
...
>
> The early city therefore has one District because the city **is** one neighbourhood, not because of a rule; more appear as it outgrows the pooling radius. ~~Splitting and redrawing is a late-game advanced action that arrives exactly when one end of the map genuinely differs from the other, and it is also what makes District-scoped Policy targetable — you cannot aim a policy at a boundary you did not choose.~~ 🔴 **Struck by [`adr/0132`](adr/0132-the-district-is-derived-and-a-ward-is-what-the-player-draws.md).** ⚠ **The second clause is true and is exactly why the split had to happen** — *you cannot aim a policy at a boundary you did not choose* is an argument that the **Policy scope** must be the player's, and this sentence answered it by handing over the **pooling** boundary. Policy is now scoped to a **Ward**, which the player does choose; the District is derived and is not redrawn at any stage of the game. ***An argument for one role was discharged by conceding the other, which is what a weld looks like from the inside.***
...
**Base Fertility** is the ceiling — the yield of untouched ground — and it is **Ruleset data keyed by terrain type, never stored** ([`adr/0155`](adr/0155-base-fertility-is-ruleset-data-keyed-by-terrain-type-and-the-old-name-invented-a-field.md)). ⚠ **It was called *terrain suitability*, and the old name invented a per-Cell field this section's own opening sentence forbids.** **Sealing** is the count of Tiles in a **Cell** ever built on, decaying at a Ruleset rate keyed by the same terrain type. **Pollution** is the existing layer, already diffusing. Fertility itself is *composed at the point of use and never stored*, per the rule in §2.4 — so this needs no new layer, and a farm reading its yield is an ordinary Rule reading Map Layer Cells under its footprint.

⚠ **It is weighted and the bare subtraction was never an implementation** ([`adr/0156`](adr/0156-fertility-composes-with-weights-and-only-one-of-them-is-a-number-anybody-chooses.md)): the three terms are a fraction, a Tile count of 0–1024 and a stock measuring about 12, so unweighted the Sealing term outweighs pollution roughly 85:1 by choice of units. **Base Fertility is a fraction with `1.0` fully fertile**, so fertility is a proportion and `adr/0022`'s *"41% — ground sealed 12%"* panel needs no conversion. 🔴 **`w_s` is derived from an endpoint** — full Sealing means every Tile built on, so no farmland — and has **no Ruleset key**; only the pollution weight is chosen. **Fertility may go negative and does not clamp**, because Sealing decays and the ordering between exhausted Cells is what the cyclical land-use arc runs on. ⚠ **Built at milestone 24 task 5** — read `MapLayers.Fertility` for what the composition actually does; the pollution weight is `[layers] fertility_pollution_percent` and is **unratified**, with `plans/0002` §D1 holding what would settle it. 🔴 **Nothing consumes it yet**, because no milestone in `06` builds a farm — so every number in the three terms was chosen against no consumer.
...
| **Woodland** | stored per Cell, **not diffused** | the generator, once | **a Day** — period `TICKS_PER_DAY`, offset 80 | a count, not a field, and **Sealing is its ceiling** ([`adr/0159`](adr/0159-woodland-is-a-tile-count-per-cell-bounded-by-sealing-because-the-ground-has-one-budget-and-not-two.md)). ⚠ **It grows back toward what the SEED laid** — a second saved column — because growing toward the bare Cell would converge every map on uniform forest and erase the seed's character |
| **Sealing** | stored per Cell, **not diffused** | construction — **roads and Buildings alike** | **two occasions, and only one of them is a cadence**: it rises on build, **at the point of laying**, and falls on a **Day** — period `TICKS_PER_DAY`, offset 48 | a count, not a field. **One Tile per built Tile, and roads are ~86% of it** ([`adr/0150`](adr/0151-sealing-authors-no-width-and-a-road-seals-where-it-is-laid-not-where-its-endpoints-are.md)). ⚠ **Its decay rate is keyed BY TERRAIN TYPE** — a `sealing_decay_tau` on each `[[terrain]]`, not a `[layers]` key, and the `[layers]` one is refused — because *rock may never recover, floodplain may recover over hundreds of Days* |
| **Radiation** | stored Layer, diffused — **industrial pollution's mechanism, its own source and its own rate** | a point source that deposits | its own cadence | ⚠ **Same geometry, same kernel, same range, and NOT one field with it** ([`adr/0153`](adr/0153-radiation-is-industrial-pollutions-mechanism-with-its-own-source-and-its-own-rate-because-a-shared-decay-is-wrong-for-one-of-them.md)). §2.5 guard rule 1 sorts on geometry and range and is **silent on persistence**; `DecayPollution` holds one `tau` for the whole Layer, so a shared column is a shared half-life and ***a shared decay is wrong for one of them in exactly the way a shared kernel was***. **The decay is small and never zero** — a source has no ceiling by construction where Sealing does. 🔴 **Nothing emits it and no milestone owns it** |
...
| Water pollution | **Bin per Water Body**, plus a shoreline source | ~~dumping~~, **runoff** | on transaction | network transport, not spread. `CONTEXT` → Water Body. ⚠ **Runoff only** — dumping needs a `Scope` that reaches a Water Body and is unbuilt (milestone 24) |
...
than about when it happens** ([`adr/0150`](adr/0151-sealing-authors-no-width-and-a-road-seals-where-it-is-laid-not-where-its-endpoints-are.md)).
...
**Composition rule: compose at the point of use, do not bake composites into stored layers.** Desirability is `− w₂·pollution − w₃·noise + w₄·amenity − w₅·shoreline`, evaluated lazily where it's needed. A stored desirability layer would need invalidating whenever any input changed, and would drift. **Land value is not a term in its own target** — it was, until [`adr/0122`](adr/0122-land-value-is-not-a-term-in-its-own-target-and-a-term-on-both-sides-of-a-lag-is-a-gain.md), and a `w₁·land_value` in a composition the paragraph below then drifts *toward* is a **gain** of `1/(1 − w₁)` on the other four terms rather than a fifth weight, divergent at `w₁ ≥ 1`. The momentum supplies the persistence it looked like it was for.

**⚠ What is composed today is THREE of those terms — `− w₂·pollution − w₃·noise − w₅·shoreline` — and amenity is absent from it** ([`adr/0123`](adr/0123-desirability-ships-without-its-only-positive-term-and-a-caveat-that-must-travel-gets-a-test.md)). Milestone 9 composed the first two; **`w₅` shoreline landed at milestone 24 task 7**, once runoff gave a Water Body's Bin an inflow — before that it would have been the present-and-permanently-zero term `0123` refused. ⚠ **On a world with no water `w₅` is still ABSENT rather than zero**, which is the same distinction one level down: a Ruleset omitting `[water]`, or stating it with no Bin, drops the term. Amenity needs a **kind** on a Business, which milestone **15** supplies, and it is **absent from the formula rather than defaulted to zero**. ⚠ **Closing one of `0123`'s two holes did not close the caveat**, because **amenity is the only positive term in the composition**: until 15, desirability is **bounded above by zero** and land value is a **disamenity field** whose maximum is clean, quiet, empty ground beside clean water. ***That is not a hole that fails loudly — it is a working mechanism that says something false about cities***, which is the opposite failure from the one *leave named holes rather than placeholders* was written for, and it is why this paragraph exists and why a corpus check refuses any document to say desirability is composed without saying amenity is missing from it.

**⚠ The composition is at a Tile and land value is stored at a Cell, so something has to reduce — and the reduction is not a quadrature that converges** ([`adr/0126`](adr/0126-a-cell-samples-desirability-at-its-quadrant-centres-and-a-line-sources-area-mean-does-not-converge.md)). A Cell's target is the mean over its **four quadrant centres**, at Tile offsets 8 and 24. The shipped geometry makes the two obvious single samples the two worst ones — a Cell is 32 Tiles and `[roads] block_tiles` is 32, so the lattice lands on Cell edges and the centre is systematically the quietest Tile in the Cell while a corner is systematically a junction. ⚠ **And raising the sample order does not settle the answer**: the area mean of a line source diverges, because the sources are *on the boundary*. Measured at orders 1, 2, 4, 8 it climbs 17.7%, 9.2%, 4.1% and keeps going. ***So the sample set defines the Cell's value rather than estimating one.*** What justifies order 2 is that the **ordering between Cells** survives it — 615 of 630 pairs agree with order 8, the other 15 being ties inside 1% — and ordering is all anything reads while no consumer reads an absolute land value.

**⚠ And the target never stops moving, so land value never rests — it orbits** ([`adr/0127`](adr/0127-the-land-value-target-never-stops-moving-so-the-question-is-what-the-lag-rests-around.md)). The noise term reads a Segment's volume **at the instant it is asked**, and land value is the only part of the composition with memory, so desirability is zero-noise at midnight and peaks at eight in a city where everybody drives. Measured at steady state on a world that emits: **185 of 262 resident Cells move on a typical cadence sample**, the widest peak-to-trough swing is **74,373** raw Q16.16 units, and **the field does not trend** — the mean over one half of a four-Day window is −567,787 against −563,871 over the other. ⚠ **This is not the ±1 dead-band flicker**: that would be a swing of *one* raw unit, and the **fifty Cells that never move at all** — clean ground, target zero, gap zero — are the direct proof that the lag's minimum step terminates. ***Whether land value should swing with rush hour at all is a design question with no number that refutes it***, and it is filed in `plans/0002` §C rather than settled.

**⚠ The weights are unratified, and `w₂` and `w₃` carry *two* entries each in [`plans/0002`](../plans/0002-open-questions.md) §D1 rather than one** ([`adr/0125`](adr/0125-a-ratifier-that-needs-a-consumer-nobody-built-is-not-reachable-so-the-weights-get-a-floor-and-a-debt.md)). **Nothing in the city reads land value** — outside `MapLayers` its only readers are a headless dump and the Ruleset loader — so the quantity that would refute an absolute scale is produced by a consumer and **every consumer is unbuilt**. ***A ratifier that needs a consumer nobody built is not reachable.*** What is reachable now is a **floor**: on `rulesets/congested.toml`, that the field varies at all, that both terms are visible in it, and the pollution/noise correlation across Cells — the last because industry and roads are placed in proportion to the same population, and **if the two sources co-vary no ratio of `w₂` to `w₃` is identifiable at all**. The **real** ratifier is owed and triggers at **milestone 13**, the price surface. ***The floor refutes and never confirms***, and the two entries exist separately because a reachable check standing in the place of an unreachable one is how a number comes to look settled.
...
|---|---|---|
| `local` | Bins on the **subject** running the rule — its Building, or its Occupant | **mine — free** |
...
> ⚠ **AMENDED 2026-08-23 by [`adr/0141`](adr/0141-a-tenant-owns-what-leaves-with-it-and-the-premises-own-the-capacity.md): the subject of a Rule is not always its Building.** ~~Bins on the Building running the rule.~~ A Bin belongs to the **Occupant whose leaving would empty it** and to the premises otherwise, and **Rules follow their Bins** — so `local` on a tenant's Rule means *the tenant's*. ⚠ **The third column does not move, and that is the point**: the scope still answers *whose is it* and it still answers **mine**. What moved is who *mine* is. A Rule's side is **derived** from its own `local` terms when the Ruleset is read — there is no `owner` key on a `[[rule]]`, because the terms already say it — and a Rule addressing both sides is **refused at load**, which is this section's own next paragraph arriving inside a single Building: ***a term crossing an ownership boundary is a trade***, and a trade is `pool`. ⚠ **A Rule still names its premises whoever runs it**, because a `map` term emits under a footprint and a `derived` apply count reads a Readout scoped to a Building.
...
> ⚠ **AMENDED 2026-08-18 by `06` milestone 10: `global` is built, so this paragraph is now about `pool` alone — and the two halves it welded together turn out to have opposite answers.** ~~Only `local` and `map` are implemented.~~ **`local`, `map` and `global` are implemented**; the treasury is where a Bin nobody owns lives ([`adr/0114`](adr/0114-a-balance-a-rule-can-fail-on-is-a-bin-and-a-bins-owner-is-discriminated.md), [`adr/0116`](adr/0116-the-treasury-opens-empty-and-a-founding-balance-is-a-ratio-this-milestone-holds-neither-side-of.md)), fitted with one unbounded empty Bin per **conserved** Resource at world creation. ~~neither is refused at load — deliberately, because a Ruleset naming a Pool is well-formed and merely early~~ — **that reason is `pool`'s, and it was given for both.** A `pool` term is *unbuilt* and stays unrefused, because every Good in this design crosses a market and such a file is going to be legal. A `global` term naming a **non-conserved** Resource is not early and never becomes legal: no Bin is fitted for it in any world this design describes, so **it is refused at load** — which is §4.3's own *"that is the shape the loader accepts"*, six sentences below, finally implemented. ***An `and` in a consequence is two consequences*** (session F), and the tell here is that one reason was offered for two holes. **What still stands:** the worked chain below loads clean and throws the first time `draw_flour_from_pool` is reached, and §1's phase table's contention example is not reachable yet. *Original paragraph:* **Two of the four are named holes, and this is a statement of design rather than of what runs.** Only `local` and `map` are implemented. `pool` needs road connectivity and `global` needs somewhere for a Bin nobody owns to live; both **throw when a Rule reaches one**, and neither is refused at load — deliberately, because a Ruleset naming a Pool is well-formed and merely early. The practical consequence is that **the worked chain below loads clean and throws the first time `draw_flour_from_pool` is reached**, and the contention example in §1's phase table, which contends over the Pool, is not reachable yet.
...
**A Policy's population is `household` or `business`; `building` is declared and refused.** [`adr/0149`](adr/0149-a-business-is-a-population-a-policy-sweeps-and-a-readout-names-every-entity-it-reads-against.md) built the second one, and what it needed from the subject was **a slot count, a liveness test and a balance Bin** — a balance is a Bin whoever holds it ([`adr/0114`](adr/0114-a-balance-a-rule-can-fail-on-is-a-bin-and-a-bins-owner-is-discriminated.md)), so the two sweeps are the same loop over a different table. ⚠ **The two remaining absences are not the same kind.** A Business population is *every live row*; a Building population is whichever rows **a predicate selects**, and that predicate does not exist — so what is missing there is a mechanism, where what was missing for a Business was a loop.

⚠ **The family is chosen on behaviour and never on what is cheap to build** (`adr/0033`). A levy on a Business is an *entitlement* and belongs here; a shop consuming flour and producing bread is a Bin Rule and belongs in §4.1. The reason the second does not exist is that a Business holds no stock, which is *unbuilt* rather than refused.

**A Readout names every entity it can be read against, and that is part of its declaration.** `balance` is readable against a Household **and** a Business; `occupancy` only against a Building. A Rule naming a Readout its subject has no row for is refused at load, which is `adr/0048`'s division of labour — and the check is a **membership test** rather than an equality, because one scalar can belong to more than one entity. ⚠ **The widening is what to be careful with**: a Readout carelessly added to two sets is a Rule reading a quantity that means a different thing on each, which no test can catch and a single scope made unreachable.
...
`global` names the **treasury**, and it appears only as the far end of an explicit **transfer** — `local` money out, `global` money in, balancing within the one atomic Rule. That is the shape the loader accepts and the only spelling available for a counterparty that is not a market: a `pool` term is a *purchase* under `adr/0050` and authors no payment at all, and a `local`→`local` money pair nets to zero and bounds nothing. So the treasury is a destination, never a balance-holder in the sense actors are. ~~**No transfer executes today**, since `global` throws.~~ ⚠ **BUILT 2026-08-18** (`06` milestone 10 task 1): `global` resolves to the treasury's Bin for the named Resource, a `local`→`global` money transfer executes with money conserved across it, and a Building that cannot pay waits on **its own** Bin rather than on the treasury's — which is this section's *no actor's balance is ever `global`* holding at run time. ⚠ **The sentence *that is the shape the loader accepts* was a specification and not a description until milestone 10 task 3**: the loader accepted `global` on any Resource for six slices, because the Rule engine's throw was doing the validating by accident. It now refuses `global` on a non-conserved Resource at load. ***A scope that throws is a scope nothing has to validate.***

> ⚠ **What *held at `local` scope* means structurally was settled 2026-08-18 by [`adr/0114`](adr/0114-a-balance-a-rule-can-fail-on-is-a-bin-and-a-bins-owner-is-discriminated.md), and the build had it the other way round.** A balance is a **Bin**, not a column — `HouseholdTable` held `Money` and `Savings` as `Column<Money>`, which no Rule can reach. The reason is not storage: `RuleEngine.Check` keys everything on a **Bin slot**, `RuleVerdict.Stopped` carries a **bin** as its blame target, and a blocked Rule queues on that Bin's wait list and is woken by a write to it. ***A balance a Rule can fail on is a Bin, because the failure surface is what a Bin is for*** — which is what makes [`adr/0050`](adr/0050-crossing-an-ownership-boundary-is-a-trade-and-payment-is-implicit-in-the-scope.md)'s *two Bins, two blame targets* split, and therefore bankruptcy as a diagnosis, buildable at all.
>
> **So `BinTable.Owner` becomes a discriminated handle** — Building, Household, Business, or the treasury. A money Bin cannot be Building-owned, because an evicted Household keeps its balance while occupying nothing ([`adr/0054`](adr/0054-a-demolished-buildings-households-are-evicted-into-the-unplaced-pool.md)) and the treasury is nobody's Building, which is the ground the `global` refusal states above. The paragraph's *no actor's balance is ever `global`* is **unchanged and is the reason the owner kind exists**: an actor's Bin is its own, and only the treasury's carries the treasury kind.
>
> ⚠ **`HouseholdTable.Savings` is deleted with it, and it was never a second account.** [`adr/0024`](adr/0024-money-is-conserved-and-the-city-has-a-balance-of-payments.md) asks for a *"reserve **sized by** its Life Stage"* and a *"savings **buffer**… where **velocity** is set"* — a **threshold** on one pool, not a second pile — and a Household has one balance. ***A threshold stored as a stock reads as a second account, and every document that later names the pair inherits it.***
...
> **⚠ That clamp is also what sizes money's smallest unit** ([`adr/0115`](adr/0115-moneys-unit-is-fixed-by-the-smallest-fraction-the-design-multiplies-by.md)). A `price` is an integer count of that unit and the step is floored, so a quantity `Q` moves only where `0.1 × Q ≥ 1` — **a price below ten units is frozen**, and the loop above is a permanent no-op on it for ever. *"The tâtonnement does all the work from there"* therefore holds only above a floor this section sets and never states, and **the floor moves with the clamp**: narrowing to ±5% doubles the unit the whole economy must be denominated in. ***A design that has chosen its fractions has already chosen its unit.*** There is no key to author — the scale is implied by the money quantities a Ruleset writes down — and §4.1's `FloorDiv(readout × percent, 100)` is the second site, where the same arithmetic collects **nothing** from a balance under `100 ÷ percent` and the stated rate from everybody else.
...
> 🔴 **AMENDED 2026-08-26 by [`adr/0170`](adr/0170-an-abandoned-shell-collapses-on-a-clock-because-a-bound-is-not-a-sink.md): THE SHELL FALLS ON A CLOCK, and the sink sentence above is true and insufficient.** A `[[building]]` kind states **`collapses_after_days`** beside `condemn_after_days`, and an abandoned Building's Lot returns to vacant that many Days later **with no player act involved**. ⚠ **The bound is real and it is not a sink.** Measured on `rulesets/declining.toml` at 10,000 Citizens with nothing but the collapse removed: **1,201 built → 0 built by Tick 32,768**, flat to 65,536, every one of the 1,374 Lots either a permanent shell or unbuildable from Tick 0. ***`adr/0006` is green the whole way down*** — the collection stops at exactly the Lot count this paragraph names — so a long-run assertion watching for an upward trend reports health over a dead city. **A bound answers *does it grow for ever*; it does not answer *can the city come back*.** ⚠ **Redevelopment stays the preferred sink and stays unbuilt**, because it reads the land value Map Layer's target; the clock is the floor under it and the case where nobody wants the land. ⚠ **`Demolish` and the clearance programme are unaffected** — they stop being *the* sink and become the player's fast path over the city's slow one.
...
> ⚠ **AMENDED 2026-08-23 by [`adr/0141`](adr/0141-a-tenant-owns-what-leaves-with-it-and-the-premises-own-the-capacity.md): pressure has TWO subjects and therefore two verdicts.** A tenant runs its own Rules over its own Bins, so **the premises' pressure condemns the Building and a tenant's ends the TENANCY**, leaving the premises standing. ⚠ **The mechanism above is untouched** — same durations, same reset, same *missed firings*, same threshold, same claim that sampling reads the duration rather than producing it. ***Only the subject of the verdict moved.*** 🔴 **What it removes is a real defect**: `ZoneRuleEngine.Condemn` took the longest pressure across the Building's whole Rule list, so **one starving tenant condemned the other's shop** — and once a tenant had Rules of its own, that list was full of other people's. ⚠ **A tenancy that ends is NOT a fifth entry route to the Pool**: it is the same eviction through `World.Unplace`, reached by a new trigger. ⚠ **Nothing records *why* such a tenancy ended.** The condemnation trail is a **Lot's** — it names the Lot, the kind and the condition behind a demolition — and a tenancy that ends leaves the Lot, the kind and the Building as they were, so an entry there would be a demolition record for a Building still standing. The channel that carries *why is this Household unhoused* is [`adr/0130`](adr/0130-the-pools-bound-is-a-duration-and-the-unhoused-channel-ships-with-the-gate.md)'s and ships with the Pool's give-up bound.
...
3b. **Every field is declared once as either `(saved AND hashed)` or `(derived AND rebuilt)`**, and the save serialiser and the State Hash are generated from that single declaration. Without it, **a field that is saved but not hashed is invisible to every tool here**: two runs diverge on it, the hashes agree, replay reports success, and the save/reload test passes because the field *is* saved. The oracle certifies a divergence it cannot see. Composition order falls out — tables in declaration order, arrays in index order. See `adr/0003`. ⚠ **A third declaration joined the two in milestone 8** ([`adr/0110`](adr/0110-scratch-is-a-third-disposition-because-derived-is-a-claim-a-scratch-column-does-not-make.md)): **scratch**, neither saved nor rebuilt, for a phase-local intermediate that is written and read inside one phase and means nothing between them. It does not weaken the pair above — the forbidden state is *saved but not hashed* and scratch is neither — and it carries an obligation the other two do not, *nothing may read it outside the phase that wrote it*, which is asserted by filling it with garbage and requiring the State Hash trace not to move.
...
For a **Citizen**: home, workplace, current activity, current or last Trip with its Fate, need satisfaction, and household finances. **And, where there is no workplace, *why* — on the Lot row's own principle below.** ⚠ **This row was violated by the first mechanism that could violate it, and the violation shipped**: milestone 5b-bis's `jobs beyond budget` counts Citizens the Commute Budget excluded and keeps **no entity reference**, so a 100,000-Tick run could report *distance rather than supply is what separates them* in aggregate and name **nobody** it was true of. [`adr/0097`](adr/0097-a-reach-failure-is-counted-on-the-citizen-and-a-stock-failure-is-not-remembered-at-all.md) repairs it with a **reach-failure count on the Citizen** — history rather than a stored reason, because *why am I jobless now* is recomputable on a click and *have I been failing persistently* is not, and persistence is what separates bad luck from **structural exclusion**. The count is also the memory [`adr/0047`](adr/0047-routing-never-keys-on-the-district.md) asked for and [`adr/0017`](adr/0017-agents-satisfice-they-never-optimise.md) recorded as owed; its consumer is milestone 19's Departure, and it is deliberately read by nothing until then.
...
> (`adr/0084`: an invariant over absent state cannot be written at all).

| Tier | When | What |
|---|---|---|
| **Per Tick** | every build | Only `O(1)` and `O(changed)` checks — no Bin negative or over capacity at the write site (`BinLevelIsWithinCapacity`), no Trip without a Fate (`TripHasAFate`), and a parking space given up once (`ParkingSpaceIsReleasedOnce`). ⚠ **This cell also said *parking occupancy conserved* until 2026-08-19, and it had said it since before the ADRs** — [`adr/0084`](adr/0084-parking-occupancy-is-two-checks-and-an-invariant-over-absent-state-cannot-be-written.md) split that obligation in two and demoted the sum to the end-of-run tier on this table's own *sort by frequency* rule, and the per-Tick half is the release check beside it. ***Splitting an invariant leaves the tier it came from naming something that moved*** |
| **Staggered** | every build, one slice per Tick | The `O(n)` sweeps, amortised across the population the same way Sweep Rules are: Goods conserved (`GoodsAreConserved`), no Citizen in two places (`CitizenIsInExactlyOnePlace`), every Household's home exists and lists them as occupant (`HouseholdIsHousedOrInThePool`, `HouseholdIsAnOccupantOfItsHome`) |
| **End of run** | headless suite | The expensive whole-world walks: **money conserved** — ⚠ **two checks rather than one, and this cell named only the weaker until 2026-08-19**: `MoneyIsRepresentable` is `adr/0003`'s overflow detector, which catches an accumulator with no sink and is content with any representable sum, and `MoneyIsConserved` ([`adr/0031`](adr/0031-one-resource-abstraction-and-depth-not-count.md), `06` milestone 10 task 4) is the equality against the money supply of record, which catches money created by a write with no counterparty and money destroyed with the row it sat on. ***An overflowed sum is also an unequal one***, so the overflow check is registered first and the ordering is a test — every cross-table handle valid (`CrossTableHandleResolves`), **summed Car Park occupancy against the Citizens holding a space** (`ParkingOccupancyIsConserved`, [`adr/0084`](adr/0084-parking-occupancy-is-two-checks-and-an-invariant-over-absent-state-cannot-be-written.md), `06` milestone 7 task 6) — the pairing is what breaks and a pairing defect is a property of a run rather than of a moment, which is `SegmentVolumeIsConserved`'s and `BinCapacityMatchesItsDeclaration`'s demotion a third time — *no Rule asleep with all inputs satisfiable* (`adr/0033`) — **built at last as `WaiterIsBlockedByTheBinItNames`, and narrower than that wording on purpose: it asks whether the Bin a waiter *named* still blocks it, which also catches a waiter resubscribed to the wrong Bin** ([`adr/0063`](adr/0063-a-wait-list-wakes-on-the-bins-state-and-a-shortfall-is-derived-rather-than-stored.md)) — ⚠ **and narrower again in *which waiter* since 2026-08-22: the HEAD of the list and only the head, because the drain stops rather than skips, so a covered waiter queued behind an uncovered one is parked correctly. Asked against the level less what armed rows will draw, since a woken waiter records no claim anywhere and the drain's guarantee is true of an instant** ([`adr/0063`](adr/0063-a-wait-list-wakes-on-the-bins-state-and-a-shortfall-is-derived-rather-than-stored.md), amended) — and, since milestone 12, **the Districts**: every membership row names a live District and a Cell that still holds Buildings (`ADistrictCellNamesALiveDistrictAndBuiltGround`, task 4), and every District's Pool is one live Bin per Good (`ADistrictPoolIsOneLiveBinPerGood`, task 5). ⚠ **The first of those was built at task 4 and did not reach this cell until task 5 added the second**, which is [`plans/0012`](../plans/0012-corpus-audit.md) **Cause 2** — *an ADR or a task issues a write to another document and the write does not land* — arriving in a table whose whole job is to be the enumeration. ⚠ **Neither is the only cover for its dangling half**: `CrossTableHandleResolves` is column-driven and reaches every saved handle column in the world the day it is declared; what the two above add is the **unset** handle and the **missing** row, and a missing row is what no walk over what exists can see |
...
3. ~~**Districts: player-drawn or automatic?**~~ ~~Player-drawn gives the player a real lever over the logistics abstraction and is more legible; automatic is less to explain and less to get wrong. Leaning player-drawn with an automatic default.~~ 🔴 **CLOSED — and then narrowed the same day. Struck 2026-08-22.** §2.1 of this document closed it as *both*; [`adr/0132`](adr/0132-the-district-is-derived-and-a-ward-is-what-the-player-draws.md) then withdrew the player half. **A District is derived, full stop** — from road topology and land use — and the player's pen moved to a separate object, the **Ward**, which Policy is scoped to and which has no logistics consequence. The finding that resolves it is that **District extent is bounded by the pooling abstraction's own validity**, so the *count* is physics rather than a design choice. ⚠ **This entry outlived its own answer by sitting in a list of forks rather than beside the passage that settled it**, and *leaning player-drawn* is the wrong half to leave standing — it names the arm that is **not** the default. `plans/0002` item 6 recorded the close, and `plans/0002` had already filed the same staleness against `06:42` and `plans/0010`'s gate section; nobody checked this document's own open-questions list. ***A settled question struck in one section and listed in another is a document disagreeing with itself, and the reader believes whichever section they opened.*** Filed to [`plans/0012`](../plans/0012-corpus-audit.md).
