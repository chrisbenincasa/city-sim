...
**Measured** ([`plans/0011`](../plans/0011-rule-engine-bins-and-rules.md) task 9): one evaluation is **82.84 ns**, flat to 1.8% across two orders of magnitude, so the 15.6 ms Tick budget holds about **188,000 evaluations**. A chain rung's *marginal* cost is **53.6 ns** — two-thirds of the head that failed, because the per-Rule overhead is paid once and amortised down the ladder — which is the first evidence behind this section's own claim that depth is not the cost driver. **What the same number says about the city is less comfortable**: at the unratified 450 Rule Instances per 1,000 Citizens and §4.3's own rate of 8, a 1M-Citizen city spends roughly **two-thirds of the 15.6 ms Tick budget on the Rule engine alone** — a whole-Tick figure, since measured, that includes Phase 3's intent sort. That sort is the one part of the engine that is not linear in the due count, and it costs 63% more per Rule at 100,000 due than at 1,000. The running total across every priced consumer is [`plans/0013`](../plans/0013-tick-budget.md), which also prices the same rows against 1×, 2× and 8×.
