...
| **Woodland** | stored per Cell, **not diffused** | the generator, once | **a Day** — period `TICKS_PER_DAY`, offset 80 | a count, not a field, and **Sealing is its ceiling** ([`adr/0158`](adr/0158-woodland-is-a-tile-count-per-cell-bounded-by-sealing-because-the-ground-has-one-budget-and-not-two.md)). ⚠ **It grows back toward what the SEED laid** — a second saved column — because growing toward the bare Cell would converge every map on uniform forest and erase the seed's character |
| **Sealing** | stored per Cell, **not diffused** | construction — **roads and Buildings alike** | **two occasions, and only one of them is a cadence**: it rises on build, **at the point of laying**, and falls on a **Day** — period `TICKS_PER_DAY`, offset 48 | a count, not a field. **One Tile per built Tile, and roads are ~86% of it** ([`adr/0150`](adr/0150-sealing-authors-no-width-and-a-road-seals-where-it-is-laid-not-where-its-endpoints-are.md)). ⚠ **Its decay rate is keyed BY TERRAIN TYPE** — a `sealing_decay_tau` on each `[[terrain]]`, not a `[layers]` key, and the `[layers]` one is refused — because *rock may never recover, floodplain may recover over hundreds of Days* |
| **Radiation** | stored Layer, diffused — **industrial pollution's mechanism, its own source and its own rate** | a point source that deposits | its own cadence | ⚠ **Same geometry, same kernel, same range, and NOT one field with it** ([`adr/0152`](adr/0152-radiation-is-industrial-pollutions-mechanism-with-its-own-source-and-its-own-rate-because-a-shared-decay-is-wrong-for-one-of-them.md)). §2.5 guard rule 1 sorts on geometry and range and is **silent on persistence**; `DecayPollution` holds one `tau` for the whole Layer, so a shared column is a shared half-life and ***a shared decay is wrong for one of them in exactly the way a shared kernel was***. **The decay is small and never zero** — a source has no ceiling by construction where Sealing does. 🔴 **Nothing emits it and no milestone owns it** |
...
than about when it happens** ([`adr/0150`](adr/0150-sealing-authors-no-width-and-a-road-seals-where-it-is-laid-not-where-its-endpoints-are.md)).
