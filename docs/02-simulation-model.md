...
| Tier | When | What |
|---|---|---|
| **Per Tick** | every build | Only `O(1)` and `O(changed)` checks — no Bin negative or over capacity at the write site (`BinLevelIsWithinCapacity`), parking occupancy conserved (`ParkingOccupancyIsConserved`), no Trip without a Fate (`TripHasAFate`), and a parking space given up once (`ParkingSpaceIsReleasedOnce`) |
