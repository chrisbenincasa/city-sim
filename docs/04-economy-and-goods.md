...
**Goods live in integer Bins** — an integer in `[0, capacity]`, no floats, no continuous flows. (**Money is a Resource too, and its Bin is unbounded**: a ceiling on money is a warehouse limit on a balance, and the loader refuses an authored one. `adr/0024`.) Necessary for supply chains to be conserved and auditable. If a hundred units of Food entered the District, a hundred units must be accounted for. This is GlassBox's model and it is genuinely excellent.
