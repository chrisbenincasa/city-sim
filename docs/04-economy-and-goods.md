...
**And where a shop *lands* is not decided by anything, which is the design rather than a gap.** A Zone Rule raising a trade's premises is **generate-and-test**: [`adr/0170`](adr/0170-a-shop-is-selected-rather-than-sited-so-the-birth-signal-is-coarse-and-death-does-the-correcting.md) settles that the demand signal decides **whether and roughly where** and bankruptcy decides **which survive**, so ***the accuracy of the model lives in the death half and not in the threshold***. The reach unit is the **District**, because `RuleEngine.MarketRow` resolves a purchase premises → Lot → Cell → District and stops — a Household buys only from sellers in its own District, so no smaller unit describes anything the economy does. ⚠ **A walking radius is buildable today and is still refused**, which under [`adr/0070`](adr/0070-an-unbuilt-mechanism-is-not-a-design-constraint.md) makes it evidence: a radius counts the Households a shop can *walk to* while the mechanism serves the Households in its District. 🔴 **Selection needs over-supply, and the entry cost that stops over-supply is the same key** — so the two halves cannot be demonstrated in one world, which is why `rulesets/provisioned.toml` and `rulesets/oversupplied.toml` differ by exactly the two keys and show birth and death respectively ([`plans/0050`](../plans/0050-the-ruleset-sweep.md)).
