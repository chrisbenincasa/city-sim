...
| **The street** | 🟡 **HALF DRAWN, [`plans/0049`](../plans/0049-visuals.md) row 8 steps 1–2 of 4.** The three `RoadKind`s draw apart, and a Segment carrying feet *and* cars draws a **footway** either side of its carriageway — off the mode mask rather than the kind, so an Arterial gets none and **Severance is on the ground for the first time**. ⚠ **What is still one flat slab is the JUNCTION**, steps 3–4: no kerb band, no dropped kerb where a Lot's door meets it, and the footways stop short of each junction leaving it bare. ⚠ **A crossing is still not drawn**, and the reason has changed — see row 8's **F58** |
