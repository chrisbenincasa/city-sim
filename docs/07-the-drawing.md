...
**Surface textures come from the library in `art/materials/library/`.** `textures.toml` lists each
CC0 texture with its tile size in metres, where that size comes from, whether a family paints it,
and how strongly its colour map is flattened. `uv run scripts/art/materials.py` fetches the maps,
writes them beside the list and records sources, transforms and hashes in `materials.json`.
