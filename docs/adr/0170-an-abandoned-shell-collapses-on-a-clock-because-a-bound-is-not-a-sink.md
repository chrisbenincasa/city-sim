...
`0091` names two clearing routes and **both are player acts** — the `Demolish` verb and the `Govern` clearance programme. *The player will clear them* is a hope about a human rather than a property of the simulation, which is the objection `plans/0045` decision **1** opened with.
