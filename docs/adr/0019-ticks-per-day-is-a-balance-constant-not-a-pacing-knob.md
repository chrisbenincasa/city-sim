...
- **`WHEEL_SIZE` is set by the longest common event horizon, not by the Day.** These are independent questions that happen to share an answer: the longest routinely-scheduled event is a work shift or a sleep, both bounded by one Day, so ~~`WHEEL_SIZE = 8192`~~ **`WHEEL_SIZE = 2048`**. That equality is a coincidence worth stating, because the tempting shortcut — sizing the wheel to a fraction of a Day to save memory — silently breaks the wheel's defining property. An entity sleeping longer than `WHEEL_SIZE` wraps and is touched several times instead of once, which is the entire benefit gone in exchange for ~~64 KB~~ **16 KiB** of bucket heads. Multi-Day Life Stage countdowns exceed any wheel and need the overflow tier regardless. ⚠ **CORRECTED 2026-08-21 by milestone 18's scoping ([`plans/0036`](../../plans/0036-the-coarse-day-wheel.md)):** [`0094`](0094-a-day-is-2048-ticks-because-ticks-per-day-is-a-sampling-rate-and-not-a-length-of-life.md) moved both constants to **2048**, and the bucket-head figure moves with the count. ***The rule in bold above is untouched and is the load-bearing half*** — it is what [`0056`](0056-the-event-wheel-is-two-levels-ticks-and-days.md) was corrected against, having sized its coarse wheel by **symmetry** instead. ⚠ **This bullet was the authority for that correction while carrying the same stale constant**, which is the defect arriving in the document that diagnoses it.
