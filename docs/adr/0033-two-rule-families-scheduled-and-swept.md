...
- **A third category of quantity: the Readout.** Bins are what a Rule *spends*; Readouts are named read-only scalars it *consults* — income, experience, occupancy, composed fertility — never consumed, never conserved, never subscribable. The readable set is bound to the `Evidence` surface, so **a Rule may read anything the player can see, and nothing else.** That is a `LEGIBLE CAUSE` guarantee rather than a convenience: a Policy predicate reading a hidden internal would be one no player could explain being subject to. It also gives the Ruleset a stable named interface that fails loudly on an unknown Readout, where field paths would fail silently on a rename.
