...
> **Amended by session C. The argument is sound and the arithmetic underneath it was not.** The Event Wheel's period is `WHEEL_SIZE` = ~~8192~~ **2048** Ticks and a Day is `TICKS_PER_DAY` = ~~8192~~ **2048** Ticks, so **the wheel's period is exactly one Day** — and a countdown of *two* Days is already unrepresentable on it. Not a tail case: **every Life Stage transition this ADR specifies** was unrepresentable on the wheel it was specified to run on, and `EventWheel.Arm` throws rather than wrapping.
>
...
>
> ⚠ **Corrected 2026-08-21 by milestone 18's scoping ([`plans/0036`](../../plans/0036-the-coarse-day-wheel.md)). The arithmetic above was restated at the old Day length; the defect it records is untouched and so is every conclusion drawn from it.** [`0094`](0094-a-day-is-2048-ticks-because-ticks-per-day-is-a-sampling-rate-and-not-a-length-of-life.md) moved both constants to **2048**. ***Both moved together, so the period is still exactly one Day and a two-Day countdown is still unrepresentable*** — this block is a **ratio** claim, and a ratio claim survives a move of the constant it is written in. The sweep that followed `0094` corrected `02 §7` and `05` and reached neither this document nor [`0056`](0056-the-event-wheel-is-two-levels-ticks-and-days.md), whose **derived** claims did not survive it and are withdrawn there.
>
> 🔴 ⚠ **What HAS changed is the character of the defect, and it is not what a reader of `plans/0002` §F2 would expect.** That entry marks this ADR ⚫ because *"its arithmetic was never checked"*, and session C's block above is that check — **written into the ADR, with the repair named.** ***So the document is no longer the thing that is wrong.*** What is still wrong is that **the coarse wheel is not built**, which under [`0070`](0070-an-unbuilt-mechanism-is-not-a-design-constraint.md) is ***unbuilt* rather than *refused*, and the answer to an unbuilt mechanism is build X.** ⚠ **This ADR is therefore correct and unbuildable**, which is a different obligation from a wrong one, and `06`'s milestone **20** is gated on the second and not the first.
