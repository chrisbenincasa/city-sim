...
⚠ **`sealing_decay_tau = 0` means never, and `rulesets/minimal.toml` calls it a *stated absence* rather than a defect** — so a zero rate is not automatically `adr/0006` in this project, and the reason it is admissible there does not reach here. ⚠ **Corrected 2026-08-24 by milestone 24 task 4: the key MOVED and `[layers]` refuses it.** The precedent survives the move intact — it now reads `[[terrain]] sealing_decay_tau = 0` on `rock` in `rulesets/varied.toml`, and *rock may never recover* is a stronger stated absence than the global zero this paragraph was pointing at, because it is one type saying never rather than a whole world saying not yet. **Sealing has a ceiling by construction**: `MapLayers.Seal` clamps to `CellGrid.TilesInCell`, so a Cell cannot seal past being wholly sealed and the quantity is bounded however long the city runs. **A pollution source has no such ceiling.** The only thing standing between a never-decaying reactor and an unbounded stock is `Invariant.LayerMagnitudeIsBounded`, which is a **defect detector and not a bound** — it reports that the city went mad, it does not stop it.
