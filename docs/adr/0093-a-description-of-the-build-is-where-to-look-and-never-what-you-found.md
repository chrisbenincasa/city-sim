...
**The corpus is structurally unable to catch it.** Its ~~three~~ ~~five~~ **six** mechanical checks — `CitationTests`, `CoverageMapTests`, `MarkdownStyleTests`, `DisqualifierTests`, `LinkResolutionTests` and `LedgerCitationTests` — check that every ADR is **cited somewhere**, that rows exist, that markdown renders, that a disqualified figure is not quoted bare, that a relative link resolves, and that an ADR naming a ledger entry by number is named back by that entry. **All six are document-to-document or document-to-directory.** Nothing anywhere checks that a sentence about `RoadGenerator` is true of `RoadGenerator`, and no such check is proposed here, because a general one would be a natural-language proof obligation. ⚠ **AMENDED 2026-08-18: the first document-to-*code* check now exists, and it is narrow rather than general.** `RefusalCountTests` counts the `Refuse(` call sites in `RulesetLoader.cs` and holds [`0048`](0048-the-ruleset-is-validated-where-it-is-parsed-and-only-integers-and-strings-cross-into-the-core.md)'s count of record to the total — built because that number had drifted from 22 to 58 with every test green, and **17 of the 36 it missed sat in sections that already existed on the day it was last corrected**. It checks no sentence; it checks a *number a sentence contains*, which is the one thing about a description that a machine can read. ***The checkable part of a claim about the build is the part that is counted***, and this ADR's own prescription — *name a symbol, never a time* — is what makes such a number findable at all. [`plans/0012`](../../plans/0012-corpus-audit.md)'s proposed **check 10** (a `Rows.Saved` column whose only writer is a test) names the same direction and is still unbuilt; this arrived first, from a different task, and does not discharge it. What is available instead is a discipline with a checkable output, which is the next section.
