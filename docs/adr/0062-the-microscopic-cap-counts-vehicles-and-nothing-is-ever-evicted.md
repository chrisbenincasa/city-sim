...
> ⚠ **AMENDED 2026-08-16 by session E — the order has a third member and this enumeration named two** ([`0108`](0108-repeated-divergence-is-a-bug-report-and-the-queue-decides-how-long-a-promotion-lasts.md)). `03 §3.5`'s **rotating audit** promotes a Segment when it finds divergence, so it is a promoter, and it is neither of the two kinds ranked here. **It ranks last**, on this ADR's own principle: the order is by what a claim *buys*, and force-promotion buys **correctness**, stress-promotion buys **accuracy**, and an audit promotion buys neither for the running city — it buys **knowledge about the model**. Ties break as above.
>
> ⚠ **The asymmetry is what makes last rank cheap: an audit refused is *deferred*, not lost.** Rotation is a function of Tick, so the Segment comes round again, and `§3.5` already accepts that latency in its own words. A refused stress-promotion is an inaccuracy that happened; a refused force-promotion is a phenomenon that did not occur.
>
> ⚠ **And it almost never has to be refused, *because* this ADR counts Vehicles.** An audited Segment is **unstressed by construction** and therefore nearly empty. Under the Segment denomination retired here, a sample of *N* Segments would have consumed *N* slots and competed head-on with *N* jammed arterials. ***Choosing the right unit made the audit affordable in a section this decision never mentions***, which its consequence list could not have predicted.
>
> ***An enumeration is written against the members that exist when it is written, and nothing re-counts it*** — the same failure [`0107`](0107-a-demotion-discards-the-cursor-and-nothing-it-discards-has-to-be-invented.md) found in the demotion list, one session question earlier.
...
The argument that does is structural. **A Segment that is already Microscopic holds state that cannot be reconstructed** — ~~queue position, headway, an in-progress Switch Lane traversal, which `adr/0016` names as exactly what is at risk on demotion~~ **position along the Lane, velocity, Lane assignment, and an in-progress Switch Lane traversal** ([`0107`](0107-a-demotion-discards-the-cursor-and-nothing-it-discards-has-to-be-invented.md), 2026-08-16; the retired three-field form was `adr/0016`'s and this ADR quoted it). **The structural argument is untouched and is slightly stronger**, since the corrected list is longer by one real field. ⚠ **But *"cannot be reconstructed"* is now wrong in the other direction**: `adr/0107` shows each of the four **is** recovered on promotion from state the world still holds, so what eviction destroys is not unreconstructible *state* — it is the **queue**, and with it hysteresis at the moment it would have been observed, which is the sentence two lines below and is the leg this decision actually stands on. **A Segment newly crossing the threshold holds none**, because it had no queue a Tick ago. Refusing the second destroys nothing; evicting the first destroys something no rule can rebuild, and destroys it at the moment it would have been observed — `03 §4` invariant 3 already says a queued Segment does not demote *because hysteresis is deleted at exactly that instant*, and eviction is that deletion with a resource limit as its motive.
