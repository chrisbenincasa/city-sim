...
here and to `0155` rather than to a Ruleset review. The key exists so the question can be looked at, not
...
(`0155`). What exists is a Ruleset number looked up by a stored terrain type.
...
argued against the alternative***, and `0158` takes the other one as well.

⚠ **The half `0158` does not get for free, recorded here so it is not read as settled:** five ranked
