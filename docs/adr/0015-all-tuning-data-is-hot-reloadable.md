...
- **The validator this ADR implies has been specified**, in [`0048`](0048-the-ruleset-is-validated-where-it-is-parsed-and-only-integers-and-strings-cross-into-the-core.md): it lives with the parser, because *"error-message quality is part of the deliverable"* means its whole output is human-readable strings and `adr/0002` forbids the core from producing one. It runs **three** refusals in one walk at load — `0045`'s `on_fail` cycle check and `fills` check, plus an unquoted decimal, which is refused rather than coerced because a library's `double` must never reach a tuning number.
