...
>
> ✅ **SETTLED 2026-08-26 by [`0171`](0171-a-markets-level-is-what-its-sellers-hold-and-the-price-divides-by-the-sum-while-a-wake-spends-the-maximum.md), which is where the reasoning lives.** Cover is **the sum over the row's sellers**, and the production-rate candidate is not merely rejected but *unreachable* — this record's tâtonnement divides a stock by a rate to get Days, so a rate in the denominator regenerates the very bug. The price now moves: `rulesets/oversupplied.toml` goes **100 → 58**.
>
> 🔴 ⚠ **AND THE PREDICTION ABOVE UNDERSTATED IT A SECOND TIME.** *Only its caller changes* named **one** call site; there were **three**, and the two nobody had looked at had no symptom. `World.RingMarket` spent the *depositing* seller's level as a wake budget, which under-wakes whenever the depositor is not the largest seller; `RuleEngine.Stop`'s rescue drain — the one that exists because Phase 3 settles intents in shuffle order — ran against a market row's structural **zero**, so it could never fire on a market at all. ***A quantity this record left undefined was not wrong in one place; it was guessed in every place***, and the sighting that had a symptom was the least of the three.
