...
- **A lint runs in CI** over the simulation core from the first commit, failing the build on any banned construct. Added later, it becomes an audit; added first, it is a guardrail that never lets the debt accumulate. **Shipped as `Borough.Analysers` in slice 3** ([`plans/0006`](../../plans/0006-analysers-and-lints.md)), covering every row of the table above; the claim that adding it first is a guardrail rather than an audit was tested immediately, in that its one violation was a defect rather than a stylistic debt. **`Borough.Core.Arithmetic` is exempt from the raw-`/` and shift diagnostics**, because it is where their stated-rounding replacements are implemented — a rule cannot ban the construct its own replacement is made of.
