...
MultiMesh. World-space value noise at 24, 12 and 6 m scales brightness by up to `macro_strength`,
±25%, across a wall.
...
| Macro variation A/B, both cities above, Tick 600 | Soft light at 0.35 (`macro-ab-street.png`, `macro-ab-apartments.png`, off above and on below) showed soft patches on the clay roofs and barely changed pastel brick or dark Bitumen, because soft light moves little near black or white. The shader now multiplies brightness instead; ±15% was still faint, ±30% showed patches on the clay roofs and some on painted brick, and the default is ±25%. Bitumen barely changes at any strength because its photograph is nearly black. Two runs of one frame differ in 10 to 13% of pixels with the shader unchanged, so pixel counts cannot measure the variation; compare by eye. `macro-ab-*.drive` |
