# 0045a — Amnesty history through 2026-09-09

**Historical snapshot, superseded for status and priorities by [0045](0045-amnesty.md).**
The original page follows intact, including its row identities, findings and corrections.
Present-tense claims and standing orders below describe that snapshot; use the active page for
current instructions. Open work remains open unless its owner records its completion.
This file is reference material, not a cold-start reading list. New findings belong in the plan
for the item that produced them; do not append another session here.
...
| **What does a Business's founder lose when it goes bankrupt?** [`adr/0146`](../docs/adr/0146-founding-costs-a-citizen-and-the-households-money-so-the-founder-is-the-first-worker.md) spends **a Citizen and the founding Household's money** to found a trade, so every founded shop has a person with a stake in it. [`0065`](0065-business-insolvency.md) settled what bankruptcy does to the STAFF — they are dismissed — and deliberately left the founder alone: ***the capital is already spent***, and whether failure claws anything further back is a second loss nobody has argued for. ⚠ **The narrow half is built and nothing is blocked**; what is owed is whether a founder is a creditor, a casualty or neither. Goes to §C under `adr/0146` | *arguable* — no measurement says what a failure should cost the person who paid for it; the census can say how many founders there are and not what they are owed | **A world where founding is common enough to watch.** `founded.toml` exists and nobody has ever watched a run of it, so the question would currently be answered against a city with no founders in view | 2026-09-06, scoping [`0065`](0065-business-insolvency.md) |
...
✅ **ROWS 28–30 CAME OFF [`06`](../docs/06-roadmap.md) ON 2026-09-06, WHICH IS A FOURTH WAY OF
ASSEMBLING THIS QUEUE AND THE FIRST ONE THAT WAS NOT INVENTED HERE.** The three questions tried so far
were *what has a failing test*, *what has a symbol*, and *what is missing at the size of a milestone*;
this one is ***what did the roadmap already decide was worth building, that nobody has started.***
⚠ **The milestone numbers are kept and not renamed** — [`adr/0140`](../docs/adr/0140-a-milestone-number-is-an-identity-and-the-roadmaps-order-is-the-sequence.md)
makes a milestone number an **identity**, so row 28 *is* milestone 16 and says so. **All three are
ungated** ([`0003`](0003-build-plan.md): 13–17 and 19–23 carry no gate and no plan document is owed
until a row is next). ⚠ **Each was CHECKED IN CODE before it was written here** rather than quoted out
of `06`'s risk column, and ***two of the three turned out to be sharper than the roadmap knew***: the
choice model's arithmetic is built and uncalled, and `skill_tier` is saved into every State Hash with
no end connected.
...
| 27 | 🔴 **A BUSINESS COULD TAKE IN LESS THAN IT PAID OUT FOR EVER AND NOTHING NOTICED.** The money loop closed over the fortnight to 2026-09-06 and nobody joined the ends: shopping credits a seller's till (`ShoppingEngine:372`, row 20), `WageEngine` pays staff out of it, `PayrollReading.Underpaying` records the failure — and has **zero readers**, while `owed` is recomputed each payday and discarded. ***So an insolvency was an INSTANT and never a CONDITION***, which is `CLAUDE.md`'s standing *balance → unbalance → balance* constraint arriving at the tenant: a Business is a tenant and had no middle. ⚠ **NOT FOUND BY A SYMBOL OR A FAILING TEST**, which is what makes it the first row past this page's own two traps — it was found by asking whether a **loop closes**, and the two halves had shipped nine days apart with nothing between them. ⚠ **SCOPED BEFORE IT WAS BUILT, AND THE SCOPING CHANGED THE MECHANISM** ([`0065`](0065-business-insolvency.md)): the first draft routed insolvency into `World.Unpremise` and did not survive one question from the player — ***if a shop failed, how can it still do business?*** It cannot; `UnfitBusiness` sends a trade's Rules and Bins away with its premises. **Losing your front and going bankrupt are two failures that share an END and not a route.** 🔴 **The correction VOIDED one of the scoping pass's own three findings** — *insolvency is the third door into the Unpremised Pool* — because road B never touches the Pool; ***a sound observation about a mechanism that turned out to be the wrong one***, struck rather than deleted. 🔴 **AND BUILDING IT FOUND WHAT SCOPING COULD NOT**: `DestroyBusiness` left a premised trade's Rule Instances behind and died on `StaleHandleException` on the first bankruptcy — **both prior callers hid it**, since `Depart` only ever gets already-pooled rows and `DestroyBuilding` is demolishing anyway, so ***a bankruptcy is the first case in the build where a premised trade dies and its Building survives.*** ⚠ **`wage_per_day` swept FOURFOLD — 512/1024/2048/4096 — and deaths moved 44/44/46/46**: the shortfall scales and the deaths do not, ***so the failures are STRUCTURAL and the city decides which shops die by where they are.*** **There is no thin margin in this world to tune**, and `insolvent.toml`'s header says so rather than letting the numbers imply one. ⚠ **A wage that can never be paid ships in `shopping.toml`** — a wage on a trade tenanting the `dwelling`, which holds no money Bin — **not refusable at parse** and reported by `--business` instead, on the payroll panel wages had never had in any mode | ✅ 26-09-06, [PR #6](https://github.com/chrisbenincasa/city-sim/pull/6) |
| 28 | 🔴 **A HOUSEHOLD TAKES THE BEST OF THREE AND THE CORPUS HAS SPENT ELEVEN MONTHS DESCRIBING A CHOICE MODEL.** [`06`](../docs/06-roadmap.md) milestone **16**, ungated, and its own risk column calls it ***the most leaned-on absence in the corpus*** — **four decisions rest on it and it gates 19 outright.** [`02 §5.4`](../docs/02-simulation-model.md) specifies a **multinomial logit**: score the survivors, then `P(i) = exp(μ·V_i) / Σ exp(μ·V_j)`, *"about ten lines of code, closed form, no iteration."* 🔴 **THE ARITHMETIC WAS BUILT FOR IT AND NOTHING HAS EVER CALLED IT** — `Arithmetic/Transcendental.cs` opens by saying so in its own first sentence, *"02 section 5.4's choice model is a softmax over `exp`, so without this the choice model could not be built"*, and `Transcendental.Exp` has **zero callers in `src/`**, only its own tests. ***A dependency built, tested, documented and never used is the sharpest possible statement that the thing it was for does not exist.*** ⚠ **What runs instead is `PlacementEngine`'s best-of-`candidates`** — a deterministic argmax over three draws, `adr/0017`'s satisficing, and **correct as far as it goes**: this row does not delete it, it puts a distribution where the `argmax` is. **Three things arrive with it, and the third is the one nobody would predict.** **`μ`** is a free design knob and `02 §5.4` is unusually explicit that the literature will mislead you here — *"when the city feels too herdy or too random, tune `μ`, not the coefficients"* — so it is a **feel** parameter and its ratifier is a person at the controls. **Only utility DIFFERENCES matter**, so *everything available is terrible and nobody moves* is inexpressible without an explicit **stay-put alternative**. 🔴 **AND THAT ALTERNATIVE IS THE HINTERLAND, WHICH IS WHY THIS ROW IS BIGGER THAN A SCORING FUNCTION**: a prospective Household compares *staying outside* against *moving here* through the identical utility function, so ***immigration stops being a system beside the choice model and becomes a consequence of it*** ([`adr/0023`](../docs/adr/0023-immigration-arrives-through-the-gate.md)), and the Hinterland's authored anchors are already in domain units for exactly this. ⚠ **Hash-bearing, and swapping the internals later is hash-breaking but DISTRIBUTIONALLY NEUTRAL** — `02 §5.4` names Gumbel-max as the same distribution with no `exp` on the hot path — so the interface is *scored candidates in, one choice out* and the algorithm is **recorded rather than frozen**. ⚠ **`adr/0003`'s float ban is why the tabulated `exp` exists at all**, and the table's resolution perturbs the effective `μ`, so the resolution is a stated figure and not an implementation detail | ✅ **BUILT 2026-09-08 — the softmax where the argmax was, the incumbent as a row in it, and the Outside as another. No recorded hash moved.** [`0068`](0068-the-choice-model.md). 🔴 **`arrivals_per_day` has stopped deciding how many people want to live here**: same doors, same asking, admissions fell from a flat 96 a Day at every gate to between 1 and 11 and the Pool from 839 waiting to 31, ordered by what the Outside costs and stated nowhere. ⚠ **Two open**: stickiness has a ceiling nobody authored at `adr/0038`'s horizon, and **who knocks is still outside the simulation** — `adr/0023`'s stock is unbuilt |
| 29 | 🔴 **EVERY CITIZEN CAN DO EVERY JOB, AND THE COLUMN THAT WOULD SAY OTHERWISE IS SAVED, HASHED AND DEAD.** [`06`](../docs/06-roadmap.md) milestone **15**, second half, ungated. `CitizenTable.SkillTier` is `Rows.Saved<byte>("skill_tier")` and **the whole build holds exactly two references to it — the declaration and the property.** ***No writer, no reader, and it folds into the State Hash of every world.*** ⚠ **This is row 2's shape twice over** — that row found `Citizens.Activity` saved with no writer; this one has neither end. [`adr/0104`](../docs/adr/0104-a-skill-tier-is-earned-by-attendance-and-the-credential-stays-a-wall.md) is titled ***a skill tier is earned by attendance and the credential stays a wall***, and **neither clause is built**: attendance runs (`ServiceEngine.Attend` makes real Trips, row 10) and **produces nothing**, and no job anywhere is gated on a tier, so **the wall has no wall.** 🔴 **IT LANDS ON WORK THAT JUST SHIPPED AND MAKES IT MEAN SOMETHING.** Row 6 gave a job a wage and row 27 made a Business fail to pay one — but `wage_per_day` is **per trade**, so every worker at a trade earns identically and a Citizen's history buys them nothing. ***A labour market with one kind of labour is a payroll, not a market***, and it is why row 27's own finding — that failure is structural rather than marginal — has no second lever to test it with. ⚠ **The attendance half is a DURATION and not a tally** ([`adr/0053`](../docs/adr/0053-failure-pressure-is-a-duration-not-a-tally.md)'s rule arriving somewhere new), and ⚠ **the credential half is a FILTER and not a penalty** — `02 §5.4`'s *hard constraints are filters, soft trade-offs are utility*, which is the same sentence row 28 needs and is worth building once. ⚠ **Ordering against 28 is genuinely open**: a tier is a term in a utility function, so 28-then-29 lets the term exist before it is scored — but 29 alone is testable and 28 alone is complete, so ***they compose rather than sequence*** | |
| 30 | ⚠ **THE CITY CANNOT SAY WHY A BUILDING WAS ABANDONED, IN THE ONE SENTENCE THE DESIGN WROTE TO SHOW WHAT SAYING WHY MEANS.** [`06`](../docs/06-roadmap.md) milestone **17**'s routed residue, ungated, and the smallest of these three. `CONTEXT.md` names **three** sources of Failure Pressure — Trips failing, Rules reaching a reporting terminal, conditions below tolerance — and ***only the second is built***: `RuleEngine.Stop` starts `RuleInstances.StarvedSince` **if and only if** `verdict.Blocking == Blocking.Supply`, which is one branch of one enum at one call site. 🔴 **SO `CONTEXT.md:203`'S OWN SPECIMEN OF `LEGIBLE CAUSE` IS UNPRODUCIBLE** — *"abandoned: 74% of work trips exceeded commute budget over 30 days"* — **and it fails in ONE of its five clauses.** The destination Building and the verdict are live parameters **twelve lines apart** inside `TripEngine.Start`, so ***the ratio is two integers*** and the **window** is the whole of the gap. ⚠ **The window has two obvious derivations and BOTH ARE REFUSED**: a decaying average is the tally [`adr/0053`](../docs/adr/0053-failure-pressure-is-a-duration-not-a-tally.md) deleted, and reusing `CondemnAfter × rule.Rate` is the welding [`adr/0079`](../docs/adr/0079-a-building-outlives-its-frontage-and-an-address-that-has-none-is-a-hole-the-trip-model-reports.md) forbids by name — ***so the window is the design work and the counting is the easy half.*** ⚠ **It belongs here and not with Evidence**, because ***Evidence REPORTS pressure and does not produce it*** ([`0028`](0028-evidence-the-accumulators.md), routed 2026-08-16). ⚠ **Impact is on the PITCH rather than on the simulation**: nothing in the city behaves differently the day a second source lands, and *when something goes wrong the game can say exactly why* is the first line of what this project is for | |
...
Row 20 includes the player's weekly-calendar decisions. `ShoppingEngine` considers low stocks,
chooses one Good by remaining consumption cover, sends an available adult to a known shop, pays
on arrival, and delivers Shopping cargo at home. Partial purchases end the search. Severe hunger
may delay work and permits one additional known shop after buying nothing. `WorkSchedule` gives
Workplaces weekly workdays and accrues wages only during attendance; `WeeklyHours` gives shops
stable opening hours without a staffing requirement. All new demonstration tuning is PROVISIONAL.

`rulesets/shopping.toml` and `--shopping` exercise the mechanism. The initial implementation produced
no shopping: replacing the instantaneous purchase removed the market-wait signal that caused shops
to open. `ZoneRuleEngine.RecomputeDemand` now also reads elapsed unserved shopping needs.
The other integration trap was the clock: the simulation Day starts at 05:00; weekdays change at
civil midnight through `WeeklyHours.DayOf`.

The driven shell at Tick 600, seed 0, 400 Citizens showed three shopping Households with 64 Goods
being carried, including one return carrying 32 sundries. The view is reproducible with `--start-at
600` and the same Ruleset and population. `ShoppingTests` covers the purchase boundaries, civil
weekend, attendance, rehousing and save/reload with cargo. A 102,400-Tick headless run at that
population completed with end-of-run invariants intact. It is not a balance or performance reading.

Cargo survives rehousing and retries an unreachable return at the configured interval. If its
carrier or return origin disappears, or the Household no longer has a Bin for that Good, the loss
is explicit in `ShoppingReading.Lost`. This is disposal, not a general salvage or inventory system.
The new saved columns and tables move the golden State Hashes; the baseline Ruleset files do not change.
