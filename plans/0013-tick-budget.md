...
| ⚠️ **A walk's recorded path** (`TripEngine.Plan`, `recordPath` for both modes) | 4 Move | **UNMEASURED, and below this machine's noise floor rather than small.** A pedestrian Leg now keeps its route's hops so the shell can place a walker on a Street, where before the search ran and the answer was thrown away — ***the search was always paid and only the bookkeeping was conditional***, so what this adds is the `_via` writes and not a second search. 🔴 **4,000 Citizens over 4,096 Ticks reads 3.27–5.77 s before against 3.38–4.25 s after, two runs each, which separates nothing** | Hops on a walk × walking Trip starts a Tick — **neither measured**, and the first is the walk-search row's own curve in distance seen from the other side | — |
| Household shortage reassessment (`PlacementEngine.ShortagePromptsMove`) | 6 | Unmeasured; rides the existing reassessment sample. Only deficient Households walk their Building’s Rule Instances and input terms. | Sampled Households × Rule Instances at their Building × input terms; unmeasured | — |
| 🔴 ⚠️ **The daily school pass** (`ServiceEngine.Attend`) | 4 Move | **UNMEASURED, and this row exists to say so.** A whole-population walk once a Day, with `Gather` rebuilding the list of standing service Buildings — `O(Buildings)` — inside it. ⚠ **The cost is not the pass, it is the SPIKE**: `Attend` returns unless `tick.Raw % Ticks.PerDay == 0`, so ***every school Trip in the city starts on one Tick in 2,048***. `CommuteEngine` partitions the equivalent across a Shift start hour ([`adr/0101`](../docs/adr/0101-a-commute-is-two-journeys-and-the-days-shape-is-a-property-of-the-job.md)) and a school day has no hours key to partition on, which the engine's own remark says. 🔴 **A mean over the Day is the wrong denominator here** — the routing row above prices a *maximum* over 256 Ticks for exactly this reason, and whatever this pass costs is paid inside one 15.6 ms budget. 🔴 ⚠ **IT GOT MORE EXPENSIVE ON 2026-09-03 AND BY HOW MUCH IS UNMEASURED.** Nearest-first admission ([`0054`](0054-the-kind.md) **F6**) ended the `Fast`-rung break, so where a family used to stop routing at the first good-enough candidate it now routes **every** candidate in its box — ***the multiplicand beside this row went from a best case of one to the worst case always.*** It also added a sort over the Day's occasions and a **second** walk for any family whose nearest school is full, which nobody pays in a world with no ceiling. ⚠ **The bound is the PLAYER'S school count and not the city's**, which is the argument for having no `candidates` key and is the whole of why this was affordable; it is an argument about shape and ***it prices nothing***. 🔴 ⚠ **AND ON THE SAME DAY IT BECAME A MATCHING, WHICH COST NO ROUTES.** [`0054`](0054-the-kind.md) **F6d** replaced the ordering with deferred acceptance, and **F6e** is the finding: the walk was already visiting every candidate in the box in order to rank them, so the whole preference list was being paid for and thrown away. ***What the matching added is MEMORY*** — one `long` per (family × reachable school), plus a `Span.Sort` per family and a heap operation per proposal — against a routing bill that did not move. ⚠ **The estimate written before it was built said *a route per displacement* and was wrong**, because it priced the algorithm against the engine it was replacing rather than the one standing. ⚠ **Still UNMEASURED, and the memory row is now the one worth taking**: it is the first structure in this pass whose size is the product of two populations | Households × service Buildings in the box, on one Tick in 2,048 — **neither measured**, and no shipped world can supply the second: `schooled.toml` is the only file declaring a `serves` kind and nothing in the world places its school | — |
...
| **Zone Rules**, worst aligned Tick | 6 Growth | **0.012 ms** ⚠ **the unit was measured before the empty-clock verdict existed** — `Condemn` gained an occupancy read and a column compare per sampled Lot on 2026-08-31, which is `O(1)` at the sample and therefore inside this row rather than beside it, but it is not inside this *figure* | 16 Rules triggering together — **guessed**; unit **measured** | **0.08%** |
...
| ⚠️ **The market's level — a seller walk on every market drain** | 6 Growth | **UNMEASURED, and NEW 2026-08-26 with [`adr/0171`](../docs/adr/0171-a-markets-level-is-what-its-sellers-hold-and-the-price-divides-by-the-sum-while-a-wake-spends-the-maximum.md).** A District Pool holds nothing, so *how much is there* is answered by walking `DistrictMarkets`' seller entries for the row — `Offered.Held` for the Day-boundary reprice, `Offered.Largest` for every wake. ⚠ **The wake path is the one that could bite**: it is on `World.Drain`, which every Bin write in the city reaches, and it is guarded only by an owner-kind read that keeps a non-market Bin at two array loads. ⚠ **It is the SAME WALK [`adr/0139`](../docs/adr/0139-a-district-pool-is-a-market-and-not-a-store-so-stock-stays-with-the-seller.md) already owes a measured row for, arriving a second time on a second path** — §the purchase's row above contains it inside a bundle a stopwatch cannot separate, and this one is not in that bundle at all. ***One structure, two unpriced consumers, and no profiler.*** 🔴 **Do not price it from the purchase's figure**: a purchase walks the row until it finds a seller with the batch and stops; this walks every seller, every time | Sellers in one `(District, Good)` row × market drains a Tick — **neither measured**, and the second is not bounded by anything the corpus states | — |
| ⚠️ **Shopping and weekly attendance** (`ShoppingEngine.Step`, `WorkSchedule.Accrue`, `CommuteEngine.Generate`) | 4 Move | **UNMEASURED.** Shopping considers staggered Households, walks saved outing rows, and sorts active outings before purchases. Attendance and overdue return checks walk Citizen slots each Tick in a weekly-schedule world. See `plans/0045` → *What shopping found* | Household and Citizen slots, active outings, remembered providers and route searches; no million-Citizen timing claimed | — |
| ⚠️ **The Need refresh** (`RuleEngine.SweepNeeds` → `RefreshNeed`) | 6 Growth | **UNMEASURED, and this row exists to say so.** One pass over every live Rule Instance once a Day, refreshing the starving ones. ⚠ **It RECOMPUTES a depth from an elapsed duration rather than accumulating per visit**, which is what makes a staggered pass sound and also what keeps the per-row unit at a few integer operations rather than a query — so ***the multiplicand is the whole risk in this row***. It returns on line one where a Ruleset states no `[needs]` | Live Rule Instances at 1M — **guessed**, and it scales with population rather than with Buildings, because a tenant carries its own Instances (`adr/0147`) | — |
| ⚠️ **The Disaster sweep** (`DisasterEngine.Sweep`) | 6 Growth | **UNMEASURED, and this row exists to say so.** ⚠ **Two costs, and only one of them is conditional**: the scheduling walk over `Disasters` rows is paid every Tick in any world stating `[disasters]`, and the flood's own work is `O(footprint)` a Tick while a flood is live — the engine's own remark says so. It returns before it looks at a row on every shipped Ruleset but `flooded.toml`. 🔴 **The footprint is the GROUND and not the city**, which makes this the one consumer in the ledger whose multiplicand does not fall when the population does | Cells the water reached × Ticks a flood lives — **neither measured**, and `flooded.toml`'s three durations are stopwatch settings that ratify nothing | — |
...
| 🔴 **The shell's per-FRAME world walk** (`Main.Draw`) | **UNMEASURED** | — | **Filed 2026-08-31.** Three whole-table walks a *frame*, not a Tick, and the rung does not slow them down: `Fill(_buildings, Buildings())` is `O(Buildings)`, `Fill(_flood, Inundated())` is `O(Cells)`, and `VisibleAgents.In` is asked for **`CellRect.World`** — the whole map — so ***nothing is culled and the box argument is doing no work at all***. ⚠ **The per-Traveller unit is not `O(1)` either**: `TryPlace` walks the Leg's route hops to interpolate a position, so the cost is `O(Travellers × hops)`. 🔴 **It is out of the Tick and it is not out of the frame budget**, which nothing in this corpus has ever priced — at `01 §1`'s pause rung the simulation costs nothing and this is the entire bill. `adr/0073` routes it here; the fix, if it needs one, is the camera's own box rather than `CellRect.World` |
