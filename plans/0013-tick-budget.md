...
| ⚠️ **School matching and scheduled departures** (`ServiceEngine.Attend`, `CivicEngine.ScheduleSchool`) | 4 Move | **UNMEASURED.** Daily matching still gathers and routes candidates. With `[school]`, that pass schedules departures around stable school bells; `CivicEngine.Step` releases journeys at their own departure Ticks. Rulesets without `[school]` retain the original attendance pass. | Children × reachable schools for matching; scheduled Citizen rows for departure and return checks; neither multiplicand measured at target population | — |
...
| ⚠️ **Care and school visits** (`CivicEngine.Step`) | 4 Move | **UNMEASURED.** Walks visit rows each Tick; care cadence sorts waiting requests and rebuilds appointment calendars. Provider discovery routes bounded candidates; bed searches walk facilities and count reservations. | Citizen rows, waiting requests, remembered clinics, appointment horizon and facilities; no target-population cost claimed | — |
