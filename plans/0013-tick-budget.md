...
| ⚠️ **Land value producer** (`MapLayers.SetLandValueTargets`) | 5 Layers | **UNMEASURED.** Four `Desirability` calls a resident Cell, on one Tick in 256 — each one Cell-Layer read plus one line-source query over a 300 m window ([`adr/0126`](../docs/adr/0126-a-cell-samples-desirability-at-its-quadrant-centres-and-a-line-sources-area-mean-does-not-converge.md)) | 🔴 **resident Cells — GUESSED, and guessed at ZERO.** The only thing in the build that creates a Cell row is a pollution emission and **no shipped Ruleset emits any**, so the loop runs over an empty table in every world that exists. ***A row whose multiplicand is zero because the world is empty is not a cheap row, it is an unpriced one*** | — |
