...
**Active. Start here; this page is the board while amnesty runs. Row 31 is in flight; row 32 is the next to start.**
...
At `7ea9ab5`, before this tracking edit, the test's counting rules measured **2,252,952 prose
words / 57,188 source lines = 39.39 words per line** (integer reading **39**). The ceiling is
now **39**, lowered from 41 under standing order 3; the exit target remains **30**. Row 33's
simulation carried the fall, because prose grew by 96,852 words and source grew by 5,081 lines over
the same span. `CorpusBudgetTests` owns both checks.
...
identities; 31, 32 and 33 are new. Rows 29 and 33 are done. Row 31 is under way on
`worktree-row-31-attracts-people`, two of its eight tasks committed. Row 32 is scoped and not
started. Row 30 needs a design pass before it can be scoped. The roadmap supplies
...
| **31** | **The city attracts people without injected arrivals.** Connect Hinterland population stock, who presents themselves, the existing choice model, gate throughput, admission and placement. Include depletion and replenishment, with population accounting and changing city conditions. | **In progress on `worktree-row-31-attracts-people`; tasks 1 and 2 of 8 committed.** [0073](0073-the-city-attracts-people.md) owns fourteen design decisions, the eight ordered tasks and the acceptance conditions. Continues [0068 D6](0068-the-choice-model.md#decisions). **Task 1 shipped the data contract** — `HinterlandPopulationRuleset`, the loader's validation with key and line on every refusal, the schema fragment and `rulesets/attracted.toml`. **Task 2 shipped the stock and the account** — `HinterlandTable` holds one row per map edge, `HinterlandPopulationTable` one row per exact composition standing behind an edge, and `PopulationLedgerTable` classifies every birth, admission, death, emigration and dissolution that frees or fills a row. Two whole-world invariants tie live Citizens and live Households to the account, so a door that forgets to record, or records twice, fails at the end of a run rather than drifting quietly. `SaveHeader.Current` went 1 → 2 and all three golden artefacts were re-recorded. **Task 3, the prospect and common admission, is uncommitted work in that worktree.** ⚠ **The Outside side of the account is open.** `World.Depart` counts its people leaving and credits no Hinterland with receiving them, which needs D9's destination choice and `HouseholdTable.ArrivalEdge`; task 5 wires it. Demonstrate arrivals changing as opportunity and the Outside change, without repeated player or Input Log requests supplying the flow. |
| **32** | **The city spends money and the player decides on what.** Connect the treasury, a service that costs money to run, the funding lever and the consequence of withdrawing it. Include an opening balance, a cost on placing a service, and money conserved across both. | **Scoped 2026-09-09; not started.** [0070](0070-the-city-spends.md) owns the scope, the four decisions and the nine findings. Watch a school be funded, staffed and paid, then lose its funding and close. |
