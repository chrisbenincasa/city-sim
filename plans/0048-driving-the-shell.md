...
| **F27** | ✅ **`focus <east> <north> [metres]` — the only verb that moves the EYE.** Added by `plans/0049` row 2, and the reason is that ***a landscape nobody can point a camera at cannot be reviewed***: the zoom clamp is derived from the city's span, so a script could see about 3 km of a 65.5 km map and no more. ⚠ **The distance is not clamped and a hand's zoom is**, which is the first place this grammar has deliberately let a script do what a player cannot |
| **F26** | 🔴 **A DRIVEN RUN DOES NOT BUILD, AND `dotnet build -c Release` BUILDS THE CONFIGURATION GODOT DOES NOT LOAD.** Godot runs `.godot/mono/temp/bin/**Debug**`; a Release build leaves it untouched, and the shell starts anyway on whatever was there last. **Caught on 2026-09-01 by `plans/0049` row 1**: a new MultiMesh layer was added, the shell was driven, and `draw` reported **nine layers and not ten** — the roofs were absent from a dump of a build that predated them. ⚠ **The picture was the more dangerous half**: it showed a plausible city and nothing said it was yesterday's, so ***a screenshot cannot report that it is stale and a draw list can.*** **`dotnet build src/Borough.Godot` — no `-c`** before any driven verification of a shell change |
...
| **F8** | 🔴 **THE CAPTION WAS COMPOSED BEFORE THE COMMANDS IT DESCRIBES.** `Draw` runs before `Drive` in a frame, so `pause` then `shoot` on one Tick wrote a caption reading `speed 1x` over a picture of a stopped city. ***A caption written from a stale compose disagrees with its own picture***, which is the one thing writing them in a single act was meant to prevent. `Shoot` and `Caption` now recompose first. 🔴 ⚠ **AMENDED 2026-08-31 — THIS FIXED THE CAPTION AND THE PICTURE WAS STILL A WHOLE FRAME BEHIND IT**, and the cell above still said *redraws first*. `Viewport.GetTexture` returns the frame that was **presented**, which is the previous one, so a `shoot` at Tick 140 photographed Tick 139. ⚠ **It has been wrong since `shoot` existed and was unreadable until `plans/0045` row 15g put a palette on screen** — the first thing whose whole content changes on one command, where a map two Ticks apart looks identical. Two `RenderingServer.ForceDraw` calls changed nothing; the capture is now **deferred to the top of the next `_Process`**, with `_quitAfterShot` so a `shoot` and a `quit` on one Tick still gets its picture. ***A finding that names a symptom is not a finding that named the cause*** |
...
| **F28** | 🔴 **TWO `shoot`s AT ONE TICK WRITE TWO CAPTIONS AND ONE PICTURE, AND NOTHING SAYS SO.** Found 2026-09-04 by [`0049`](0049-visuals.md) row 8, driving a change to the carriageway: a script holding `shoot a.png` and `shoot b.png` **both at Tick 1** produced `a.txt`, `b.txt` and **`b.png` alone**. No refusal, no warning, exit status zero. ⚠ **§5 of this plan promises the opposite in as many words** — the frame and the readout are *"written in one act so they cannot disagree about which Tick they are of"* — and they disagreed about whether the frame **exists**. 🔴 **It is the third arrival of one shape, and that is why it is worth a row rather than a fix.** **F17** wrote a structurally valid draw list of 21,504 zeros; the headless shutter writes a caption beside no picture and says so; **this** writes a caption beside no picture and says **nothing**. ***Each time, the handle is fine and the emptiness is in what the handle points at.*** ⚠ **The cause is the shutter being a frame behind its own caption** (`790267e`): the caption is written when the verb applies and the frame is grabbed after the draw, so a second `shoot` in the same Tick replaces the pending path before the first one fires. 🔴 **AND THE SAME CLASS OF DEFECT WAS ALREADY KNOWN AND ALREADY SOLVED ONE CASE OVER.** `_quitAfterShot` exists because *"a `shoot` and a `quit` on one Tick would lose the picture outright — so the quit waits for the shutter"*. ***The pending frame was made to survive a `quit` and was never made to survive a second `shoot`***, which is the one-frame window being closed against the arrival that was thought of and left open against the one that was not. ⚠ **NOT FIXED, and the local workaround is not the discharge** ([`adr/0073`](../docs/adr/0073-a-local-workaround-is-not-a-discharge-and-a-finding-about-shared-code-must-reach-it.md)) — putting the shots on different Ticks is what `0049` row 8's drive script does and it is a comment in that file, not a repair here. **The repair is a choice this row does not take**: refuse the second `shoot` at parse the way `DriveScript.Stopped` refuses a stopped clock, or queue the requests and grab a frame per pending path. ***The first is one line and honest; the second is what a caller would expect*** |
| **F29** | 🔴 ✅ **HALF THE SCREEN WAS NOT IN THE CAPTION, AND THE HALF LEFT OUT IS THE ONE THAT SAYS WHAT A CLICK WOULD DO.** Found 2026-09-04 driving [`0045`](0045-amnesty.md) row 23: a new hover line was written, the shell was driven, and `readout` came back without it — because `readout`, `shoot` and the socket reply all carried `_readout.Text` alone and the hover is a second `Control`. ⚠ **`Main.Palette`'s own remark states the rule and it is correct** — *a `Control` is invisible to `readout`, `shoot` and the socket's reply* — so this is not a defect in a claim, it is a **panel that should have been in the caption and was never put there**: `Aiming`, `Virgin` and every hover arm write there, which is to say ***everything a verb is aimed by was unassertable***. ✅ **Fixed in `Main.Captioned`** — the readout, a `hover —` separator and the hover beneath it, on all three channels. ⚠ **The palette and the two editing panels stay out**, so the rule stands and one panel joined it |
