...
|---|---|---|---|---|
| **1** | code | ⚠ **UNPICKED — milestone 6 closed on 2026-08-17 and nothing has been scoped to replace it.** This row is deliberately not filled in by the session that emptied it: choosing the next milestone is a reading of [`06`](../docs/06-roadmap.md)'s spine against the six roots and the gate board, and a session that has just spent itself inside one milestone is the worst-placed reader of what should follow. ⚠ **Two things the next picker needs.** Milestone 6's position was argued from **cost of delay** rather than from a dependency — unique in that table — so nothing downstream was waiting on it and nothing downstream is unblocked by it; the ordering question is genuinely open. And **`06`'s inventory cannot be trusted to be complete**: milestone 6 task 5 found `05 §1`'s name-resolution path specified and unbuilt, `adr/0098` found mode choice in neither of `06`'s two inventories because ***an inventory of unplaced mechanisms structurally cannot list a mechanism nobody designed***, and the District Pool sat in none of forty rows because it lives inside a milestone marked done. **Read the roots, not the row numbers.** | [`06`](../docs/06-roadmap.md) | The first row of the re-derived Phase 2 spine is shipped, and the second has not been named |
| **1b** | code | **`06` milestone 8 — Save/load. ✅ DONE 2026-08-18, [`0030`](0030-save-load.md), all ten tasks, ungated** — and it is **running in parallel with milestone 6 in a worktree**, because it adds no table and no column, so it **moves no State Hash and re-records no golden baseline**. Task 1 is the whole-world **rebuild audit** and needs no save format: `World.RebuildDerived()` (`World.cs:888`) and `Rows.FoldAll` (`Rows.cs:204`) measure the milestone's named risk directly, where the Factorio test measures it only once M Ticks have carried a wrong derived value into saved state — coverage today is **2 of 28** derived columns. ⚠ **Scoping moved the weight from the write side to the load side**: `Rows` has **no restore path at all**, so `06`'s *the milestone got smaller* is true of the format and false of the build. ⚠ **Three findings.** `BOR0901`'s own diagnostic tells every developer that *"both the save serialiser and the State Hash are generated from that one declaration"* (`Diagnostics.cs:181-183`) — [`adr/0093`](../docs/adr/0093-a-description-of-the-build-is-where-to-look-and-never-what-you-found.md) on **a compiler diagnostic**, which is the most persuasive description of the build there is because it arrives with the build's authority and is read when the reader is least able to check it; this milestone makes it true rather than correcting it. **"Milestone 8" now names two different milestones inside `src/`** — six sites mean *parking* (old 8 → new 7) and six mean *Save/load* (old 10 → new 8), filed to [`0012`](0012-corpus-audit.md). And `layer_cell.pollution_pass` is declared `Derived` with **nothing rebuilding it**, deliberately — so the audit needs an exemption and the declaration has no word for scratch. ✅ **That one is settled**: `Disposition` gains a **third value**, and the argument that decided it is that ***`Reference` is the same decision already taken in this codebase, on one column, with its ground written down*** (`Declaration.cs:120-135`) — a declaration-driven walk with one exempt column, where *a list of fields shares its blind spot with the bug it exists to find*. ⚠ **And the standing *one instance is not a general mechanism* objection is refutable rather than arguable**: that remark's own *"only one column needs it"* is now **seven columns across five tables**, reached within two milestones. The exemption is paid for by an **assertion** rather than a name — fill every `Scratch` column with garbage at a phase boundary and the hash trace must not move. ⚠ **The second settled decision moved a seam**: the write is **synchronous** here and the thread is the host's, but `adr/0087`:87-90 puts the file's **hash** on the background side at ~3× the copy, so the boundary is **copy** against **hash + serialise + write** and task 6 had it at *copy | write*. ✅ **Tasks 1–4 have shipped, 1,504 green and no baseline re-recorded**, and **two more decisions closed with them**. All five fields on the `Simulation` — `_phase`, `_inForce`, `_opened`, `_reloads`, `_degradation` — **dissolve** rather than joining the World, and ⚠ **three of the five were already answered in the doc-comment on the field** while the question sat open in [`0002`](0002-open-questions.md) for two milestones; ⚠ **dissolving out of the save is not dissolving out of the loader**, since a fresh `Simulation` has `_opened = false` and would take the first Ruleset after a load through the **opening** branch, recording no provenance trail entry at the exact moment the trail was designed for. And ⚠ **the header carries no generator version and no world seed** ([`adr/0111`](../docs/adr/0111-a-save-that-re-derives-nothing-needs-neither-a-seed-nor-a-generator-version.md), amending [`adr/0086`](../docs/adr/0086-a-save-has-no-schema-of-its-own-and-the-field-declaration-is-the-format.md)): the two are **one requirement**, no load path calls a generator, and since `adr/0021` **pins** that number ***a placeholder inverts the guard*** — an absent one is refused by the format version and `generator_version = 1` agrees with itself and loads a pre-terrain city onto a landscape that was not there. What replaces it is a **class** — the world-creation values that live in the binary rather than in a table — of which four are built and **each says *baked into the save* in its own file**, with nowhere to be baked into until now. ⚠ **Its live exposure is the Input Log's**, whose replay re-runs the generator from `log.Seed` every read: ***a version number guards the artefact that re-derives, not the one that restores***, routed there on the day per `adr/0073`. ✅ **Task 5 shipped too, and with it the last open decision.** The save is **streamed rather than assembled**: `Core` writes through an `ISaveSink` and reads through an `ISaveSource`, handing over each column's own contiguous storage, so **nothing proportional to the file is allocated at either end** — the largest single hand-over at 1,000,000 Citizens is **7.63 MiB against a 131.33 MiB file**. ⚠ **The deciding argument was not among the three options the brief listed**: `adr/0087` already spends a copy of the world at save time, so a staged file would put a second body of the same order beside it at the one moment memory is highest. ***A format with no authored layout can be streamed, because there is nothing to lay out.*** ⚠ **Task 2's `WriteBytes`/`ReadBytes` pair is deleted** — ***a copy exists to bridge two layouts and there is only one layout here*** — and ⚠ **two growth defects fell out, one of them shipped in task 3**: `Rows.GrowTo` doubled from the declared capacity and **did not terminate from zero**, which the loader reaches directly because it builds its world at zero capacity; the allocator's own `Grow` shares the premise and fails by indexing past the end of an empty array. ***A doubling growth rule assumes a non-zero base, and neither site said so.*** ✅ **All five open decisions are closed**, each by the task it blocked. ✅ **Tasks 6 and 7 have shipped, 1,530 green, and `05 §4` invariant 6 — *run N, save, reload, run M* — has machinery for the first time in the project's life**, green over seven cases and two Rulesets. ⚠ ***"The end of phase 7" and "the end of the Tick" are different instants***: the copy is taken after `World.Advance`, one statement later than `adr/0087` says, because `Advance` increments the **Tick** and the Tick has been saved state since `adr/0058` — a copy before it reloads into a world that **re-runs that Tick**. ⚠ **Finding that cost the same error it is about**: the first write-up blamed the **double-buffer swap**, which is wrong (`MapLayers` swaps inside the Layers phase), and the mechanism was inferred from the shape of the failure rather than read — `adr/0093` committed *while writing up a finding about an ADR making the same kind of mistake*. What caught it was the theory's own prediction failing: **all five sweep cases failed where it predicted only the diffusion-boundary ones would**. ⚠ **`adr/0087`'s hash clause is not buildable as written** — *"computed on the background thread from the copy"* cannot be done, because `HandleColumn.Fold` folds the **target row's monotonic id**, which the bytes do not contain — so **D4's conclusion survives and one of its two reasons falls**, and whether a save carries a verified hash is **the one open question this milestone added**. ⚠ **Task 7's two findings are about the instruments rather than the save.** ***A structural test over one fixture measures the fixture's content as much as the structure***: `adr/0086`'s owed column-set test covered **170 of 187** columns and reported nothing wrong, because corruption is silent about a table with no rows — `route_hop` is empty wherever nobody drives, and **`layer_cell` stands at zero rows under all four shipped Rulesets**, none of which emits pollution and whose land-value producer has only test callers. The fixtures were made to reach the tables and the residue is pinned **by name at the empty string**. And ***a test that allocates heavily is not a local decision in a suite that runs in parallel and asserts on allocation***: the first version allocated ~300 MB and made **two unrelated allocation assertions fail**, one of them over arithmetic that cannot allocate at all, settled by four full runs and routed on the day to `TableAllocationTests`' own remark — which said `GC.GetAllocatedBytesForCurrentThread` **is exact**, with six files resting on it. *"Exact" is a claim about what it counts, not about what it reads*. ✅ **Task 8 has shipped, 1,539 green with task 9.** `--save PATH` writes the world at the end of a run and **reloads it and runs both cities on**, printing the two hash traces side by side, because ***a save that is never loaded demonstrates nothing***; `--load PATH` resumes in a later invocation, which is the only property a save has that an in-memory copy does not. **Both are flags rather than a ninth mode, on `--series`' written criterion rather than a fresh judgement** — every mode here builds a city of its own to photograph and these ride the run already happening. ⚠ ***A label derived from a loop counter is a claim about where the loop started***: `Session.Write` labelled each trace sample `(i + 1) × hash-every`, which is the Tick only for a run beginning at 0 — every run this runner could do until `--load` existed — so a resumed run labelled its samples **128 and 256 while standing at Ticks 640 and 768**, in a file whose whole purpose is to be diffed against another. The header also carried the **default** `citizens 10000` for a run that never supplied it, in the one field that exists to say what would have to match for two traces to be comparable. ⚠ **A save resumed under a Ruleset it was not saved under is refused, and forcing it marks the trace `hash-broken`** — `RulesetCheck`'s polarity applied to a file, which is the mark `05 §7` asks for. ✅ **Task 9 closes the milestone — 49 Days, 100,352 Ticks, 49 saves, and the file's size is the declaration's computed total to the byte.** ⚠ ***A slot-exact save is sized by the city's high-water mark rather than by its population, so its size is an `adr/0006` instrument in its own right***: Lots stood at **488** all run and Buildings oscillated **231–286** with no trend, and the file still grew 1.0% and then **saturated at Day 34** — because the size is `SlotCount`, `adr/0086` forbids compaction, and a slot count is a high-water mark. **A file that keeps growing in a city that is not growing is a slot leak**, visible in one number. What is asserted is that it never *falls*: ***where it saturates is a property of the run's length; that it never shrinks is a property of the format***. ⚠ **The copy is published as a rate and not a duration** (`plans/0012` **Cause 5**) — **7.14 GB/s**, which puts task 2's 131.33 MiB at **~18 ms against `adr/0087`'s ~10 ms**, with the error direction unknown because a Debug build flatters the ADR and a 604 KB file fitting in cache flatters the measurement; **the copy at 1M is owed and this fixture cannot produce it**. The write is 2.57 GB/s, the bigger half, and it is the background thread's. ✅ **Both of the two questions that were not on the list are closed, 2026-08-18, with the user in the room — and task 10 was added to close the first.** ***Whether `Borough` is the name*** closed by **splitting the question**: `Borough` is the **codename** and is frozen, the **public** name is a separate deferred decision, and the magic number is a codename in a file header — so the rename trigger is ***discharged rather than met***, the only player-visible strings being two file extensions that nothing dispatches on. *(Task 4 had fired that trigger four tasks early and in silence, against D2's decision: **a decision is reversed by the build far more quietly than by an argument**, and this corpus's checks are all document-to-document, so none of them can see a header.)* ⚠ **The second closed by its own premise turning out to be false, which is the milestone's sharpest finding.** ***A save does carry a verified State Hash and it is folded from the copy***, so the simulation thread pays nothing: task 6's *a fold over the copy's bytes is not the State Hash* is true of **a column** and false of **the file**, because `Rows.cs:72-73` declares `id` and `generation` as `Saved` and the monotonic id a handle resolves to is therefore in the copy, in another table's block. ***A value absent from a column's own bytes can still be present in the copy.*** So `adr/0087`'s *never on the simulation thread* is **honoured rather than overturned**, the header goes to nine fields and 60 bytes, and **`05 §4` invariant 6 becomes a property of every load** rather than of the seven cases in the suite ([`adr/0112`](../docs/adr/0112-the-saved-set-is-the-hashed-set-so-a-save-can-compute-its-own-state-hash.md)). ⚠ **The false claim was pinned by a *test*, and that is what generalises**: `A_fold_over_the_bytes_is_not_the_state_hash` is still green and was always about something else — it folds the buffer flat, byte after byte, which nobody would want. ***A test name is a description of the build, so it says which symbol to read and never what is in it*** (`adr/0093`, on a surface that ADR does not list), and **a negative test is the most quotable kind of all, because it reads as a closed door** — three documents cited this one and none of them opened it. ⚠ **The cheap-looking answer would have spent a revisit trigger**: folding the *live* world was defensible on `adr/0087`'s own arithmetic and was the recommendation carried into the session, and it would have taken the save's blocking half from ~10 ms to **~42 ms at 1M** — the size that ADR's own trigger names as *visible*, reached by adding a feature rather than by growing the city. ***An argument that a cost is affordable is not a reason to pay it when it can be moved.*** ⚠ **One implementation against two sources, deliberately**: `Column.Fold` takes its bytes and a `TargetIds`, so the live path and the save path are the same code — a second fold beside the first would have been `plans/0012` *Cause 1* built on purpose. **1,549 green, no baseline re-recorded** | [`06`](../docs/06-roadmap.md) → milestone 8 | Ungated by session K, decided by [`adr/0086`](../docs/adr/0086-a-save-has-no-schema-of-its-own-and-the-field-declaration-is-the-format.md) and [`adr/0087`](../docs/adr/0087-a-save-is-copied-at-save-cadence-not-read-from-a-past-that-no-longer-exists.md), and it closes **`05 §4` invariant 6** — one of the two lints that has never had machinery. Landing it now makes it milestone 6's regression harness rather than a late audit |
