...
|---|---|---|---|---|
| **1** | code | **`06` milestone 7 — Parking. ✅ DONE 2026-08-19: scoped 2026-08-17, [`0031`](0031-parking.md), all eight tasks shipped, ungated** — session **H** cleared it on 2026-08-12 and both copies of the clearance agree. ✅ **All four decisions settled and tasks 1 through 6 shipped** — tasks 1 and 2 on 2026-08-18, tasks 3 to 6 on 2026-08-19 (`639a3a0`, `601b0f8`, `44e11e2`, `bf04db8`, `f53710e`) — `CarParkTable`, `[[building]] parking`, `CitizenTable.ParkedIn`, the supply created, ceilinged, located and freed; `[parking] radius_metres = 400` in all five shipped Rulesets; the shed query and its per-Segment supply index; the acquire and release pair; the endpoint swap that gave `adr/0008`'s two flanking Legs a real cost; and `Invariant.ParkingOccupancyIsConserved` in the end-of-run tier. ⚠ **The milestone's named risk is still unpaid rather than paid by task 5**: `minimal.toml` declares no `car_ownership_percent`, so the golden session has no drivers, no car Trip and no Leg to swap — ***a task that changes what a car Trip costs changes nothing in a world with no cars***, and no baseline has moved for any of tasks 4 to 6. **Task 7 shipped the same day** — `--parking`, which prints the walk **from** the car against the walk **to** it and refuses four of the seven shipped Rulesets, because only `congested.toml` states both `[parking]` and a `car_ownership_percent`. ⚠ **Its first run found the arrival walk bounded by the shed and the departure walk bounded by nothing** — 0 of 3,330 arrival walks past the 4.8-minute ceiling against 57 of 3,330 departure walks, the longest at 16.6 minutes, in a city where **no Trip failed at all** — so the obvious explanation was refuted by the run that found it. ***An instrument is worth building at the moment it can refute the story you would otherwise have written down.*** Filed to [`0002`](0002-open-questions.md) §C. ✅ **TASK 8 SHIPPED THE SAME DAY, and the milestone is DONE.** [`rulesets/scarce.toml`](../rulesets/scarce.toml) as the eighth shipped file — the *sixth Ruleset* §D1 asked for — a seven-rung scarcity sweep, a forty-eight-Day `adr/0006` run, and **a second golden session that drives**. ⚠ **The Definition of done's golden-session obligation was unmeetable as written and nobody had noticed**: *the walk Leg's cost is non-zero for at least one Citizen in the committed golden session*, and that session runs `minimal.toml`, which grants nobody a car **by design** — ***an obligation naming a fixture that cannot satisfy it is not a demanding obligation, it is an unread one.*** Met by a **second** committed session on `congested.toml`, `populate` and nothing else over two whole Days, so nothing the first one covers stopped being covered — which also closes two coverage holes `tests/Borough.Tests/Golden/README.md` had been carrying since 5c task 6 and milestone 7 task 1. ⚠ **Two Days rather than one, and the count is the argument**: departure walks that leave their starting Address run 0 of 975 at half a Day, **2 of 1,883** at one Day and **136 of 3,330** at two — ***one Day is not vacuous, it is worse, because it clears the assertion on two walks.*** ⚠ **And the fourth reading task 8 offered *for free* is VOID**: `adr/0083`'s stale-shed trigger describes a **materialised** shed, `World.TryChooseParking` builds one fresh on every arrival, and **a structure that does not persist cannot go stale** — ***a revisit trigger written against a design is not a revisit trigger against the build that design chose.*** Filed and struck in one entry in `0002` §B. ⚠ **Task 6 found a defect check 7 structurally cannot see**: `02 §10`'s tier table named `ParkingOccupancyIsConserved` in its **per-Tick** row, the tier `adr/0084` demoted it out of, and ***a check that an obligation has a member does not check that the row it sits in is true of it*** — the same hole is open for every other name in that table. ⚠ **Task 3's index shipped with two defects, both fixed in `0d8b114`, and neither was caught by anything task 3 wrote** — a `Derived` column nothing inside the `World` rebuilt, found four days later by milestone 8's `DerivedRebuildAuditTests`, and a self-linking `IndexList` that made the suite **stop terminating** rather than fail. ***A defect that produces no output is not caught by a suite that reports failures.*** Three ADRs settle most of it in advance — [`adr/0009`](../docs/adr/0009-parking-is-modelled-supply-never-search.md) *supply, never search*, [`adr/0083`](../docs/adr/0083-a-sheds-use-is-the-arrival-query-and-a-stale-shed-is-wrong-by-a-bounded-walk.md) *the shed has exactly one caller*, [`adr/0084`](../docs/adr/0084-parking-occupancy-is-two-checks-and-an-invariant-over-absent-state-cannot-be-written.md) *two invariants, neither per-Tick* — and **two [`0002`](0002-open-questions.md) questions already name this milestone's own first run as their ratifier**. ⚠ **Read decision 1 before task 1**: the thing that holds a parking space does not exist and two ADRs each put it on an object that is freed when the journey ends, which carries a live defect in `adr/0084`'s conservation sum. ⚠ **Read decision 3 before task 2**: a generated city cannot vary parking occupancy, so the shed radius's ratifier needs a **world**. ⚠ **Two prohibitions become enforceable here for the first time** — the vehicle Access Point is *replaced* and never supplemented, and an exhausted Shed may never return it — and the behaviour they forbid **reads as generous rather than as broken**, because a full car park would cost less than an empty one. 5c examined both and found they had **no ground**, since no Shed existed to produce the fallback. ⚠ **This is the milestone that makes every car Trip worse**, by `adr/0008`'s recorded direction of error: every congestion figure and every Commute Budget rung in the corpus was taken on a journey missing both ends | [`06`](../docs/06-roadmap.md) → milestone 7 | It is the ungated row nobody had taken. ⚠ **Its position is argued from cost of delay rather than from a dependency**, like milestone 6's — so nothing downstream waits on it, and that is exactly why it can slip for ever |
