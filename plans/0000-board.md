...
| **6** | code | **5a-bis — the Lot subdivider and `build_road`. UNGATED, and it was not on this board until 2026-08-11. Brief: [`0022`](0022-the-lot-subdivider-and-build-road.md).** [`0020`](0020-the-road-graph.md) → *What this excludes, and who owns it* scoped both and named the slice; nothing then scheduled it. `02 §2.2` specifies the subdivider fully — *Lots are generated, not painted*, frontage on at least one Street, **Arterials grant no frontage** — and `plans/0012:373` flags that it has **no milestone anywhere**. `CommandKind.Connect` is declared at `Command.cs:18` and throws at `Simulation.cs:332`; `01 §2` counts it among the player's five verbs and the corpus calls road editing *the player's core verb*. **Putting edits in the Input Log is what makes the Epoch exercised by replay**, which is the half worth doing deliberately. **This is the code work available while F runs**, and the two do not contend | [`0022`](0022-the-lot-subdivider-and-build-road.md), [`0020`](0020-the-road-graph.md), `02 §2.2` | `02` carries the admission 5a made urgent: *"every Building is on the Road Graph by construction is currently true **because there is no Road Graph**, not because of construction."* |
...
**So the code track is what is left, and it has two things in it** — **5a-bis** (row 6, the Lot subdivider
and `build_road`, ungated) and **5b** (Trips, Legs and the pedestrian layer, unblocked by F this
afternoon). [`0021`](0021-trips-legs-and-the-pedestrian-layer.md) task 1 says which order they want:
5a-bis produces **frontage**, and an Access Point is downstream of frontage, so **5a-bis first** means 5b
inherits real Access Points instead of inventing them nearest-Segment-by-construction.
