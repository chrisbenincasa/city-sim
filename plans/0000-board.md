...
|---|---|---|---|---|
| **1** | code | 🔴 **Milestone 26 — the purchase, and its CONTENT GATE IS DISCHARGED.** Session W settled what raises a Provider, what declines one and the land-use split ([`adr/0163`](../docs/adr/0163-demand-for-a-shop-is-elapsed-unserved-need-in-reach-and-building-claims-it.md)–[`adr/0166`](../docs/adr/0166-a-business-runs-rules-and-its-rules-live-as-long-as-its-tenancy.md)). ⚠ **It did not get smaller: `adr/0166` widens the Rule Instance's subject to a Business and 26's decline half depends on it** | [`0044`](0044-the-purchase-and-the-provider-that-answers-it.md) | **Decomposed 2026-08-25 into ten tasks, and the inherited order was backwards.** 🔴 **`adr/0166` is a precondition of the PURCHASE and not only of the decline half**, because a buyer's `pool` term resolves to a seller's Bin and no Business can hold stock or run a Rule. ⚠ **Four open decisions, none in a ledger, and two could make the work smaller** — 🟢 **DECISION 4 IS SETTLED and it made the work LARGER**, because the reading that would have shrunk it was refuted by opening the symbol it named. 🟢 **TASKS 1 AND 2 OF TEN LANDED 2026-08-25** — a Business runs Rules and owns Bins (`adr/0166`), then the land-use split (`adr/0165`), assertion tier green at 2,311 with all three golden artefacts re-baselined; 🔴 **task 2 was three lines of painting and five subsystems of cascade**, the live one being `PlacementEngine` drawing candidates over the whole Lot table so that commercial land made `candidates` quietly mean less — new `Space.ZonedLots`, `TradeBlockStride` filed in [`0002`](0002-open-questions.md) §D1, queue item 19 filed (`0044` **F4**–**F13**) |
