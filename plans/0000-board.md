...
| **2** | code | **Milestone 24 — terrain and the land rows.** Scoped out of sequence 2026-08-22 and **split in the scoping**: the terrain half has no upstream at all, so it builds beside 12, while **Shocks, Disasters and the Intensity Dial are UNPLACED** pending 13, 15 and 16. **Ten tasks, six open decisions, none settled** | [`0038`](0038-terrain-and-the-land-rows.md) | The only other ungated code row with **nothing upstream of it**, and the split is settled by [`adr/0139`](../docs/adr/0139-milestone-24-is-two-milestones-because-a-dial-cannot-scale-a-figure-nothing-authors.md) |
