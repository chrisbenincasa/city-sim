...
| **5** | code | 🔴 **[`0003`](0003-build-plan.md) hash-moving queue item 14 — `World.Drain` and `Invariant.WaiterIsBlockedByTheBinItNames` contradict each other**, filed unfixed 2026-08-22 by item 11 and expressly not fixed inside it. The drain **stops rather than skips** at an uncovered waiter — deliberately, because *"skipping an uncovered waiter to reach a smaller one behind it would starve every large waiter permanently"* — so a small waiter queued **behind** a large one sits parked on a Bin that covers it. `CheckQueueStillBlocks` then walks **every** waiter with no positional budget and `BinStillBlocks` compares each requirement against the Bin's **whole** level. ⚠ **The invariant is stated more strongly than the drain can deliver.** ⚠ **MEASURED, not reasoned**: adding an end-of-run check to `BinTests.The_drain_stops_at_an_uncovered_waiter_rather_than_skipping_it` fires the invariant at Tick 0, and **that test asserts both waiters stay waiting and is correct to**. 🔴 ⚠ **It is latent ONLY because no shipped Ruleset puts two Rules on one Bin** — `BinWaitListTests`' own header: *"under `local` scope no two Rules share a Bin they do not both own"* — and ***`Scope.Pool` is precisely the mechanism that ends that***, so **row 1 is what makes it reachable and it wants settling before the Pool ships** | [`0003`](0003-build-plan.md) → *The hash-moving queue* item 14 | ⚠ **The repair is a design question and must not be taken inside another item's commit**: either the invariant narrows to the drain's guarantee, or the drain stops starving small waiters, and ***those are different cities*** |
