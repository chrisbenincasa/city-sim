...
| **7** — Parking | ✅ **Cleared** by session **H**, 2026-08-12 — and 🟡 **SCOPED 2026-08-17**, [`0031`](0031-parking.md), eight tasks. ⚠ **Four open decisions, and three of them are owed before task 1**: what holds a parking space (a Trip and a Traveller are both freed when the journey ends, and the subject that survives has never been built); whether a parking Bin is a `BinTable` row or its own table (**four structural mismatches against one name** — no Address, a Goods `Resource`, two wait lists where *nothing ever waits*, and `CONTEXT.md:252` reserving the type for Goods and Money); and whether supply exists at Road Segments or only at Buildings, `adr/0009` having said **both** while the street-parking player tool is a **seventh verb** against a list [`adr/0091`](../docs/adr/0091-clearing-land-is-bought-rather-than-taken-and-demolish-is-the-sixth-verb.md) fixed at six with the user in the room. **The fourth is what ratifies the shed radius** | — |
