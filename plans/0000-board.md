...
|---|---|---|---|---|
| **1** | argument | 🔴 **Session W — the Provider kind's content.** The **one thing milestone 26 is gated on**, and [`0002`](0002-open-questions.md) §A said *nobody owns it* until 2026-08-25. Four decisions: what **raises** a Provider, what **declines** one, the **land-use split**, and **which Ruleset** carries it. ⚠ **It is CONTENT and may not be settled by whoever happens to be writing the Ruleset** — it runs with the user in the room | [`0043`](0043-session-w-the-provider-kinds-content.md) | **The standing rule is satisfied rather than stretched**: 26 is concretely blocked on this and on nothing else. 🔴 ⚠ **The brief finds 26's SECOND gate is not a gate** — the bankrupt world is 26's output rather than its precondition (**W1**) — ***but a gate is not struck before the sitting that owns it has run.*** 🔴 ⚠ **And it names a live risk worth reading BEFORE promoting this**: if commercial demand classifies as *unbuilt*, [`adr/0070`](../docs/adr/0070-an-unbuilt-mechanism-is-not-a-design-constraint.md) makes the answer **build it**, and 26 gets BIGGER rather than unblocked |
| **2** | code | 🔴 **Milestone 26 — the purchase, `Scope.Pool` stops throwing.** Milestone **27 closed 2026-08-24**, all five tasks, so 26's largest gate is discharged. ⚠ **THREE GATES REMAIN AND THEY ARE NOT 27's** — the Provider kind's three content decisions ([`0002`](0002-open-questions.md) §A) and a world where a Building genuinely runs out of money; [`0003`](0003-build-plan.md)'s row enumerates them and this cell keeps no copy. **Its tasks are [`0037`](0037-goods-between-buildings-the-district-pool.md)'s 7–10**, kept there as written | [`0037`](0037-goods-between-buildings-the-district-pool.md) tasks 7–10 | **27 made the actor EXIST and 26 is what it was for**: a purchase needs a payer that can hold, earn and spend, and until `Scope.Pool` resolves a Business has one Bin with nothing to put in it — which is why every number `levied.toml` and `founded.toml` produce ratifies nothing |
