...
| **5** | code | **[`0003`](0003-build-plan.md) hash-moving queue item 11 — filed unfixed 2026-08-20 by milestone 9 task 7, ✅ DIAGNOSED 2026-08-21, fix held.** `Invariant.WaiterIsBlockedByTheBinItNames` panics on **every** Ruleset for a headless run shorter than **64 Ticks**. 🔴 **The cause is neither reading this row was filed with**: `RuleEngine.Evaluate` is Phase 2 and writes nothing, Phase 3 applies — so a Rule judged short in Phase 2 is **made whole by another Rule's deposit applied earlier in the same Phase 3**, and `RuleEngine.Stop` subscribes it anyway, onto a Bin whose `World.Drain` has already run against a queue it had not yet joined. ***A waiter parks on a Bin that the same Tick has already refilled.*** ⚠ **The fix is HELD, and not for a hash reason** — it re-records the golden baselines while milestone 11 is re-recording the same files, which is a collision on a shared artefact ([`adr/0100`](../docs/adr/0100-moving-the-state-hash-costs-nothing-until-somebody-is-carrying-a-save.md)) | [`0003`](0003-build-plan.md) → *The hash-moving queue* | ⚠ ***Nothing in the suite runs the headless runner for under 64 Ticks***, which is why a panic on every shipped Ruleset has stood unseen. 🔴 ⚠ **Three particulars of the original filing are struck** — the rows are **not** the same every time and neither is the **queue** (`N=4` → 166/111 Supply, `N=16` → 1336/891 Supply, `N=32` and `N=48` → 24/17 **Space**); *clean at 64 and 128* is really **zero for every `N` from 64 to 96**; and the *arms wrong or parked legitimately* pair is false. **The hash column is answered: yes.** ⚠ **That it can strand a Rule permanently is an argument from the mechanism, not a measurement** — all 36 spurious parks in a 2,048-Tick run healed, and the world that would settle it is one whose producer stops writing the Bin. [`0003`](0003-build-plan.md) → *The hash-moving queue* owns the record |
...
| **11** — the Outside *(milestone **14** from 2026-08-16 until the 2026-08-18 economic reorder)* | ✅ **ASSESSED 2026-08-20, and the answer is that NOTHING names a gate on it** — 🟡 **SCOPED the same day**, [`0035`](0035-hinterlands-and-arrival-through-the-gate.md). *(Original row: **NOT cleared, and never scheduled**; session **J** cleared `05 §7`'s save-format half and the Outside *layout* against a milestone that was **8**.)* ⚠ **The one text that reads as a gate is a citation about a milestone that did not exist** — `05 §6`'s *"gates milestones 10 and 11"*, three times disclaimed, and `06` states that what §6 still owns *"gates nothing in 6–24."* ⚠ **What scoping found instead is not a gate and must not be recorded as one**: arrival is specified as milestone **16**'s choice model, which is *unbuilt* rather than refused ([`adr/0070`](../docs/adr/0070-an-unbuilt-mechanism-is-not-a-design-constraint.md)), so the answer is **build X** and the cost of doing so is that it inverts an edge. That is [`0035`](0035-hinterlands-and-arrival-through-the-gate.md) decision **1** | **nothing named** |
