...
| **K2** | `06`'s Phase 2 **ordering** only | Re-derive the sequence and place the **eighteen mechanisms `06` lists as having no milestone**. **Last** — E and G move what it sequences, *and* it wants 5c's routing load. ⚠ **Its input is known incomplete as of 2026-08-15**: that list's entry condition is *settled by an **ADR***, which excludes anything specified in a design document, so a sweep on the *existence* test is owed before K2 can place anything | Planning Phase 2 at all |
