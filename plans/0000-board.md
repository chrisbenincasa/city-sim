...
predicate it immediately found broken in the committed golden baseline. **Slice 10 task 11 has since
shipped too**, so `adr/0059`'s `revisit_ticks` is in and at 1M the shipped Ruleset raises **2,898
Buildings** where it raised none. **Tasks 3 and 4 then added two more, decided and unshipped**
...
and a Bin's `level` and `capacity` widen to `long` — because `Money` is a `long` and a Bin's level was
an `int`, so every payment into a Bin **narrowed 64 bits to 32**. **They are what remain, as one
commit** — [`0003`](0003-build-plan.md)'s hash-moving queue, item 3. **No gate is red anywhere in the corpus**, and no session gates a slice any more. S4, S0a, S0b and S2 R0–R8 have
