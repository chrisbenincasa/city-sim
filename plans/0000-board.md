...
✅ **Milestone 8 — Save/load is CLOSED**, all ten tasks, and its row has left the board.
[`0030`](0030-save-load.md) owns the record and [`0000a`](0000a-board-archive.md) indexes it. ⚠ **Its
row came back once, and the way it came back is worth knowing.** `main` struck it on 2026-08-18 when
the milestone shipped; the parking branch still carried the live copy, and merging the branch
**restored the struck row** without a conflict, alongside silently dropping milestone 7's cleared row
from the per-milestone gate table below — the very thing that table's own warning says must not happen
again. ***A merge resolves text and cannot resolve status, so a view that lives in a file is restored
by a merge exactly as readily as it is edited by one.*** Both repaired 2026-08-19.
