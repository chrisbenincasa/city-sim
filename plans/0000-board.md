...
| ~~**3**~~ | ~~spike~~ | ~~**S5 — the Lane kernel.**~~ **RUN 2026-08-10, all four rounds twice plus a BenchmarkDotNet cross-check; PUBLISHED 2026-08-11 into [`spike-results`](../docs/spike-results.md) → *S5*, [`0013`](0013-tick-budget.md), [`adr/0016`](../docs/adr/0016-the-lane-is-the-entity-not-the-car.md) and [`0002`](0002-open-questions.md).** **~325,000–330,000 Vehicles a Tick a core** with Overlaps, **~4,510–4,580 Microscopic Segments in 15.6 ms**, and `0013` has the Microscopic row it had **never** held. **Two of five tripwires fired and both reach a sentence rather than a design**: `adr/0016`'s transplanted 400,000 is not reproduced, and its *"constant of a `memcpy`"* is **26–29× a bare walk**. **Every structural claim survived** — flat `O(n)`, no spatial index, zero allocation, determinism nearly free — so **the gap is `adr/0003`'s arithmetic and not the model**: three integer divisions a Vehicle a Tick, two removable for 1.63–1.75×, which is now a §D2 row because *how the IDM is spelled* is hash-bearing. **Owes the canonical `performance` capture**, and the harness cannot be deleted before it | [`0019`](0019-s5-lane-kernel.md), [`spike-results`](../docs/spike-results.md) → *S5* | **The rarest thing in the corpus, and it stayed that way**: a spike that blocked on nothing, contended with nothing, and came back with the direction of surprise pointing **down** for the first time — every absolute is a `powersave` lower bound, so the one number that disappointed can only improve. **And it is a negative sighting of scatter ≈1.5**, the first: this kernel is arithmetic-bound, so there is nothing for scatter to move |
...
| **S5** | **The Microscopic tier's price, which the corpus had transplanted from somebody else's float engine.** **~325,000–330,000 Vehicles a Tick a core**, ~4,510–4,580 Segments in 15.6 ms, break-even promotion residency **1 Tick**. `adr/0016`'s structure survives whole and **its two quantitative claims do not** — 400,000 not reproduced, and the *`memcpy` constant* is 26–29× a bare walk. **The gap is `adr/0003`'s integer division**, which makes *how the IDM is spelled* a hash-bearing choice worth 1.63–1.75×. Also found the **arrival Tick is a demotion discard `03` invariant 3 does not enumerate**, and that the clever Overlap cursor beats the naive scan by only 3–7%. Owes its `performance` capture | [`0019`](0019-s5-lane-kernel.md), [`spike-results`](../docs/spike-results.md) → *S5* |
