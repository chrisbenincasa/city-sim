...
**The next code row is `06` milestone **9** — the land value target and the composed Layers, 🟡 SCOPED
2026-08-20 ([`0034`](0034-the-land-value-target-and-the-composed-layers.md)) and unstarted.** 6, 7, 8 and 10 are all shipped; 9 is the position they went round. ⚠ **It had no row on this board
at all until 2026-08-20** — not in *Do these next*, not in the per-milestone table in *Blocked*, not in
[`0002`](0002-open-questions.md) — so nothing here ever said whether it was gated, and the answer, arrived at by
grep rather than by assessment, is that **no document names a gate on it**. ***A milestone with no row is not a
milestone with no gate***, and the way this one stayed invisible is that every reader took the ranked table's top
row as *the next thing* and the table's top row was a milestone that had shipped. Its named risk is live and was
checked rather than quoted: `MapLayers.SetLandValueTarget` (`src/Borough.Core/Space/MapLayers.cs:319`) has test
callers and no caller in `src/`.

⚠ **Scoping found the milestone is two milestones, and the sharpest finding is a formula rather than a
schedule.** `02 §2.4` composes desirability with `w₁·land_value` as its first term **and** moves land value
toward desirability, so the field is a first-order lag on an affine function of itself: `w₁` is a **gain** of
`1/(1 − w₁)` on the other four terms, not a weight among five, and at `w₁ ≥ 1` every Cell diverges. Both halves
have sat in adjacent paragraphs of that section since slice 6, and the formula appears in exactly two documents,
neither of which mentions the loop — ***a formula nobody evaluates is not checked by being read.*** Beside it,
`Step`'s minimum-step-of-one, which exists to stop a Cell settling short of its target *on whichever side it
approached from*, reaches a clean stop **only because the target holds still**: against a target that moves when
land value moves the fixed point is not an integer and every Cell oscillates by ±1 for ever, in saved and hashed
state. ***Two decisions each correct alone can be incorrect jointly, and the seam is invisible from either side.***
Six decisions owed, five of them before any task that composes; [`0034`](0034-the-land-value-target-and-the-composed-layers.md) **F1**–**F7**.

✅ **Decision 1 is SETTLED, 2026-08-20 — `w₁` is deleted**
([`adr/0122`](../docs/adr/0122-land-value-is-not-a-term-in-its-own-target-and-a-term-on-both-sides-of-a-lag-is-a-gain.md)).
The question put was *accident of notation, or prestige feedback somebody meant to build?*, and the answer was
**accident** — so the loop goes rather than being bounded, and no `w₁ < 1` check is written, because policing a
deleted term is machinery with no subject. Prestige feedback is **undesigned** rather than refused. ⚠ **And the
formula had THREE homes, not the two `0034` recorded**: the third is `MapLayers.Desirability`'s own doc-comment,
the copy a programmer reads, and it is invisible to every check in `tests/Borough.Tests/Corpus/` because those are
...
✅ **Decisions 3 and 4 are SETTLED, 2026-08-20, and task 8 is DONE with them** ([`adr/0124`](../docs/adr/0124-terrain-suitability-is-baked-at-world-creation-and-the-layer-holes-that-need-it-move-to-milestone-24.md)). Fertility, Sealing's decay, Woodland, replanting and Water Bodies leave 9 for **24**, which **absorbs** them. ⚠ **They are *placed*, not deferred** — each needs terrain and terrain has one source, and **15 was never available for Water Bodies**, because water is immutable and seed-generated. ***A row with nowhere else to be is not deferred; it is placed.*** ⚠ **The sitting found an artefact 24 owes that no document named**: terrain suitability **baked at world creation into a stored per-Cell column**, because `adr/0021` says terrain *feeds desirability* **and** *is never read in a Tick phase*, and Fertility composes **inside a Tick** — ***two cells each correct alone constrain the build jointly and neither says so***, which is decision 1's shape in a second document. ⚠ **`06`'s note that 24 *depends on nothing in the spine* was falsified by the move and rewritten in the same edit** — ***a row that absorbs work absorbs the sentences that were true while it was empty***. ⚠ **Sealing's decay is blocked twice**: the stated `sealing_decay_tau = 0` **and** no caller for `DecaySealing` anywhere in `src/`; scheduling it is a hash-bearing cadence owing a ratifier, so it goes to 24 with the rate. **No Ruleset was edited** — baseline comments move hashes. **Two decisions remain: 5 the ratifiers, 6 the ±1 oscillation.**

✅ **Decision 5 is SETTLED, 2026-08-20 — a floor and a debt, two §D1 entries per weight** ([`adr/0125`](../docs/adr/0125-a-ratifier-that-needs-a-consumer-nobody-built-is-not-reachable-so-the-weights-get-a-floor-and-a-debt.md)). ⚠ **The decisive fact came out of `src/`: nothing in the city reads land value** — outside `MapLayers` the only readers are the headless layer dump and the Ruleset loader resolving a name — so **the quantity that would refute an absolute scale does not exist**, every quantity of that kind being a consumer's. ***A ratifier that needs a consumer nobody built is not reachable***, and writing one anyway repeats **milestone 7 task 8** one milestone later: that ratifier named a machine, a world and a quantity correctly and was still unreachable, because ***nothing in `adr/0052`'s checklist asks whether the named state can occur***. Seen this time **before** the number was written. **The floor** is reachable now — the acceptance run, on `congested.toml` because only it and `scarce.toml` have a driver, reading that the field varies, that both terms are visible, and the **pollution/noise correlation**. **The debt** is triggered by **milestone 13, the price surface**, and the weights stand unratified from 9 to 13. ⚠ **The floor refutes and never confirms** — ***a reachable check standing in the place of an unreachable one is how a number comes to look settled.*** ⚠ **`w₃` is not yet meaningful at all**: noise's output units are task 1's choice and `w₃` absorbs them, so it is chosen **after** task 1. ✅ **TASK 1 IS SHIPPED, 2026-08-20** — `LineSourceQueries.Noise` and `.NearRoadPollution` compose; `.Amenity` still throws. **No Ruleset key, no hash moved.** ⚠ **The landing site already existed and [`0034`](0034-the-land-value-target-and-the-composed-layers.md)'s survey missed it** — a 3c stub whose **three** throw messages all said the Road Graph *does not exist in Phase 1*. **Four findings, three of them defects the tests caught: F8** the sum's domain was never stated and **summing log-domain values is wrong**, so intensities accumulate and the logarithm is applied once — decision 1's *two correct sentences constraining jointly* for a third time; **F9** the intensity scale is **not** a scale under `Log1P` but decides whether the city sits in the linear or the logarithmic regime, ***found only because the test asserted sub-linearity rather than a number***; **F10** `Frontage.Locate` answers *which Segment does an Address front* and gave a zero background everywhere off the carriageway, **disabling the enumerate-by-loudness crossover without failing anything**; **F11** `RoadFixtures.Chain` declares `block_tiles = 512` with 32-Tile spacing, so **every test written against it exercised the linear scan** and the lattice window had no coverage — ***a fixture named for a shape is not a fixture of that shape.*** ✅ **TASK 2 IS SHIPPED, 2026-08-20** — `Desirability` composes `− w₂·pollution − w₃·noise` and no longer throws; no hash moved. ⚠ **F12: the composition is per-Cell and noise is explicitly sub-Cell, and no document had noticed** — and the shipped geometry makes the obvious fix the worst one, because a Cell is 32 Tiles and `block_tiles` is **32**, so ***Streets run along Cell edges and a Cell's centre is systematically the quietest Tile in it.*** Settled with the user: **the composition works at a Tile**, and how a Cell samples it becomes an explicit hash-bearing decision owned by **task 4** rather than one buried in the composition. ✅ **`DesirabilityShortfallTests` is written and verified to bite** — a document may say *desirability is composed* only if it names **amenity** — with a **vacuity guard** beside it, because a rewording would otherwise leave it green over a corpus it had stopped reading. **Deleted at milestone 15.** ⚠ **F13: a partial composition is what the named-hole discipline does *not* cover** — a hole that throws is safe because nothing can read it, and this returns plausible numbers with its only positive term missing. ✅ **TASK 3 IS SHIPPED, 2026-08-20** — four `[layers]` keys in all eight Rulesets and **five §D1 entries**, two of them `w₂`'s floor and its debt. ⚠ **F14: decision 5 said *two weights, not five*, and the query brought two more hash-bearing numbers with it** — a range and an intensity, neither a weight and neither optional. ***A count of the numbers a decision opens, taken before the mechanism is built, is a count of the ones the decision could see.*** **Only one of the four has a derivation and the file says which**: `noise_intensity_percent = 400`, because at 100 a Street at its stated capacity drops under unity by about 150 m and its noise then adds **linearly** — task 1's F9. ⚠ **F15: *editing a golden Ruleset moves thirty-two committed hashes* IS WRONG.** What moved was a **file fingerprint** in eight header lines; ***not one of the thirty-two State Hash samples moved***, because a key nothing reads cannot change the city — ***saying “moves N committed hashes” of a fingerprint change is Cause 5 with the units dropped.*** Corrected in `CLAUDE.md` and the Golden README. ⚠ **And I nearly deferred the task into task 4 to regenerate the baselines once, which `adr/0100` forbids by name** — the ADR caught a live instance of the thing it was written for, and the cost it guards against turned out to be eight lines. ✅ **TASK 4 IS SHIPPED, 2026-08-20** — `SetLandValueTargets` runs from phase 5 on the land value cadence, retargeting before it drifts, and [`adr/0126`](../docs/adr/0126-a-cell-samples-desirability-at-its-quadrant-centres-and-a-line-sources-area-mean-does-not-converge.md) records the Cell-sampling decision. ⚠ **F16: the derivation the task was opened to write down was REFUTED by the measurement.** A quadrature order justified by convergence was expected; **the area mean of a line source does not converge**, because the Segments sit on the Cell's own edges — orders 1/2/4/8 give −252,011 / −296,734 / −323,982 / −337,116, still moving 4.1% at the last step. ***The sample set defines the Cell's value; it does not estimate one.*** What replaced it is true and **reachable**: the *ordering* between Cells survives the order — **615 of 630 pairs agree** between 2 and 8, the other 15 being ties inside 1% — and ordering is all anything reads. ⚠ **Contrast task 3 deliberately**: same field, same week, and this number got a ratifier a machine can reach because its property does not need a consumer to exist. 🔴 ⚠ **F17: NO BASELINE MOVED and the plan said every one would.** The only thing that creates a Cell row is `EmitPollution`, and ***no shipped Ruleset emits any pollution*** — all eight say so in their own headers — while `MapLayers.Seal` has no caller in `src/` at all. ***The milestone's mechanism is built and unobservable, for want of Ruleset content rather than of code.*** 🔴 ⚠ **F18: that makes decision 5's FLOOR unreachable — `adr/0125`'s own failure one level down, four hours after it was written.** The floor names `congested.toml`, whose Cell table is empty, so all three of its readings are unreadable. ***`adr/0052` still does not ask whether the named state can occur, and knowing that it does not ask was not enough to make me ask.*** **Routed**: both §D1 floor rows carry the 🔴, `0013`'s row is filed with its multiplicand **guessed at zero**, and **task 7 now owes a ninth Ruleset with an emitting kind before it owes a run.** ✅ **TASK 7'S WORLD IS BUILT, 2026-08-20 — `rulesets/fouled.toml`, the ninth Ruleset**: `congested.toml` plus **one Rule that emits pollution**, so the difference between the two files is the field and nothing else. **Decision 5's floor readings 1 and 2 are taken and pass**; reading 3, the correlation, is owed to the run. ⚠ **F19: the emitting kind is `dwelling`, contradicting the sentence the other eight files repeat.** An industrial kind was tried and abandoned — a Zone Rule builds only while the Unplaced Pool is non-empty and the Pool is unhoused **Households**, so ***the only demand signal the build has is demand for homes***; the two-kind city could not be explained. ⚠ **And the reason it could not be was a swapped index** — `dwelling` is kind byte **1**, so the diagnostic's counters read each other's kind and the city was filling with `works`, which is `CLAUDE.md`'s own named hazard arriving as written. ***A count printed against a hand-written index is a measurement of the index.*** ⚠ **F20: the first two emission settings read as ZERO for two different reasons**, and the second is the one to keep — a diffused Layer is stored **in kernel units** and normalised at the *read* site, so a column holding 1,393 reads as 0. ***A stored magnitude and a read magnitude are different numbers.*** Settled at `rate = 128`, `amount = 1`: pollution **26–29** against noise **2.5–3.8**, land value settling at **−28 from day 4**, **177–180 of ~262 Cells distinct**. ⚠ **F21: THE NOISE TERM IS ZERO AT MIDNIGHT** in a 100%-car city — volume is instantaneous and land value is the only part of the composition with memory, so ***the target oscillates at the Day's own period***. **That is decision 6 arriving as a measurement before its sitting**: not whether the lag can rest, but what it rests around. ⚠ **F22: `CLAUDE.md` said *all seven Rulesets* of the `[parking]` keys while eight files stood and six carried them** — a count in prose, drifting, in the file that says ***count them rather than quoting a total***. Replaced with the condition. ⚠ **F23: a defect was found on the way and is filed unfixed** — `Invariant.WaiterIsBlockedByTheBinItNames` panics on **every** Ruleset for a headless run shorter than **64 Ticks** (red at 4, 8, 16, 32; clean at 64 and 128; same two rows every time), and ***nothing in the suite runs the runner for under 64 Ticks***, which is why it has stood unseen. [`0003`](0003-build-plan.md) hash-moving queue **item 6**, routed under `adr/0073` rather than worked around. ✅ **TASK 5 IS SHIPPED AND DECISION 6 IS SETTLED, 2026-08-20** ([`adr/0127`](../docs/adr/0127-the-land-value-target-never-stops-moving-so-the-question-is-what-the-lag-rests-around.md)) — **all six of this milestone's scoped decisions are now closed.** The reading on `fouled.toml`: **185 of 262 Cells move per cadence sample**, **50 never move at all**, widest swing **74,373** raw Q16.16, mean value **−567,787 against −563,871** across a four-Day window, so **no trend**. ⚠ **F24: the thing decision 6 worried about is not what the field does, and the SIZE of the motion is what says so.** The minimum-step-of-one is safe — `adr/0122` deleted `w₁`, the target is exogenous, the gap reaches zero, and **the fifty still Cells are the direct proof**. ***A ±1 flicker is a swing of ONE raw unit and the observed swing is 74,373, so a test that merely asked whether it moves would have confirmed the wrong phenomenon*** — the assertion checks the swing is **large**. ⚠ **F25: the task asked for a bound and found the existing one wrong by a factor of ten.** `LayerMagnitudeIsBounded` permits a source of ~327,000; `Desirability` lifted the read value with `Fixed.FromInt`, which throws above 32,767 — ***the composition threw on a world the invariant calls legal***. Fixed by **removing a conversion rather than widening a type**: pollution is a count, the weight is a ratio, the product is already Q16.16. ⚠ **And the first version of the test did not bite** — one Cell at the ceiling contributes only `source / gain` ≈ 4,041 to its own read value, so it takes a full kernel support of them; ***a bound stated per Cell is not a bound on what a Cell reads***. ⚠ **The arguable half is filed, not decided**: *should* land value swing with rush hour? [`0002`](0002-open-questions.md) §C, trigger milestone 13, and **not to be closed by tuning `land_value_tau`**. ✅ **TASK 6 IS SHIPPED, 2026-08-20 — `--land-value`, three grids**: the target, the lag, and the gap, plus what the field did over the run. ⚠ **F26: it is not `--layer` grown, and the task's own sentence is why.** `--layer` hand-places sources; land value is a **history**, so its picture needs a city that has been running — and ***a lag is not a property of a value, it is a property of a pair***, so one grid was never going to carry the claim. ⚠ **F27: `--layer`'s refusal carried a stale sentence, exactly `adr/0093`'s shape** — *no session can place a source until Rules exist*, true when written, false since slice 7, **wrong about the trigger**. Corrected in place. ⚠ **F28: the dump prints the HOUR and it is the most useful line in the header** — at a round multiple of 2,048 Ticks the run lands at midnight, every Segment is empty, and the picture is of a **one-term composition wearing a two-term formula**, which would read as working. The demonstration is **21163** Ticks, not 20480, and a test pins `THE HOUR IS 08:00`. ***A picture of an instantaneous quantity that does not say which instant is not a picture of the city.*** **And the mode carries no ordinal** — the board already records two branches each shipping a tenth. ***Count the enum.*** **Read**: target peak 29.00, lag 29.19, gap 1.07, so the lag runs about **4%** behind. ✅ **TASK 7 IS SHIPPED — THE MILESTONE'S ACCEPTANCE RUN, 2026-08-20.** 100,000 Ticks on `fouled.toml`, 49 Days read. **The level settles** (mean ≈ −168,000 Q16.16 from Day 13, drifting under 10% between halves), **the flow does not trend** (11,135,893 on the fill down to a 200k–1.7M band), **the collection does not grow** (163 Cell rows, constant). 🔴 ⚠ **F29: THE FLOOR REFUTED ONE OF ITS OWN READINGS, AND THAT IS WHAT A FLOOR IS FOR.** Readings 1 and 2 pass; **reading 3 fails** — pollution and noise are rank-concordant at **86–100%** on every readable Day, so ***no ratio between `w₂` and `w₃` is identifiable in this world at all***. `adr/0125` said *it refutes and never confirms*, and it did. ⚠ **The cause is sharper than §D1 predicted**: not co-varying because both scale with population — ***the same source***, since the emitting kind is `dwelling` and dwellings also generate the commute. **The simplification that made the floor reachable is what makes its third reading unreadable**, and the hand-authored world §D1 named is now owed for a reason a run produced. ⚠ **F30: the instrument had to be fixed twice.** A reading every 2,048 Ticks lands at the same hour, and the obvious phase lands at **23:59** where the noise term is zero everywhere — ***a periodic reading of an instantaneous quantity samples ONE HOUR of the day, and which hour is a choice somebody has to make on purpose.*** At 08:00 it still read zero on ten of forty-nine Days; the shipped 06:00–12:00 band cuts it to **one**, asserted as a quiet morning rather than tuned away. ⚠ **The flatness is asserted on the FLOW and the exempt axis is named**: the level *within* a Day is exempt because the target moves with the Day; the flow *across* Days is not. ⚠ **A Tick-budget row is owed to [`0013`](0013-tick-budget.md) when task 4 lands**, not now. **One decision remains — 6, the ±1 oscillation — and it is *measurable then arguable*, so it wants a run rather than a sitting.**
...
flanking Legs cost something, the occupancy is checked against the Citizens holding it**, `--parking`
is the tenth runner mode, and **the golden baseline now contains a city that drives**. ⚠ **Task 8's
three ratifications came out one held, one refuted and one amended**, which is the milestone's largest
finding and is not what a ratifying run is usually expected to produce: `adr/0009`'s
*scarcity is a gradient* **holds** under the one reading that could have refuted it, `[parking]
shed_keeps = 24` is **refuted** in the *too small* direction [`0002`](0002-open-questions.md) §D1
named, and the shed radius **stands with its ratifying clause amended** — *occupancy approaching 1* is
unreachable, because a Trip that cannot park is refused and a refused Trip is a car that needs no
space. ***A shed cannot be filled by shrinking it.***
...
finding is not about conservation**: pin one draw and 188 of 360 Households sit at exactly zero for
the whole run while **every conservation reading stays byte-identical**. ***A conserved economy tells
you nothing about who holds it***, which is the argument for `01 §5.1`'s money supply and treasury
being two reported figures rather than one.

⚠ **The sentence above this one said *one milestone is in flight, and it is 7* while the table below
said milestone 10 was in flight too, and both were on this page.** `plans/0012` **Cause 1** at its
shortest range yet — not two documents but two sections, and the board is the document whose whole job
is to be the single view. ***A view is only one copy if it is written in one place***, and a headline
paragraph beside a table is two. Struck 2026-08-19 by the milestone closing, which is the wrong repair
arriving by luck: it would have stayed wrong for as long as 10 ran.
...
~~**After 5c**, milestones **7** and **8** are the only Phase 2 rows with no gate on them.~~ **Both are
shipped, and the sentence was wrong about the *set* rather than about either row** — **9** had no gate on it
either, and no row anywhere to say so, for the whole time this sentence was on the page. ***A list of the ungated
rows is only a list if somebody enumerated the rows***, which is the disqualifier the per-milestone table
below was supposed to be and was not. Struck 2026-08-20. **21**, **22**
...
|---|---|---|---|---|
| **1** | code | **`06` milestone 9 — the land value target and the composed Layers. 🟡 SCOPED 2026-08-20, [`0034`](0034-the-land-value-target-and-the-composed-layers.md), unstarted — and it had never had a board row at all.** It is the next unstarted position in `06`'s sequence — 6, 7, 8 and 10 are shipped and ⚠ **10 shipped ahead of 9**, on its own branch, so what the sequence is missing is a row in the **middle** and not the next one off the end. ⚠ **No document names a gate on it**: it is absent from the per-milestone table in *Blocked* below, from [`0002`](0002-open-questions.md) and from every plan. That is an **unassessed** state and not a cleared one — ***unassessed is not the same as unblocked*** is milestone 19's own finding — so the grep taken 2026-08-20 is what makes it the second rather than the first, and it is recorded in *Blocked* rather than left as an absence. ✅ **Its named risk was checked rather than quoted**: `MapLayers.SetLandValueTarget` (`src/Borough.Core/Space/MapLayers.cs:319`) is called from `LayerFieldsTests`, `LayerQueryTests` and `FactorioTests` and from **nothing in `src/`**, so `06`'s *six decisions read a field nothing sets* ([`02 §2.4`](../docs/02-simulation-model.md)) still holds. The Layer, its drift and its cadence all shipped in slice **3c**, so this is a **producer for a structure already in the tree** rather than a subsystem, and the Fertility and Desirability holes beside it close in the same row ([`adr/0022`](../docs/adr/0022-land-is-a-stock-the-city-spends.md), [`adr/0034`](../docs/adr/0034-fields-are-sorted-by-source-geometry.md)) | [`0034`](0034-the-land-value-target-and-the-composed-layers.md), [`06`](../docs/06-roadmap.md) → milestone 9 | It is the ungated row the board never carried. ⚠ **Scoping found the milestone is two milestones and `06` states one** — it names four inventory rows, of which **Fertility, Sealing's decay and Woodland are blocked on the world generator at 24** and the water graph is a milestone of its own, so what is left is noise, a three-term composition, two weights and a caller. ⚠ **Six decisions were owed and five came before any task that composes**, because [`0034`](0034-the-land-value-target-and-the-composed-layers.md) **F3** found `w₁` is not a weight: land value drifts toward a target whose first term is land value, which makes `w₁` a gain of `1/(1 − w₁)` on the other four and the field divergent at `w₁ ≥ 1`, and ***a formula nobody evaluates is not checked by being read***. ✅ **Decision 1 is settled and `w₁` is deleted** ([`adr/0122`](../docs/adr/0122-land-value-is-not-a-term-in-its-own-target-and-a-term-on-both-sides-of-a-lag-is-a-gain.md)) — **five remain, four before the composing task**, and the surviving weights to author are **two** rather than five |
...
| **7** | **Parking** — ✅ **DONE 2026-08-19**, all eight tasks, all four decisions settled before task 1 | [`0031`](0031-parking.md) | ***A shed cannot be filled by shrinking it.*** Task 8's three ratifications came out one held, one refuted and one amended, and the amended one is the shed radius: *occupancy approaching 1* is unreachable, because a Trip that cannot park is refused and a refused Trip is a car that needs no space |
...
| **10** | **Conserved Money and the treasury** — ✅ **DONE 2026-08-19**, every task, scoped and closed in two days | [`0033`](0033-conserved-money-and-the-treasury.md) | ***A conserved economy tells you nothing about who holds it.*** The acceptance run asserts the money sum as an exact equality at all 48 readings — the only exact conservation assertion this project will ever have — and then shows that a city with 188 of its 360 Households destitute produces **byte-identical** readings to a healthy one. ⚠ Also ***a writer arriving does not discharge a condition about what a reader can tell apart***, and ***the line that keeps two quantities from being read as one belongs in the reader, not in the producer*** |
...
| **9** — the land value target and the composed Layers | ⚠ **NOT assessed until 2026-08-20, and it had no row anywhere until then** — not here, not in *Do these next*, not in [`0002`](0002-open-questions.md). The assessment is a grep and it found **no document naming a gate on it**, which makes it ungated as far as anything written down goes; scoping is what would find one. ***A milestone with no row is not a milestone with no gate***, and this one went unlisted through the whole of 6, 7, 8 and 10 — including 10, which is **behind it in `06`'s order** and shipped anyway. The per-milestone table listing the cleared ones is what should have made it visible and did not, because a table of the milestones somebody thought about is not a table of the milestones | **nothing named.** ⚠ *Unassessed is not the same as unblocked* — milestone 19's own finding, and the reason this row says *nothing named* rather than *cleared* |
