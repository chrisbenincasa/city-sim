...
|---|---|---|---|---|
| **1** | code | 🔴 **Milestone 26 — the purchase, and its CONTENT GATE IS DISCHARGED.** Session W settled what raises a Provider, what declines one and the land-use split ([`adr/0163`](../docs/adr/0163-demand-for-a-shop-is-elapsed-unserved-need-in-reach-and-building-claims-it.md)–[`adr/0166`](../docs/adr/0166-a-business-runs-rules-and-its-rules-live-as-long-as-its-tenancy.md)). ⚠ **It did not get smaller: `adr/0166` widens the Rule Instance's subject to a Business and 26's decline half depends on it** | [`0044`](0044-the-purchase-and-the-provider-that-answers-it.md) | **Decomposed 2026-08-25 into ten tasks, and the inherited order was backwards** — `adr/0166` is a precondition of the PURCHASE and not only of the decline half. 🟢 **TASKS 1–4 OF TEN LANDED**: a Business runs Rules ([`adr/0166`](../docs/adr/0166-a-business-runs-rules-and-its-rules-live-as-long-as-its-tenancy.md)), the land-use split ([`adr/0165`](../docs/adr/0165-a-zone-permits-building-kinds-so-the-split-is-exclusive-and-the-instrument-paints-it.md)), `rulesets/provisioned.toml`, and 2026-08-26 **`Scope.Pool` RESOLVES** — the milestone's own risk, with open decisions 2 and 3 settled as [`adr/0167`](../docs/adr/0167-a-purchase-picks-its-seller-by-a-draw-and-waits-on-the-market-rather-than-on-a-shop.md). 🔴 ⚠ **Two things task 4 leaves live and neither is a task**: the purchase is [`0013`](0013-tick-budget.md)'s only super-linear consumer with `adr/0139`'s named fallback already spent — re-opened in [`0002`](0002-open-questions.md) **§B** — and `adr/0139` was found contradicting itself about where a price sits, filed in [`0012`](0012-corpus-audit.md). 🟢 **TASK 5 (Evidence) LANDED 2026-08-26 and found a DEFECT IN TASK 4**: a buyer blocked on money never slept, because `Requirement` walks a Rule's terms and `adr/0050` gives the payment none — so `Stop`'s own drain woke every buyer it had just parked, and it spun on its rate for ever while reporting itself armed. **323,438 money-blamed stops, 0 on the wait list; 61 after the fix.** ⚠ **Only an instrument could have found it** — `adr/0137` predicted the outcome and named the wrong mechanism, so reading the code confirmed the wrong thing. **Findings, decisions and per-task detail: [`0044`](0044-the-purchase-and-the-provider-that-answers-it.md) F1–F33.**
