...
**The next code row is `06` milestone **11** — Hinterlands and arrival through the gate, and it is
UNSCOPED.** ⚠ **Its gate state is *unassessed*, not cleared.** The per-milestone table below says *NOT
cleared, and never scheduled* and names nothing that holds it; session **J** cleared `05 §7`'s
save-format half and the Outside *layout* against a milestone that was **8**. That is milestone 9's own
finding arriving on the very next row — ***a milestone nobody assessed is not a milestone with no
gate*** — so **the first act of scoping it is the assessment, not the plan**.
...
run. [`0034`](0034-the-land-value-target-and-the-composed-layers.md) owns the record and
[`0000a`](0000a-board-archive.md) indexes it; **its thirty findings live there and are not summarised
here**. What ships: `MapLayers.SetLandValueTargets` runs from phase 5 on the land value cadence, so
`06`'s *six decisions read a field nothing sets* no longer holds; `Desirability` composes **two** terms,
`− w₂·pollution − w₃·noise`; six records, [`adr/0122`](../docs/adr/0122-land-value-is-not-a-term-in-its-own-target-and-a-term-on-both-sides-of-a-lag-is-a-gain.md)–[`adr/0127`](../docs/adr/0127-the-land-value-target-never-stops-moving-so-the-question-is-what-the-lag-rests-around.md).
⚠ **What ships is bounded above by zero** — amenity is the only positive term and it belongs to **15**.

⚠ **Three things left that milestone and are still live, and they are the reason this paragraph is not
one line.**

- 🔴 **The producer was built, correct and unobservable.** Nothing in the build creates a Cell row but a
  pollution emission and **no shipped Ruleset emitted one**, so the whole mechanism ran over an empty
  table, no baseline moved, and decision 5's floor was unreachable four hours after it was written.
  Discharged by authoring [`rulesets/fouled.toml`](../rulesets/fouled.toml), the ninth Ruleset, rather
  than by weakening the reading. ***A mechanism with no world is not a shipped mechanism***, and
  `adr/0052` still does not ask whether the state a ratifier names can occur.
- 🔴 **The floor refuted one of its own three readings, and that is what a floor is for.** Pollution and
  noise are rank-concordant across Cells at **86 to 100 percent** on every readable Day, so ***no ratio
  between `w₂` and `w₃` is identifiable in this world at all*** — not because both scale with
  population but because in `fouled.toml` they have **the same source**, the emitting kind being
  `dwelling`. The **hand-authored world** [`0002`](0002-open-questions.md) §D1 already named is owed,
  now for a reason a run produced. Both weights stand **unratified from 9 until 13**.
- ⚠ **A defect is filed unfixed, routed rather than worked around** (`adr/0073`):
  `Invariant.WaiterIsBlockedByTheBinItNames` panics on **every** Ruleset for a headless run shorter than
  **64 Ticks**, and ***nothing in the suite runs the runner that short***, which is why it has stood
  unseen. [`0003`](0003-build-plan.md) hash-moving queue **item 11**.

⚠ **The closure found two more things, and both are about numbering and refusal rather than about land
value.** The defect above was **filed as queue *item 6*, and 6 was taken** — by `SyntheticCity` paving
what it populates, shipped 2026-08-13 — because the queue's **table stops at 6 and its items 7 to 10
live in the prose below it**, so a number taken by reading the table was checked against part of the
set. ***A ledger whose rows and whose prose hold entries of the same kind is one ledger for the purpose
of numbering***; renumbered to **11**. And **item 12** was filed on the way: `RulesetLoader.TryLayer`
resolves all three Layer names, so a Rule may name `layer = "land-value"` or `"sealing"` in its outputs,
**load clean, and panic the first time it fires** — `RuleEngine.Emit` refuses correctly and refuses
**one Tick too late**, where [`adr/0048`](../docs/adr/0048-the-ruleset-is-validated-where-it-is-parsed-and-only-integers-and-strings-cross-into-the-core.md)
puts the check at the parse. ***A refusal in the engine is a refusal the designer meets as a crash.***

⚠ **And one arguable question was filed rather than decided**: *should* land value swing with rush hour?
The lag is the only part of the composition with memory and volume is instantaneous, so the target
oscillates at the Day's own period ([`adr/0127`](../docs/adr/0127-the-land-value-target-never-stops-moving-so-the-question-is-what-the-lag-rests-around.md)).
[`0002`](0002-open-questions.md) §C, trigger milestone **13**, and **not to be closed by tuning
`land_value_tau`** — damping a daily cycle with a lag constant hides a modelling choice behind a number.
...
**Where it is.** **Phase 2, between milestones, and the head of the sequence is clear for the first
time in a month.** 5c — statistical resolution and the travel-time matrix — closed 2026-08-16 on all
...
dotnet run --project src/Borough.Headless -- --land-value --ruleset rulesets/fouled.toml  --citizens 4000 --ticks 21163   # 08:00, and the hour is why
...
|---|---|---|---|---|
| **1** | code | **`06` milestone 11 — Hinterlands and arrival through the gate. ⬜ UNSCOPED, no plan document, and ⚠ its gate is *unassessed* rather than cleared.** It is the next unstarted position in `06`'s sequence now that **9** is closed — 6, 7, 8, 9 and 10 are all shipped, so for the first time in a month what the sequence is missing is the row off the **end** and not one in the middle. ⚠ **The *Blocked* table below says *NOT cleared, and never scheduled* and names nothing that holds it**: session **J** cleared `05 §7`'s save-format half and the Outside *layout*, and the milestone those clearances were recorded against was **Save/load**, which is now 8. ***Unassessed is not the same as unblocked*** — milestone 19's finding, which milestone 9 then walked into in full — so **the first act is the assessment and the second is the scope**. ⚠ **It is money's only source and sink and the anchor every price, wage and rent hangs off**, which is why the 2026-08-18 economic reorder moved it from **14** to **11**: nothing economic can precede it, and **13, the price surface — the trigger on both weights milestone 9 left unratified — is behind it** | [`06`](../docs/06-roadmap.md) → milestone 11; no plan document yet | It is the ungated-as-far-as-anything-says row at the head of the sequence, and the two §D1 debts milestone 9 filed are triggered by the milestone behind it |
...
| **5** | code | **[`0003`](0003-build-plan.md) hash-moving queue item 11 — filed unfixed 2026-08-20 by milestone 9 task 7, and ⚠ **filed under a number that was already taken; renumbered 2026-08-20**.** `Invariant.WaiterIsBlockedByTheBinItNames` panics on **every** Ruleset for a headless run shorter than **64 Ticks** — red at 4, 8, 16 and 32, clean at 64 and 128, the same two rows every time on three different Rulesets. So it is a property of **how young the world is**, not of the content | [`0003`](0003-build-plan.md) → *The hash-moving queue* | ⚠ ***Nothing in the suite runs the headless runner for under 64 Ticks***, which is why a panic on every shipped Ruleset has stood unseen. Either a waiter is legitimately parked in a state the re-derivation does not reproduce before the first arming stagger completes, or the invariant is right and something arms wrong — and **which of those it is decides whether this moves a hash** |
| **6** | tidy | **Delete `spikes/S4.Kernels/`** — S4's task 11, open since the spike closed and gated on nothing | [`0004`](0004-s4-kernel-benchmark.md) | The same discipline as row 4: a deletion that size is taken deliberately, not as a consequence of a green suite |
...
| **9** | **The land value target and the composed Layers** — ✅ **DONE 2026-08-20**, all eight tasks, all six decisions, scoped and closed inside one day | [`0034`](0034-the-land-value-target-and-the-composed-layers.md) | ***A mechanism with no world is not a shipped mechanism.*** The producer was built, correct and **unobservable** — nothing creates a Cell row but a pollution emission and no shipped Ruleset emitted one — and the floor written to ratify its weights then **refuted one of its own readings**, which is what a floor is for |
...
| **9** — the land value target and the composed Layers | ✅ **CLOSED 2026-08-20** — scoped, built and closed inside one day, [`0034`](0034-the-land-value-target-and-the-composed-layers.md). *(Original row: **NOT assessed until 2026-08-20, and it had no row anywhere until then** — not here, not in *Do these next*, not in [`0002`](0002-open-questions.md). The assessment was a `grep` and it found no document naming a gate, which makes it ungated as far as anything written down goes.)* ⚠ **Scoping is what would have found one, and scoping found no gate either — it found the milestone was two milestones**, and [`adr/0124`](../docs/adr/0124-terrain-suitability-is-baked-at-world-creation-and-the-layer-holes-that-need-it-move-to-milestone-24.md) moved the half that needed terrain to **24**. ***A milestone with no row is not a milestone with no gate***, and this one went unlisted through the whole of 6, 7, 8 and 10 — including 10, which is **behind it in `06`'s order** and shipped anyway. The per-milestone table listing the cleared ones is what should have made it visible and did not, because a table of the milestones somebody thought about is not a table of the milestones | — |
| **11** — the Outside *(milestone **14** from 2026-08-16 until the 2026-08-18 economic reorder)* | ⚠ **NOT cleared, and never scheduled — and as of 2026-08-20 it is *Do these next* row 1, so the assessment is now owed rather than merely absent.** ***Unassessed is not the same as unblocked***, and milestone 9 is the worked example: it read as ungated by `grep`, and what scoping then found was not a gate but that the milestone was two milestones. **The first act on this row is the assessment.** Session **J** cleared `05 §7`'s **save-format** half and the Outside *layout*; the milestone it was recorded against was **Save/load** (old 10, now **8**), and the Outside itself had a row in `06`'s inventory the whole time. It is now milestone **11** — it was **14**, and moved on 2026-08-18 because it is money's only source and sink **and** the anchor every price, wage and rent hangs off, so nothing economic can precede it. `plans/0012` *Cause 1* | — |
