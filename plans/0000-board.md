...
neither, and `05 §6`'s threading policy is an *argument* — and that last one names *"milestones 10 and
...
feedback term. ✅ **This discharges K2's third gate**; two remain.
...
defect, filed unfixed because its two repairs are a design question). Phase 2 has shipped **5a** (the
...
| **6** | tidy | **Delete `spikes/S4.Kernels/`** — S4's task 11, open since the spike closed and gated on nothing | [`0004`](0004-s4-kernel-benchmark.md) | The same discipline as row 3: a deletion that size is taken deliberately, not as a consequence of a green suite |
...
| **S2** | **Retired the risk it existed for** — *pathfinding is slow at a load nobody measured, on a map already committed to*. R0–R8 done, R7's tail closed 2026-08-11. The one act left is deleting the harness, which is row 3 of *Do these next* and is **blocked** | [`0010`](0010-s2-routing.md), [`spike-results`](../docs/spike-results.md) → *S2* |
...
|---|---|---|---|
| **E** | `adr/0005` + `adr/0007` — fidelity | One session, not two: `0007` moved Fidelity from person to **place**, and `0005`'s tiers are what it moved. ⚠ **7a needs the `0007` half only** — `adr/0005` says in its own last line that its fidelity half was superseded | milestones **7a**, **7b** |
| **G** | `adr/0016` — the lane is the entity | Carries the order-of-magnitude claim the whole microscopic tier rests on. ⚠ **Partly discharged by measurement**, and `0002` §F marks it a *type-before-you-grill* suspect — a session **may not close** whatever part turns out measurable | milestone **6** |
| **K2** | `06`'s Phase 2 **ordering** only | Re-derive the sequence and place the ~~eighteen~~ **thirty-nine mechanisms `06` lists as having no milestone**, plus the **five obligations no milestone can hold**. **Last** — E and G move what it sequences, *and* it wants 5c's routing load. ✅ **The owed sweep RAN 2026-08-15 and that gate is discharged** — twenty-one rows came back and `06` carries them. ⚠ **What the sweep found matters more to K2 than the rows do**: the inventory is a **flat list** and the mechanisms are a **dependency graph** — perhaps five roots and the rest consequences, with the **Provider List** and the **land value Layer** deepest — so ***a re-derivation reading that table top to bottom would sequence it wrongly***. ⚠ **And the sweep is a snapshot, not an instrument**: nothing generates the list, so K2 treats it as *the best available* and never as *complete* | Planning Phase 2 at all |
...
- ✅ **RUN 2026-08-14 — re-check every 🔴-blocked debt for a gate whose stated reason covers only part of
  what it blocks.** Three clean positives (**milestone 6**, **Phase 3 / session L**, **K2**), one partial
  (**7a**). ⚠ **Its other half found a different failure**: **stale gates whose stated reason had gone
...
| **`03 §3.8`** | **Retyped by session D task 3: *measurable*, not a decision.** Run `03 §5.1`'s **spillback** scenario with force-promotion disabled and see whether the upstream Segment blocks. **No session may close it** | milestone 6's acceptance suite |
...
| **Planning Phase 2 at all** | `06`'s ordering must be re-derived. ⚠ **The S0 half is struck** — S0a and S0b have both run, so it is **one gate, not two**. ⚠ **And Phase 2 has been under way throughout**, so the sentence was blocking something already happening: *a block nobody enforces is a document disagreeing with the build* | session **K2**, and it has **three** gates rather than the one this cell used to name. **(1)** the **routing load** in situ, which milestone **5c** produces; **(2)** sessions **E** and **G**, because *Open tracks* books K2 **last** on the ground that they move what it sequences — a gate this cell did not know about, stated in the other row all along; ~~**(3)** ⚠ **NEW 2026-08-15** — its own input is incomplete, since `06`'s unplaced-mechanism list excludes anything specified outside an ADR.~~ ✅ **DISCHARGED THE SAME DAY** by a three-way sweep of the ADRs, `CONTEXT.md`, `00`–`05` and `plans/`; `06` gained twenty-one rows and a table of five obligations no milestone can hold. **Two gates remain, not three.** ***A row whose gate is stated in two places will be believed in whichever place is read***, and this is the same one-row-two-gates defect the blanket row above was split five times to remove |
