...
ADRs, marked 🔴/🟡/🟢, with ⚫ where a supporting claim has been measured false. **§A–§E are questions
...
**The long-form board, as it stood before the 2026-08-10 restructure, is frozen at the bottom of this
file.** It is history and is not maintained. Where it disagrees with anything above it, the sections
above are right.
...
row for the first time**; sessions A, B, C, M, D, eight and nine are closed.
...
| **8** | code | ⬅ **NEXT. 5b task 5 — Phase 4, behind `CommandKind.Trip`.** Phase 4 has been an empty method since the Tick was written, and the three Movement tables built by tasks 1–3 are **not in `World._tables`** — outside the State Hash, never constructed by anything. This wires them: the Traveller advancing its cursor, volume attribution on entry and exit ([`adr/0041`](../docs/adr/0041-volume-is-attributed-by-the-traveller-not-the-district-pair.md)), Fate resolution, and the Census sink `adr/0006` requires — `TripTable`'s own doc comment flags that *"this table needs a sink, and Trips are transient does not supply one"*. **A Trip enters through the Input Log** on `CommandKind.Populate`'s precedent, so replay reproduces it by construction and nothing in the simulation invents one ([`adr/0080`](../docs/adr/0080-phase-4-does-not-wait-on-a-trip-generator-and-a-trip-is-entered-by-command.md)). **Baselines re-record**, because the tables enter the hash. ⚠ **Task 5's volume is nearly vacuous by construction and that is expected**: `adr/0041` increments on **vehicular** Legs only, and 5b has none | [`0021`](0021-trips-legs-and-the-pedestrian-layer.md) | **It closes milestone 5b**, whose remit shrank on 2026-08-12 rather than whose blockers cleared. Tasks 4, 6 and 8 left for 5b-bis because each measures a **distribution** and no generator exists |
| **9** | code | **THEN: `06` milestone 5b-bis — jobs, the commute, and the first Trip generator.** New milestone, created 2026-08-12 by the §A sitting ([`adr/0081`](../docs/adr/0081-the-commute-is-the-first-trip-generator-and-a-job-is-taken-by-satisficing-on-distance.md)). **Its first task is not the generator**: there is **no query from a place to the Buildings near it** anywhere in `Borough.Core`, and all three candidate generators needed one. Then a `[[building]] jobs` key, a job-assignment pass replacing `SyntheticCity`'s `(i * 7) % buildings` stride on `CitizenTable.Workplace`, the commute generator itself, and 5b's transferred tasks 4, 6 and 8. **The `[trips]` Ruleset table is created here**, and the **Commute Budget** and the **crossing cost** finally land — both hash-bearing, both unset, and **neither may be chosen by argument** (`adr/0052`) | needs a plan document | **Its named risk**: that every number 5b was to produce is taken against a fabricated origin-destination draw and lands in [`0013`](0013-tick-budget.md) and [`0002`](0002-open-questions.md) as measured fact. **S2 R4 already ran that experiment** — 18.52% detour on a uniform draw against **128.82%** on a local one, *"a different city"* |
...
**So the code track is what is left, and it now holds two rows rather than one.** 5a-bis shipped
2026-08-11 (row 6), and **5b's remit shrank on 2026-08-12** rather than its blockers clearing: the §A
sitting found that **no milestone in Phase 2 ever produced a Trip destination**, so tasks 4, 6 and 8
left for a new milestone. **Row 8 is what is left of 5b — Phase 4, behind a command — and row 9 is
5b-bis**, which builds the generator those tasks were waiting for.
...
# Archive — the board as it stood before the 2026-08-10 restructure

**Everything below this line is history and is not maintained.** It is the long-form board, moved
verbatim and unedited, and it is kept for the same reason [`0002`](0002-open-questions.md) keeps its
session record: **the reasoning is worth more than the conclusions on their own**, and several entries
record *which half* of a withdrawn claim was wrong, which a bare strike destroys.

**It is not a status surface.** Where it disagrees with the sections above, the sections above are
right. Where it disagrees with [`0003`](0003-build-plan.md), [`0002`](0002-open-questions.md) or an
ADR, those are right.

**Why it exists.** The board is a *view* and had accumulated its own history inside its live cells —
42 lines carrying strikethrough, single table cells running to 400 words holding current state and
withdrawn reasoning together. That is the right convention for an ADR, where the reasoning is the
artefact, and the wrong one for a view, whose only job is to be scannable. This is `adr/0042`'s rule
applied one level further: **a view cites, a record owns.**

**How to retire it.** Each block below is either restated in the sections above or owned by a slice
plan, a spike plan, `docs/spike-results.md` or an ADR. Delete a block once you have satisfied yourself
of that — and note that `git` holds it regardless, so the cautious reading is that this archive is
already redundant and is kept only until somebody has checked.
...
| **1c** | **code** | **Slice 10 task 11 — the Zone Rule sample is the wrong unit. PLANNED, and it waits for slice 8** | [`0014`](0014-zone-rules-and-the-sweep-family.md) → *The correction S0b found*, and [`adr/0059`](../docs/adr/0059-a-zone-rules-sample-is-a-revisit-period-so-the-ruleset-states-a-duration.md) | **A closed slice shipped a defect and its own tripwire is what hid it.** `sample` is an absolute count of Lots per cycle, so the quantity the city feels — a fraction of the city per cycle — is inversely proportional to the city: a Lot is visited once per **0.12 Day at 1,000 Citizens and once per 117 Days at 1M**, and at target scale the shipped Ruleset builds **nothing**. **Not the create predicate** — `created` equals `vacant` exactly in every interval of every capture, so the Pool and the permission bit never declined a candidate. `adr/0059` settles it: author `revisit_ticks`, a **duration**, and derive `sample = ceil(Lots × interval ÷ revisit_ticks)` — `pollution_decay_ticks`'s shape against the Lot count instead of the cadence. **The default is `TICKS_PER_DAY` and therefore derived**, so `0002` §D **loses a row rather than gaining one**, which has not happened before. Five sub-tasks: the Ruleset key and its three refusals (a `sample` still on disk is refused **by name** — silence is how tuning stops taking effect), the derivation with a **ceiling** rather than a floor (flooring gives zero below 256 Lots and would delete the mechanism on every small world, the same defect with the opposite sign), retiring `ZoneSample.Draw`'s `O(sample²)` duplicate scan whose *"a sample is a handful"* premise is gone, restating the scratch buffer's `adr/0006` bound, and re-recording the three baselines. **Sequenced behind slice 8 deliberately**: it is hash-bearing and slice 8 task 10 re-records the same files. **Why it matters beyond the fix**: `adr/0052` was followed to the letter here — the ratifier was named in advance, ran, bounded the value at *54–78 Buildings of 121*, and passed — and **a ratifier that runs at one city size cannot catch a number that is absolute.** That is the case for a third question beside *measurable/arguable* and *who ratifies it*: **what does this number scale with?** |
...
**S2's findings have moved to [`0010`](0010-s2-routing.md) → *Findings that change a later round*** —
twenty-five entries, verbatim and unedited. They are instructions from one spike round to the next, so
their record is the spike's plan and not this view. **Nothing was dropped; the checkboxes travelled
with them.**

What stays below is the handful that are **not** S2's.
...
| ~~**Phase 2 milestones 5a–10**~~ **SPLIT 2026-08-11 — 5a is not blocked, and this row was hiding it.** Every session in the argument track points at something that *runs on* the graph rather than at the graph: D→5b/5c/6/7a, F→5b, G→6, H→8, I→5c, J→10, and **5a appears in no row**. Its named risk is retired in prose by `adr/0014` (uniform nodes and edges, the simulation never sees a spline), `adr/0040` and `adr/0047`. K2 does not cover it either: K2 re-derives Phase 2's **ordering**, and 5a is the one milestone whose position in that ordering cannot move, because 5b, 5c, 6, 7a, 7b and 8 all run on the Road Graph. **The remainder is the largest blocker in the corpus** — 13 named holes in `src/`, the whole of Tick phase 4, ~30 documented mechanisms, 23 ADRs and 21 rows of `0002`, headed by the `pool` scope, Trip generation and the line-source queries. Scoped in [`0020`](0020-the-road-graph.md) | 🔴 **for 5b–10 only** — `03 §5` and six research-written ADRs, plus S2 | sessions **D**–**J**, plus a spike |
