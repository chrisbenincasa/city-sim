...
| **`Heuristic.cs`'s hoist has no marker** | `adr/0073` requires a local workaround to name what it works around and where the finding went. `spikes/S2.Routing/Routing/Heuristic.cs` works around a defect **fixed on 2026-08-11**, and gives up ~2 parts in 10,000 of heuristic tightness to do it — so the hoist is now **buyable back** and nobody reading that file would know. Deferred only because another session is working in that harness | S2, or whoever closes the harness |
