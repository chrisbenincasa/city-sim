...
| **3** | spike | **S5 — the Lane kernel.** What `adr/0016`'s Lane model costs in **integer** arithmetic, and from it how many Microscopic Segments fit in 15.6 ms | [`0017`](0017-session-d-the-traffic-model.md) → *the spike this pass emitted*, [`spike-results`](../docs/spike-results.md) | Session D's typing pass opened it: the tier's affordability rests on **Eickhoff's ~400,000 cars on one core**, measured on a float engine, and [`0013`](0013-tick-budget.md) holds **no row for the Microscopic tier at all**. S4-class — arithmetic substrate only, no content, no Trip generation — so it **blocks on nothing and contends with nothing** |
