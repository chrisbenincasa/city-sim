...
🟡 **Two milestones are scoped and in flight, and they are 7 and 8.** **Milestone 7 — Parking**, scoped
2026-08-17 ([`0031`](0031-parking.md)), eight tasks, ungated by session **H** on 2026-08-12 — ✅ **all
four decisions settled and tasks 1 and 2 shipped 2026-08-18**, so **the supply exists, the reach is
stated and nothing parks in it yet**; task 3, the Parking Shed, is next. **Milestone 8 — Save/load**, scoped the same evening (`plans/0030-save-load.md`), nine
tasks, ungated by session **K** — ⚠ **and it is on an unmerged branch in a worktree**
(`milestone-8-save-load`), **tasks 1 to 3 shipped plus `adr/0110` *(scratch is a third Disposition)***.
⚠ **Everything this board says about 8 is read off that branch's commit subjects and is second-hand
until it merges** — [`adr/0093`](../docs/adr/0093-a-description-of-the-build-is-where-to-look-and-never-what-you-found.md)
governs a commit subject exactly as it governs a doc-comment, and *a description of the build is where
to look and never what you found*. Its own board row and `0012` filings live on that branch.

⚠ **This board cannot link to either of them, and that is the finding rather than a formatting
problem.** `plans/0030-save-load.md` and `adr/0110` exist only on that branch, so
[`0012`](0012-corpus-audit.md) **check 8** — every relative link resolves — fails on a citation that is
perfectly correct and merely early. ***A cross-reference is a claim about the tree you are in***, so
work in a worktree is unciteable from `main` until it lands, and the only honest form is a bare name.
Both become links when the branch merges.
