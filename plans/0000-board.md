...
✅ **DECISION 5 SETTLED — Upkeep does NOT ship at 12, and it is UNPLACED rather than moved**
...
`Scope.Pool` resolvable from a Segment"*, and 12's scope is now settled with **no Rule attached to a
Segment** — so the placement is known wrong **before the milestone starts**. ***A trigger evaluable
...
is the same move a third time***, and 21's own position is marked provisional. **Three blockers landing
at three times**: the construction-cost *quantity* needs the Lane (**21**); *design life* needs a **mature
city**; and 🔴 **the actor — a Rule whose subject is not its payer — has NO MILESTONE AT ALL**, which is
what makes this an unplacing. ⚠ **That shape should be watched for a second claimant**: a Policy scoped to
a **Ward** wants the same thing, and ***a shape two mechanisms want is a shape somebody should design
deliberately rather than one that waits for whoever needs it first.***

✅ **Two of `adr/0117`'s four grounds moved, and both moved because of 12's OTHER decisions.** Ground 1 is
fully discharged. **Ground 2's *money* half is discharged** — it read *"no Ruleset key anywhere authors a
cost of anything"*, and after `adr/0135` none needs to: `adr/0035` makes infrastructure money *"a
transfer, not a sink"*, so construction buys Materials at the market price. ***The blocker was an
authoring gap and the answer was a market, not a key.*** ✅ **And ground 3 is SETTLED** — `adr/0035` §3's
*"the only authored number is a duration"* makes *money or Materials* a false binary, so a designer
authors the **design life** and Upkeep is a **purchase**. ***A ground answerable from a document already
cited had been carried three ADRs on the strength of the company it kept.***
...
**This is a view, not a source.** [`06`](../docs/06-roadmap.md) owns the **sequence** and the risk each
milestone retires, [`0003`](0003-build-plan.md) owns **what is done and what gates it** — both ledgers
and both gate boards, Phase 0 through Phase 2 — [`0002`](0002-open-questions.md) owns **every open
question**, `docs/adr/` owns the decisions, [`0013`](0013-tick-budget.md) owns what a Tick costs. When
they disagree with this file, **they win**. Update this file whenever a task lands.

**The three documents, and the one question each answers.** ***Why in this order*** is `06`'s;
***what is done*** is `0003`'s; ***what is next*** is this file's, and it is nothing but an index over
the other two. ⚠ **`0003` covered only Phase 0 and Phase 1 until 2026-08-22**, so for eleven shipped
Phase 2 milestones there was no *what is done* layer at all and this file grew one — a 551-line *What
is next*, the third inflation and the first with a structural cause. See
...
Each row there names the gate, links the plan document that owns the tasks and the findings, and
states what is done.

⚠ **A copy of that table lived here until 2026-08-22 and had already drifted** — it stopped at
milestone 10 while 11 and 12 stood, which is `0012` *Cause 1* on the axis this file is most exposed
to. ***A view that keeps its own copy of the source is no longer a view.***
...
from them as a statement about a congested city. Stated in full by
[`0010`](0010-s2-routing.md):1016 and [`spike-results`](../docs/spike-results.md), which own it.
...
Phase 1's is its *gate board*, Phase 2's is the *Gate* column of its Phase 2 ledger. Neither has a red
row.

⚠ **A per-milestone gate table lived here until 2026-08-22**, because `0003` had declined Phase 2 and
this file said so in as many words: *"everything `0003` does not own."* It owns it now. The history of
that table — a blanket row narrowed five times, then a split that silently dropped three milestones —
is in [`0000a`](0000a-board-archive.md). ***A blanket row is a status whose granularity is coarser
than the claims it covers.***

| | Blocked on | Which is |
|---|---|---|
| **Phase 3** | 🔴 **a presentation design that does not exist** | session **L**. ⚠ **S1 and S3 are themselves ungated** ([`0003`](0003-build-plan.md) gives both `Gate: none`), so the head of the chain is runnable and what actually stops it is that **Track B has never been stood up** |
