...
| **23** | 🔴 ⚠ **A READOUT SHRINKING A RULE'S BAND LEAVES IT ASLEEP ON A BIN THAT NOW COVERS IT, AND NOTHING RE-DRAINS BECAUSE NOTHING WROTE THE BIN** — filed unfixed 2026-08-26 while closing queue item 22, **unmasked by [`adr/0171`](../docs/adr/0171-a-markets-level-is-what-its-sellers-hold-and-the-price-divides-by-the-sum-while-a-wake-spends-the-maximum.md) rather than caused by it**. Read off `rulesets/provisioned.toml` at 2,000 Citizens: at Tick 32,768 bin **1523** is a Household's larder holding **294**, and Rule Instance **725** is asleep on it needing **280**. ⚠ **THE BIN NEVER MOVED — THE REQUIREMENT CAME DOWN TO MEET IT**, traced tick by tick at **320 → 280 → 240**. `RuleEngine.Requirement` is derived live ([`adr/0063`](../docs/adr/0063-a-wait-list-wakes-on-the-bins-state-and-a-shortfall-is-derived-rather-than-stored.md)) as `floor × amount`, and `RuleEngine.Band` takes `floor` from `Readouts.Read` when a Rule's `apply` is derived — ***so the requirement is a function of the CITY and not of the Bin***, while every drain in the design hangs off a Bin write. **A readout that falls past a sleeping waiter's level is a wake nobody owes.** 🔴 **IT IS PRE-EXISTING AND THAT WAS CHECKED RATHER THAN ASSUMED**: a probe reaches it on `provisioned.toml` with `adr/0171` in and does **not** reach it at `2051d5f`, the commit before — ***a trajectory change is not a cause***, and the hole is reachable by anything that moves an occupancy. ⚠ **THE PLACEMENT IS THE WHOLE QUESTION AND IT IS ARGUABLE** ([`adr/0043`](../docs/adr/0043-a-claim-a-measurement-could-settle-must-not-be-settled-by-argument.md)): `adr/0171` re-drained at the one place a market's membership changes, and **a readout has no such place** — the set is open-ended and a readout is derived from whatever it reads, so *drain on every readout change* names no site. The candidates are a re-drain at each tenancy change (cheap, incomplete: a readout is not only occupancy), a periodic sweep of the wait lists (complete, and a whole-world walk `S0a` already priced at 95% of a run), or **making the band's inputs declare their waiters**. ***None of the three is obviously right and none should be picked in a market milestone.*** `MarketLongRunTests.The_only_missed_wake_left_is_the_one_a_readout_shrank` is an **allowlist of exactly this shape** — an ordinary Bin, a **positive** requirement — so a market row's Bin or a requirement of **zero** fails there and names queue item 22 coming back. `plans/0044` **F60** |
