...
>
> **⚠ AMENDED 2026-09-11 — Phase 3 now has a ledger here too, because it got built.** ~~Phase 3 is
> untouched and is still not planned here.~~ The shell, the five verbs, the information interface and
> the drive channel all shipped through [`0045`](0045-amnesty.md)'s queue while `06` went on saying
> Phase 3 could not be planned. **What is built is recorded below like anything else; the missing
> design is still missing and is `06`'s to place.**
...
| **18** | Needs and the coarse Day wheel — *wheel half only* | ✅ assessed 2026-08-21 — nothing names one | [`0036`](0036-the-coarse-day-wheel.md) | ✅ **Shipped.** `LifeStageWheel` buckets a countdown by Day and `Invariant.ACoarseRowIsDueOnTheDayItsBucketNames` holds it to that; `RuleEngine.SweepNeeds` carries the Needs half. It was scoped early because it is the repair for milestone **20**'s `adr/0011` gate |
...
| **13** | The price surface | none | — | 🟡 **Built in part, and no plan document owns the record.** `World.RepriceDistrictPools` clears a District Pool against `[market]`'s `decay_percent` and `move_cap_percent`, and `BlockPatterns.Ladder` carries the density rungs. ⚠ **Both arrived through amnesty rows rather than through this milestone**, so what `06` states as the row's risk has never been assessed against what shipped |
| **14** | The Provider List and the shopping occasion | none | [`0045a`](0045a-amnesty-history.md) row 20 | ✅ **Shipped as amnesty row 20**, PR #5. `ShoppingEngine.Step` runs the outing, picks a shop from the known-shop table and carries Goods home; `rulesets/shopping.toml` is the demonstration |
| **15** | Attended services, wages and Skill Tiers | none | [`0071`](0071-education-changes-a-working-life.md), [`0045a`](0045a-amnesty-history.md) rows 6, 10, 19 | 🟡 **Three of its four parts shipped, across four amnesty rows.** `ServiceEngine.Attend` and `CivicEngine.Step` carry Attended services, `WageEngine.Pay` the wages, and `SchoolingEngine` with `CitizenTable.SkillTier` the tiers. 🔴 **The Dispatched half is unbuilt** — `Incident` and `Crime` appear nowhere in `Borough.Core`, so `adr/0030`'s crime and police have no code |
| **16** | The residential choice model | none | [`0068`](0068-the-choice-model.md) | ✅ **Shipped as amnesty row 28, 2026-09-08.** `Choice` draws over sampled candidates with the stay-put alternative where the argmax was, and no recorded hash moved. The row kept the milestone identity and says so |
| **17** | Decline, Demolish and cleared land | none | [`0047`](0047-decline-demolish-and-cleared-land.md) | ✅ **Merged 2026-08-27.** Its failure-source residue is amnesty row 30, expanded and queued for scoping against [`0064`](0064-the-information-interface.md). ⚠ **This document dated two hash-queue items against that merge while this row still read *not started*** |
| **19** | Households, the Unplaced Pool and Departure | none | [`0045a`](0045a-amnesty-history.md) rows 16, 16a–16c | ✅ **Shipped.** `UnplacedTable` holds the Pool, `PlacementEngine.Place` drains it and `PlacementEngine.Reassess` returns a housed Household to it, and `World.Depart` is the sink |
| **20** | Life Stages and self-generation | ✅ cleared by **18** | [`0046`](0046-life-stages-and-a-self-generating-population.md) | ✅ **Shipped, all five tasks.** `LifeStageEngine.Sweep` advances and dissolves Households on the coarse wheel and spawns child Households into the Pool; `rulesets/aged.toml` is the demonstration |
| **21**–**23** | Lane-as-entity traffic, Stress-driven Fidelity, the rotating Audit | — | [`06`](../docs/06-roadmap.md) | 🔴 **Not started, and checked against the symbols rather than against this cell.** No `LaneTable` exists and `Fidelity` occurs in `Borough.Core` only inside a doc-comment. Still sequenced provisionally, pending sessions **E** and **G** |
...
⚠ **Phase 3 was built before it was planned, and this table is the first place that is written down.**
[`06`](../docs/06-roadmap.md) still says Phase 3 cannot be planned until spikes **S1** and **S3** have
run and a presentation design has been written and grilled. **None of those three has happened.** The
shell shipped anyway, through [`0045`](0045-amnesty.md)'s queue, and it is a source of *what is done*
like any other row here. ⚠ **This table records what exists; whether the absent design is now owed a
sitting is `06`'s question and not this document's.**

| Half | Plan | State |
|---|---|---|
| The eye and the shell | [`0045a`](0045a-amnesty-history.md) rows 11a, 11b | ✅ **Shipped.** `src/Borough.Godot` runs on Godot 4.7.2, drawing the city per Chunk with Travellers over it |
| Picking, and the five verbs | [`0045a`](0045a-amnesty-history.md) rows 15a–15g | ✅ **Shipped.** A click resolves to a Tile; `Look`, `Zone`, `Connect`, `Demolish` and `Service` are all issued from the shell, with a tool palette and refusals reaching the player as sentences |
| The information interface | [`0064`](0064-the-information-interface.md) | ✅ **Shipped.** Fifteen panels, eight overlays and the hover readout. ⚠ **Panel transitions remain unbuilt**, and dashboards, graphs, notifications and persistent Pins are deferred in that plan |
| Retained spatial rendering | [`0066`](0066-retained-spatial-rendering.md) | 🟡 **Shipped with its thresholds PROVISIONAL.** LOD meshes, whole-map aggregation and measured Chunk sizing remain open in that plan |
| Aged-city performance | [`0067`](0067-aged-city-performance.md) | 🟡 **Nine numbered items remain open**, including 1M-Citizen controls and quiet-machine reference captures |
| The drive channel | [`0048`](0048-driving-the-shell.md) | ✅ **Shipped.** `--drive`, `--listen` and `--record` step, photograph and assert against a running shell without a hand on the keyboard |
| **S1** — 20k Buildings via chunked `MultiMeshInstance3D` | — | 🔴 **Never run**, and the shell was built past it |
| **S3** — one data panel with a live multi-series graph | — | 🔴 **Never run**, and `0064` built the panels past it |
| A written presentation design | — | 🔴 **Does not exist.** [`0002`](0002-open-questions.md) §C carries it as *undesigned rather than unplanned* |
...
| **21** | ✅ ⚠ **STRUCK 2026-08-26 by [`adr/0171`](../docs/adr/0171-a-markets-level-is-what-its-sellers-hold-and-the-price-divides-by-the-sum-while-a-wake-spends-the-maximum.md).** The cover is now `DistrictMarkets.Stock(row).Held` — the sum over the row's sellers — and the price moves: `rulesets/oversupplied.toml` at 2,000 Citizens over 24,576 Ticks goes **100 → 58** with 6 changes on one row and 11 across the table, where every world ever run had produced **zero**. 🔴 ⚠ **IT WAS NOT A ONE-LINE FIX AND THE FILING SAID SO CORRECTLY, BUT FOR THE WRONG REASON.** The record it owed was cheap to write; what was expensive was that **two more call sites were reading the same undefined quantity and neither had a symptom** — `World.RingMarket` spent the *depositing* seller's level as a wake budget, and `RuleEngine.Stop`'s rescue drained a market row against the Pool Bin's structural **zero**, so the one thing that method exists to do could never fire on a market. ***Both were found by asking the same question of every site rather than by a second failure.*** ⚠ **`rulesets/provisioned.toml` STILL PRINTS A FLAT PRICE and that is now the mechanism**: it holds under a Day's cover, and a market with less than a Day of supply prices at its import ceiling. ***The defect and the correct result print the same digits***, so `MarketDumpTests` runs on both files and asserts the difference. `plans/0044` **F50**, **F57** |
| **22** | ✅ ⚠ **STRUCK 2026-08-26 by [`adr/0171`](../docs/adr/0171-a-markets-level-is-what-its-sellers-hold-and-the-price-divides-by-the-sum-while-a-wake-spends-the-maximum.md).** `World.EvaluateDistricts` ends by draining **every** market row. `MarketLongRunTests` runs both worlds to 524,288 Ticks with **no invariant violated**, where `oversupplied.toml` broke `WaiterIsBlockedByTheBinItNames` at Tick 362,496. ⚠ **NEITHER PLACEMENT THE FILING OFFERED WAS TAKEN, and both were wrong for reasons the filing could not see.** Eager re-homing in `DistrictWatershed.Migrate` is **incomplete** — membership changes in **four** places in that method and only one is `Migrate`; a Cell whose incumbent District is dying moves for free and a newly built Cell is filed without moving at all — and it is also the **wrong direction of lookup**, Cell → Buildings → occupants → Rule Instances, ***where a drain gets the same set by walking the queue that already exists.*** Lazy re-homing at the next evaluation is worse: there **is** no next evaluation, because `02 §4.1`'s *does not re-arm* means a stranded waiter is not late but gone. 🔴 ⚠ **A DRAIN FROM THE HEAD WAS SHIPPED FIRST AND WAS NOT ENOUGH** — it moved the violation from Tick 362,496 on `oversupplied.toml` to Tick **32,768** on `provisioned.toml`, which had been clean. ***The head changes***: a stranded waiter behind a legitimately blocked one survives the drain and is invisible to the invariant too, until the waiter in front of it wakes and it becomes the head. ✅ **So the ring SWEEPS the whole queue for waiters whose `Requirement` is zero** — the invariant's own predicate, and an exact test for *the Rule is right and the queue is stale* — and then drains, because a boundary move also changes which **sellers** a row has and a migrating shop is stock arriving with no deposit to announce it. ⚠ **A sweep is not a `WakeAll`**: a Household genuinely short of sundries stays asleep, so `02 §4.1`'s *a starved District costs nothing until supply arrives* is untouched. `plans/0044` **F53**, **F58** |
| **23** | ✅ 🔴 **STRUCK 2026-08-27 by the milestone-17 merge, AND IT WAS FILED AGAINST THE WRONG INPUT.** It was recorded as a **readout** shrinking a Rule's band, which made the repair site open-ended — the three candidate placements below, none obviously right. ***The arithmetic says it was never a readout.*** The filed trace is `restock` stepping **320 → 280 → 240**, and `restock`'s apply is `{ min = 1, max = 4 }` — **not derived**, so `RuleEngine.Band` returns a constant floor of 1 and `Readouts.Read` is never called. `RuleEngine.Requirement` is `floor × amount × price` = `4 × price`, so 320/280/240 is the **PRICE** stepping 80 → 70 → 60 — steps of exactly `[market] move_cap_percent` of a ceiling of 100. ⚠ **The Bin never moved and the requirement did come down to meet it; what moved it was the market, not the city.** So the site was never open-ended: it is the reprice. `World.RingEveryMoneyBin` drains the money Bins whenever `RepriceDistrictPools` moves anything — money Bins only, and that is derived rather than a narrowing for speed, because the GOOD leg of a purchase costs `floor × amount` with no price in it and only the MONEY leg carries one. It closes both sightings: the requirement that fell to something the buyer could afford (Tick 21,249, Rule Instance 865 on bin 661 holding **351** against **344**), and the requirement that fell to **zero** when a glut walked the price to zero (Tick 251,904, waiter 432 on bin 517). ⚠ **A price of zero is NOT the defect and a floor of one was tried and REVERTED** — `plans/0037` decision 4, settled with the user in the room, and `PoolPriceTests.A_glut_walks_the_price_to_nothing` asserts it. ***The price is allowed to reach zero; what was not allowed is nobody noticing.*** ⚠ **PRE-EXISTING and reached rather than caused**: on `main` `oversupplied.toml` bottoms out at 38 over 262,144 Ticks with 9 moves, and with milestone 17's decline in it the same run makes 111 moves and touches 0. `MarketLongRunTests.No_waiter_is_left_asleep_on_a_bin_that_could_serve_it` was the allowlist and is now the regression test |
| **24** | ⚠ **[`adr/0031`](../docs/adr/0031-one-resource-abstraction-and-depth-not-count.md)'s CHAIN-DEPTH GUARD WAS NEVER BUILT, AND IT IS THE GUARD THAT REPLACED ANOTHER GUARD** — filed 2026-08-26 by milestone 26 task 10, **found by re-reading `docs/06`'s placement row rather than by a failure**. `adr/0031` retires `04 §1`'s *five Goods, and a sixth should replace something* in favour of **maximum chain depth of three**, describing it in its own words as *"the constraint the same document already states and never enforced"* — and `RulesetLoader` still checks no depth. 🔴 **So the anti-goal has NO guard at all**: the counting bound was withdrawn and its replacement was never written, which is worse than either alone because the record reads as though a constraint is in force. ⚠ **[`adr/0049`](../docs/adr/0049-a-rules-apply-count-is-decided-against-the-past-and-settle-may-only-reduce-it.md) RESTS ON IT** — *"depth of three, so the graph is a shallow DAG and a topological sort over Bins is feasible"* — so this is not only a taxonomy bound; ***a Ruleset authoring a deep chain would put a decision about apply counts on ground that decision assumed away.*** ⚠ **It is a loader refusal and its shape is known**: the Rule graph is Bin-to-Bin, `RulesetLoader` already walks it for the money-balance refusal, and the depth is a longest-path over a DAG it can already build. **No design question is open** — the number is `adr/0031`'s and it is three. ⚠ **Check the DAG assumption before the depth**, because a cycle has no longest path and nothing refuses one today either |
| **25** | 🔴 ⚠ **THE POST-SUBMIT BALANCE LANE GOT 3–5.6× SLOWER ACROSS THE MILESTONE-24 TERRAIN MERGE, AND THE THIRD RUN NO LONGER HAPPENS AT ALL** — filed 2026-08-26 from the lane's own history, **not from a measurement**. Between `7048c157` (green) and `43b07b6c` (red), on the same runner class and the same commands: `minimal.toml` at 1,000,000 Ticks **54m42s → 2h43m56s**, `fouled.toml` at 100,000 **2m22s → 13m16s**. The first run now eats 2h44m of the job's 180-minute ceiling, so `evicted.toml` is **cancelled before it starts** — ⚠ ***the only lane that reaches a world where a tenancy ends has not run since 2026-08-25***, and that is the run [`adr/0006`](../docs/adr/0006-no-collection-grows-with-elapsed-time.md)'s per-tenancy allocation cycle is reachable through. 🔴 ⚠ **NOT ONE OF THOSE FOUR FIGURES MAY BE QUOTED AS A COST**: a runner is not the reference machine ([`adr/0106`](../docs/adr/0106-a-wall-clock-budget-names-a-machine-class-and-a-thread-count-or-it-is-not-a-budget.md), [`adr/0121`](../docs/adr/0121-the-commit-gate-is-the-assertion-tier-and-a-long-test-runs-post-submit-on-a-machine-that-is-not-yours.md)), and its class is not stable between runs. What the lane is entitled to report is that something **moved against its own history**, which is what `adr/0121` §4 puts it there for — ***so this is a summons to measure and not a measurement***. ⚠ **The interval is a merge and not a commit**, so a bisect is the first work: water, drainage, sealing, a saved handle per Cell and two new tables landed together. ⚠ **`minimal.toml` states neither `[water]` nor `[[terrain]]` and slowed by 3× anyway**, so whatever it is, is being paid **unconditionally** — the shape of a fold or a sweep over all 262,144 Cells rather than of new content. ⚠ **Raising the 180-minute ceiling is the workaround and not the fix** ([`adr/0073`](../docs/adr/0073-a-local-workaround-is-not-a-discharge-and-a-finding-about-shared-code-must-reach-it.md)); the third run's absence is the symptom to clear, not the budget. Cost row owed to [`0013`](0013-tick-budget.md) as **UNMEASURED**. `plans/0044` **F65** 🔴 ⚠ **THE REFERENCE MACHINE CORROBORATES IT, AS AN UPPER BOUND, 2026-08-26**: the whole suite ran **52m48s** against the **36m22s** [`CLAUDE.md`](../CLAUDE.md) records — and a single instrument class, `ParkingArrivalStreamTests`, took **50m42s for five tests**, which alone exceeds the documented whole-suite figure. ⚠ **BOTH ARE UPPER BOUNDS AND NEITHER IS A CAPTURE**: the machine was carrying other test runs and two builds, and ***a timing capture names nothing else running on this machine as its first control***, which these cannot. ⚠ **The class figure is NOT a claim that the class moved** — there is no prior reading for it, so it is recorded as a magnitude worth explaining rather than as a delta. ***What the pair buys is that this is no longer dismissible as runner noise***: the lane and the reference machine moved the same way, in the same direction, across the same interval. The bisect is still the first work, and a quiet-machine capture is the second. |
| **26** | ⚠ **There are TWO spellings of *remove a thing* in the Input Log and [`adr/0091`](../docs/adr/0091-clearing-land-is-bought-rather-than-taken-and-demolish-is-the-sixth-verb.md) says there should be one** — filed unfixed 2026-08-26 by milestone 17 task 4, **found by building the verb the ADR asked for**. That ADR's Consequences are explicit: *"`Connect`'s bulldoze flag is **superseded rather than retained**, so there is one spelling of remove a thing in the Input Log rather than two."* Task 4 shipped `CommandKind.Demolish` over Buildings and left `ConnectAction.Bulldoze` standing over Streets, so `Connect` still means *lay a road* and *unlay a road* — which is less bad than the three meanings `adr/0091` found, and is not what it decided. | 🔴 **NO to the city and YES to the ARTEFACT, which is why it was not done inside task 4.** Retiring `ConnectAction` re-spells **six of the committed golden session's seven `connect` commands** — payload words 2 and 3 are bulldozes — so `session.borough` is rewritten and `session-trace.txt` re-recorded. ⚠ **The city it describes is identical**: the same Segments are laid and unlaid in the same order, so this is a **rename in the codec** rather than a change to what runs. ⚠ **That distinction is exactly what [`adr/0100`](../docs/adr/0100-moving-the-state-hash-costs-nothing-until-somebody-is-carrying-a-save.md) forbids using as a reason to defer**, so the reason recorded here is SCOPE and not cost: task 4's subject is a verb over Buildings, and rewriting the baseline inside it would have put two unrelated things under one commit message. | ⚠ **The shape is settled and only the work is owed**: `Demolish` takes a payload — an axis and a road kind, which is `ConnectPayload` minus its action — and `Command`'s four fields still hold it, so **`InputLogCodec.Version` does not move** and the ADR's own *a sixth field is what bumps it* still holds. ⚠ **`ConnectAction` should be DELETED rather than left unused**: a two-valued enum with one live value is a lay-or-bulldoze flag wearing a disguise, and the next reader restores the second meaning. ⚠ **Do it in its own commit with the re-baseline**, per `tests/Borough.Tests/Golden/README.md` |
| **27** | 🔴 ⚠ **A FOOTPATH IS DRAWN AT CARRIAGEWAY WIDTH, SO THE PICTURE SHOWS A CITY OF ROADS WHERE THE GRAPH HOLDS PATHS** — filed 2026-08-31 by [`0048`](0048-driving-the-shell.md) **F16**, from the first draw list ever taken. `Main.RoadWidthMetres` is **8 m applied to every Segment whatever its kind**, and its own doc comment says so without noticing the consequence: *how wide the carriageway is drawn*. Measured on `flooded.toml` at 2,000 Citizens — of 149 Segments drawn, **5 came back at 8 × 181.019 m at 135°** (128√2, the diagonal cut-throughs), and `Borough.Headless --roads` confirms **5 FootPaths** at that population. So a path carrying **24,000 Vehicles/Day at 5 km/h draws exactly as wide as a Street carrying 86,400 at 50**, and `[roads] foot_paths_per_thousand_blocks` — which `CLAUDE.md` calls *the stronger Severance lever* — is **invisible in the one instrument built to show Severance to a person**. ⚠ **It was found by a column of numbers rather than by looking**, and that is the entry's point as much as the defect is: at the framing this shell uses, five diagonals in a lattice read as scenery. ⚠ **The repair is the shell inventing THREE thicknesses where it invents one**, and each must be labelled as invented — a Segment states no width and `adr/0078` gives a Lot no depth for the same reason. ***Not a Ruleset key***: a drawing width is host-side and a designer who could reload it would be tuning a picture. | **no** — `Borough.Godot` holds no simulation state and a drawing width enters no fold. ⚠ **It is not in `Borough.slnx` either**, so no test in the suite can reach the repair; what asserts it is a **draw list**, which is why this row could be written at all | **Cheap, and it blocks a reading rather than a build.** Nothing waits on it — but the Severance work is the one place the picture is the deliverable, and ***an instrument that draws two classes identically cannot be used to judge the thing that distinguishes them***. Take it before anybody looks at a foot network |
