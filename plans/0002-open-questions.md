...
| **The Parking Shed's radius** — *"acceptable walking distance"* | Parking, which is Phase 2 | **NEW, from session D task 0, and it is a row that should have been opened when `adr/0009` was written.** Hash-bearing: the radius decides which Bin a car takes, which decides the walk Leg, which counts against the Commute Budget, which decides whether the Trip fails. `adr/0009` names it under *"a new balance surface"* and nothing has ever entered it here. **Look for the derivation before reaching for a value**, on tau's precedent — the ADR gives the shape of one: a **District is bounded by where transport can be ignored** and a **shed is bounded by where transport must be measured**, so the two scales *cannot* coincide, which is a constraint rather than a number. ~~Named ratifier owed at the moment a value is chosen~~ **⚠ RATIFIER NAMED 2026-08-12 by session H ([`adr/0083`](../docs/adr/0083-a-sheds-use-is-the-arrival-query-and-a-stale-shed-is-wrong-by-a-bounded-walk.md)), and the value is still unset — which is the point: `adr/0052` asks for the ratifier on the day the number is *written down*, and this row had gone without one since `adr/0009`.** **Ratifier: milestone 7's first run reporting the walk-Leg length distribution as shed occupancy approaches 1** — which is §B's *does parking scarcity degrade as a gradient*, **the same instrument**, so the number and the claim ratify together and neither needs its own machine. **Two constraints are stated and neither is a value.** It is **bounded above by the Commute Budget's walk allowance**, since a shed wider than a Trip can afford to walk has outer Bins that can never be taken — which also makes it the *second* number denominated in `adr/0082`'s Tick. And its **cost gradient is measured**: R5.6 gives 110 Bins found at 400 m against **596 at 800 m**, so doubling the radius is roughly 5× the shed. ⚠ **Do not reach for the walking-time intuition** — five minutes at 5 km/h is 417 m, which is exactly the sort of plausible number `adr/0044` had to measure back out of three documents that cited it as settled |
...
| **`lots_per_segment`** — how many Buildings share one Street Segment | ~~Being chosen now~~ — **the value is taken rather than chosen; what is owed is a measurement of the claim it is taken from** | **NEW, from 5a-bis** ([`adr/0078`](../docs/adr/0078-frontage-is-derived-on-the-epoch-and-a-lots-width-is-the-segments-own-building-count.md)), and hash-bearing — it decides how many Lots a zoned block yields, therefore how many Buildings a city holds. **It is entered here as a *derived* number, which is the third time this section has been asked for a value and found one already in the corpus** (tau, the arming stagger, and `adr/0059`'s deletion). The value is `CONTEXT.md` → Address's own **five** — *"five Buildings share a Segment at the working figures"* — which is not decoration but **the premise of the decision that keeps an Address off a Node**, and therefore of the ~30,000-Segment figure every routing cost in [`0013`](0013-tick-budget.md) is priced against. **So its source is not its ratifier**: a derived number is refuted by whatever refutes its derivation, and citing the sentence you took it from would be `adr/0044`'s failure exactly. **The corroboration is worth more than the derivation, because neither side was derived from the other**: 5a's graph gives **33,024** Street Segments at the shipped `[roads]` against *"~30,000"*, so the product is **165,120** Lots — and `World` sizes its Lot table at 225 per 1,000 Citizens, S4 task 2's row ratio chosen years of documents before there was a graph, which is **225,000** at 1M. Two figures that never met, agreeing within a quarter. **Named ratifier: the first `06` 5b run that reports a real Buildings-per-Segment distribution**, since a Trip is what makes the quantity observable rather than assumed; if the five moves, this number moves with it and one sentence fixes both. **Reopens** when density bands arrive ([`adr/0025`](../docs/adr/0025-density-is-a-cap-and-it-trades-land-for-materials.md)) — `02 §2.2` asks for targets that *"vary by zone density"*, so one number becomes one per band, which is the half 5a-bis leaves unbuilt on purpose. **Its sibling was deleted rather than entered**: Lot **depth** has no row here, because a Lot has no extent in `LotTable` and a depth would be a hash-bearing number chosen for a consumer nobody has designed |
...
**117 written, numbered to `0118`; `0028` is reserved and unwritten.** Grouped by what grilled them.
