...
| **`[households] car_ownership_percent`** — what share of Households keeps a car | ~~5c task 6's volume-delay function~~ **5c task 8's long run.** ⚠ Task 6 shipped 2026-08-14 and **does not discharge the caveat below**: a drive is priced against congestion now, but a *generated* city never reaches a load where the function changes anything (`adr/0099`), so a rate chosen against one is still chosen against a city where driving has no downside | **NEW 2026-08-14, from [`adr/0098`](../docs/adr/0098-a-citizen-travels-in-their-households-mode-and-mode-choice-is-undesigned-rather-than-unbuilt.md)**, and hash-bearing: it decides who drives, which decides what a commute costs, which decides who takes which job. ⚠ **STATED as of 5c task 7, in exactly one file, so a *debt* rather than a gap** — `rulesets/congested.toml` sets it to **100**, and 100 rather than a partial rate because that file exists to put Vehicles on Segments and a partial rate would make the demonstration a measurement of the rate instead of of the function. The other three shipped files omit the table, and the absence of it means *nobody drives*, which the loader reaches by omission rather than by a defaulted key, because **zero is inside the range of legitimate answers** and a placeholder there could not announce itself. ⚠ **There is no derivation to find and this row says so rather than leaving the search implied** — tau's precedent is that the cheapest way to satisfy `adr/0052` is to find the derivation, and here every candidate is a real-world calibration input rather than a quantity this simulation produces. **What the corpus can supply is a bracket**: US household vehicle availability is about 91%, and the urban core is far lower, so the interesting range is wide and the interesting *question* is which end of it a city-builder wants. ⚠ **Choosing it before task 6 would choose it on a free-flow drive**, which is quoted too cheap by construction — so a rate picked now is picked against a city where driving has no downside at all. **Named ratifier: 5c task 8's long run**, and the refuting readings are stated in both directions — a rate at which `jobs beyond budget` never leaves zero means the Commute Budget has stopped binding on anybody and the labour market is supply-limited by accident, and a rate at which the three Budget rungs' populations collapse into *fast* means the rungs have stopped being a vocabulary |
| **`[traffic] alpha_percent`, `beta`, `clamp_percent`** — BPR's two parameters and the ceiling on what it reads | **5c task 8's long run** | **NEW 2026-08-14, from [`adr/0099`](../docs/adr/0099-a-legs-cost-is-a-plan-and-a-drive-is-priced-segment-by-segment-as-it-is-met.md)**, and hash-bearing: they decide what a loaded Segment costs, which decides how long a Trip takes, which moves every downstream count. ⚠ **STATED as of 5c task 7, in exactly one file, so a *debt* rather than a gap** — `rulesets/congested.toml`, written so that task 8 has something to ratify against; the other three shipped files omit the table and `--traffic` refuses all three. Omitting it is a city whose roads never slow down, which is also every city this project described before task 6, so an older Ruleset still means what it meant. ⚠ **They are *sourced* and not *ratified*, and `adr/0052` keeps those apart.** 15% and 4 are BPR's textbook figures; 400% is **recovered arithmetic** rather than a quotation — S2 R8.0 published *an arc at the clamp costs 39.4× free-flow* and `1 + 0.15 × 4⁴ = 39.4` exactly, so the spike's clamp is backed out of its own published delay. **S2 is synthetic and ran a uniform origin–destination draw, which R4 measured is the longest-trip distribution available**, so a number carried out of it is carried out of a different city (`plans/0012` **Cause 5**). ⚠ **Nothing this project can currently build ratifies them**, and that is the sharper half: a generated city peaks at `v/c` **0.44** at 4,000, 16,000 and 64,000 Citizens alike, so the curve is only ever evaluated on a stretch where it is nearly flat. ⚠ **A three-dial sweep on 2026-08-14 found no authored configuration that moves them at all** — 5.3× less road per Building, a rush hour cut from eight hours to eight minutes, and both together, all **×1.0000** — so **task 8 cannot ratify these numbers on a generated city and should not be read as having done so**; the sweep and what would close it are in §C's routing cluster. ⚠ **A fourth dial then did move them, and it is the supply side: `street_capacity_per_hour` 3600 → 400 gives ×1.0088 and 200 gives ×1.8949.** That is what `congested.toml` states — but 400 Vehicles an hour is a Segment holding **one** Vehicle where the shipped 3,600 holds 9.2, so it is a rung chosen to make the mechanism visible and **not** a city. ⚠ **Task 8 must run against `congested.toml` or it ratifies nothing, and must not read that file's numbers as ratified by a run it was built for.** **The refuting readings are named in both directions** — a long run in which the loaded and free-flow occupancies are byte-identical means the parameters are decorative on any city a player will build, and one in which the clamp binds routinely means the curve has stopped discriminating where the game is played and it is the *clamp* that wants moving rather than α. **β is bounded by arithmetic rather than by taste**: a clamped ratio of 4 to the eighth is 65,536, exactly Q16.16's whole range, so the loader refuses above 6 |
