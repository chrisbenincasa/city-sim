...
| ~~**What the Lane model costs**~~ **ANSWERED 2026-08-11 by S5, and published the same day** → [`spike-results`](../docs/spike-results.md) → *S5*, [`0019`](0019-s5-lane-kernel.md). **~325,000–330,000 Vehicles a Tick a core** with Overlaps, ~379,000–381,000 without, and **~4,510–4,580 Microscopic Segments in 15.6 ms** at 72 Vehicles each. **Two of five tripwires fired and both reach sentences rather than designs**: `adr/0016`'s transplanted 400,000 is **not reproduced** (T1) and its *"constant of a `memcpy`"* is **26–29× a bare walk** (T3). **The structure survives entirely** — flat in `O(n)`, no spatial index, zero allocation — and **the gap is `adr/0003`'s arithmetic**: three integer divisions a Vehicle a Tick, two of them removable for 1.63–1.75×. `0013` has the row it never had. **Two residues, both entered below**: the arithmetic *form* is hash-bearing (§D2), and every absolute is a `powersave` lower bound pending the canonical capture | ~~**S5**~~ **run; owes the `performance` re-capture** | **NEW, from session D task 0, and the pass's largest finding.** `adr/0016`'s affordability rests on one external number — Eickhoff's *"roughly 400,000 individually simulated cars on a single core"*, *"the number that makes microscopic traffic affordable at all"* — taken off a **float-based Rust engine**, where `adr/0003` forbids us floats entirely. **[`0013`](0013-tick-budget.md) has no row for the Microscopic tier at all**, not even *unbuilt*, so the half of the movement subsystem that is not routing is priced nowhere while the half that is carries 60–67 of the ledger's ≥114 points. S5 also prices the two claims that ride with it: *O(n) with the constant of a `memcpy`* and **promotion cost** — materialising Lane queues from in-flight Trips, which `adr/0016`'s own revisit trigger names as a condition and gives no machine |
...
| **How the IDM is *spelled*** — whether its divisions are taken as written or against precomputed reciprocals | The Lane model, which is Phase 2 | **NEW 2026-08-11, from S5's L1 attribution, and it is a consequence nobody anticipated when the row above was filed.** Not a tuning parameter and not a value: it is the **arithmetic form** of a kernel, and it is hash-bearing because `adr/0003`'s integer division and a reciprocal multiply do not agree in the last bit. **The price of the choice is measured: 1.63–1.75×** of the whole Lane model, which is the largest single lever S5 found and larger than anything the tuning parameters can do. It also costs **4 bytes a Vehicle** of state a promotion must materialise. **So `CLAUDE.md`'s own test bites in an uncomfortable place** — *a change is an optimisation if the State Hash is unchanged and a design change otherwise* — and by that test **the fast form is a design change**, motivated purely by speed, in a kernel whose output is emergent travel time. **Named ratifier owed at the moment a form is chosen**, and the choice must be made **before** any Ruleset is balanced against Microscopic travel times, because afterwards it is not available: switching form would move every one of them. **The cheap escape is worth checking first** — if the two forms can be made to agree exactly at Q16.16 (the reciprocal is a Ruleset constant and could be pre-rounded so the products match), the question dissolves and this row is retired rather than filled, which is `adr/0059`'s direction |
