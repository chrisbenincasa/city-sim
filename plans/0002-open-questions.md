...
**110 written, numbered to `0120`; `0028` is reserved and unwritten.** Grouped by what grilled them.

⚠ **`0110`–`0118` are missing from this branch and are not free** — they are **claimed by the
unmerged `milestone-8-save-load` and `milestone-10-conserved-money` worktrees** and arrive at merge. So
a **gap in this column is not evidence of a missing decision** while more than one milestone is in
flight, and the sibling row-completeness check below cannot tell the two apart. `plans/0000` named this
hazard for **plan** numbers the same week.
