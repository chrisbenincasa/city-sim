...
- **`§6` threading, the whole of it** — which thread runs `step()`; who owns and publishes the
  ...
  spatial search into Decide. **Phase 2 — it gates milestones 10 and 11, not 1–4.**
...
- ~~⚠ **A failed Trip must demote the option that produced it.**~~ **CLOSED 2026-08-13, in the sitting
  ...
  ***an answer that arrives after a debt is filed does not find its way to the ledger***, which is
  ...
  named as milestone **9a**'s Departure and deliberately not designed. **No `adr/0052` number opened**,
...
- ~~**`01-player §1` (core loop), `§3` (first ten minutes)**~~ **— both closed 2026-08-12 by session P**
  ...
  not be grilled.
- **`06`'s Phase 2 ordering (K2)**, which stays last — re-deriving it before `03 §5` is grilled is
  writing it twice.
...
> **no milestone holds it**: 6 is the Lane kernel and 7a is Stress. The ratifier cells now say so
...
**Three unset hash-bearing numbers go to §D2 as gaps**: a kind's throughput ceiling, its price offset against the Hinterland's, and the generator's count and siting. Ratifier for all three is the first Ruleset with an Outside Connection in it, which is milestone 10's.
