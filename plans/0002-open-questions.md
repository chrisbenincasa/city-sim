...
| **How many Households a Building holds** — a Building has no occupancy at all, and the two halves of the model disagree about it | Phase 2, with `02 §5.4`–`§5.6`'s pro-forma. **The machine exists already**: `ZoneRuleLongRunTests` | **NEW, from slice 10 task 10, and it is a measurement rather than a worry.** `SyntheticCity` puts **three** Households in each Building; `ZoneRuleEngine.Create` places **one** (`adr/0054`'s *drained blind* — there is no rent, commute or tolerance to place more on). So each demolish-and-rebuild cycle nets **+2** into the Unplaced Pool, and a 100,000-Tick run settles at **~60 of 121 Lots built on and ~300 of 360 Households homeless**, with nothing trending and every table recycling. **Not `adr/0006` and not a balance mistake** — an artefact of a capacity the Ruleset cannot express, so no tuning done before it exists is worth keeping. The number that settles it is an occupancy declared in a `[[kind]]` |
