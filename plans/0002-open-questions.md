...
now** and are the ones to watch — they are the test of the rule above.

| Number | Waiting on | Note |
|---|---|---|
| **Pollution decay rate (tau)** | **Being chosen now** — [`adr/0051`](../docs/adr/0051-industrial-pollution-is-a-stock-the-environment-absorbs.md) ships one global value in the Ruleset | **Hash-bearing, and worse-placed than the Layer cadence was**: tau sets the *equilibrium* a steady emitter settles at, so it decides what every pollution number in the game means rather than how often one is refreshed. Per-Cell absorption is in [`deferred.md`](../docs/deferred.md) with parks |
| **Rule Instance arming stagger** | **Being chosen now** — slice 7 task 10a | **Hash-bearing**, and it needs its own `purpose_tag`. A stagger is not optional: one delay for every Building puts the whole city in one Event Wheel bucket. *Which* stagger is unargued |
