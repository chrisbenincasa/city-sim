...
**110 written, numbered to `0113`; `0028` is reserved and unwritten.** Grouped by what grilled them.

⚠ **`0110` and `0111` are missing from this branch and are not free** — they are **claimed by the
unmerged `milestone-8-save-load` worktree** and arrive at merge. Milestone 7's opening sitting took
`0112`/`0113` on 2026-08-18 after reading the claimed filenames off `git log --all`, which is the only
instrument that sees them. ***Two sessions on two branches cannot both hold the next free number, and
neither branch's tree contains the other's claim*** — so a **gap in this column is not evidence of a
missing decision** while more than one milestone is in flight, and the sibling row-completeness check
below cannot tell the two apart. `plans/0000` named this hazard for **plan** numbers the same week.
