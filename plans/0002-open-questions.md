...
| **Table sizing ratios** | 225 Lots and 150 Buildings per 1,000 in `World`; the populator builds **120 of each**; Households at **exactly capacity**; Bins and Rule Instances at 450 | **A live inconsistency, not merely an unratified number.** `SyntheticCity.HouseholdsPerBuilding = 3` yields 120 Buildings per 1,000 while its own docstring says *"~150"* and `World` allocates for 150 — so `World` over-allocates Lots by 87% and Buildings by 25% against what anything actually builds, and **part of S0a's 86 MiB is capacity for rows that never exist** (`0011` finding 5, same shape). Nothing checks the two agree. Owed by slice 7 |
