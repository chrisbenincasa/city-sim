...
- ⚠ **What makes a city in this simulation congest at all?** **NEW 2026-08-14, from 5c task 6.** The
  volume-delay function is built, correct and exercised — and **inert on every world this project can
  currently produce**. It is an *arguable* entry under `adr/0043` only because the measurement half is
  already done; what is open is a **design** question about demand, and the numbers are here so nobody
  re-derives them.

  **A three-dial sweep at 16,000 Citizens on `rulesets/minimal.toml`**, each dial authored Ruleset data,
  each run twice (with and without `[traffic]`):
  ...
  keeping. `SyntheticCity` sizes the paved extent to hit a **Lot** count, so cutting roads per Building
  shrinks the map, shortens every commute and takes total traffic **down 2.1×** while taking `v/c` up
  0.1. It is the √population finding seen from a second dial: ***every supply dial this simulation
  exposes moves demand with it.***

  ⚠ **And the last row is the sharp one: `v/c` 1.09 is genuinely over capacity and the run is still
  byte-identical.** Mean load there is **0.032 Vehicles per Segment per Tick** — the road is empty about
  97% of the time — so a busiest-Segment reading of 10 is a **momentary coincidence of arrival times,
  not a jam**. BPR is an average-flow model, and the aggregate is dominated by the overwhelming majority
  of crossings at volume 1, where the delay is 2×10⁻⁵ and below what Q16.16 can represent.
  ***A peak that a mean does not follow is a coincidence, and a volume-delay function prices means.***

  **So the binding constraint is trips, not road.** Everybody makes exactly **one journey a Day**,
  because the commute is the only Trip generator built and `06`'s *Mechanisms with no milestone* lists
  **seven** more. Even in the sharpest burst the arithmetic is ~8,000 departures into **769 Segments**
  with a commute lasting about **one Tick** — roughly one Vehicle per Segment. **The network is bigger
  than the fleet at every setting of every dial.**

  ⚠ **The evening leg is not the answer either, and it is already refused rather than merely absent.**
  `CommuteEngine`'s own doc-comment: *"a return journey makes a Citizen's day a **schedule**, and a
  schedule is what arrives when `adr/0067`'s shopping or `adr/0032`'s school gives it a second entry"*
  — `adr/0070`, and it also gives up a free guarantee, since one-Trip-per-Day plus a Budget in minutes
  is what makes an overlapping departure arithmetically unreachable. **Doubling the trips doubles 0.032
  to 0.064.** The gap is not a factor of two.

  **What would close this.** Either a demand mechanism that puts many more simultaneous travellers on
  the network than it has Segments — which is several of `06`'s unplaced generators together, not one —
  or a **player** laying road independently of where people live, which is
  [`adr/0090`](../docs/adr/0090-the-generator-makes-land-and-the-player-makes-every-road.md)'s city and
  is unreachable from `CommandKind.Populate`. **Until one of them exists, `[traffic]`'s three numbers
  cannot be ratified by any run**, which is a live qualification on §D2's row and on 5c task 8.

  ⚠ **A fourth dial closed the loop on 2026-08-14, and it is the supply side rather than the demand
  side: `street_capacity_per_hour`.** Task 7 needed a Ruleset the volume-delay function could be *seen*
  on, and the sweep that chose one is the other half of this entry. At 16,000 Citizens over 512 Ticks,
  varying nothing but the Street's capacity:
  ...
  generated city will congest it***, and that is worth more as a recorded fact than a prettier
  demonstration would be. It does **not** close this entry: a file written to make a mechanism visible
  is `severance.toml`'s precedent, not a city, and §D2's ratifier still needs a world a player could
  plausibly build.

  ⚠ **The middle rows are a finding about the *instrument*, not about the city.** Between 3,600 and 600
  the bill moves and the two panels do not, because the extra dwell is real and **shorter than a Tick**
  — it changes what a crossing costs without changing how many Vehicles stand on a road at any Tick
  boundary. Below 400 both panels clip at *over capacity* and stop discriminating from the other end.
  ***A per-Tick snapshot cannot see a sub-Tick delay***, so the bill and the picture are driven by
  different thresholds and an instrument showing only one of them is silent over most of its range.
