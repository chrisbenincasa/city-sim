...
| `0094` a Day is 2048 Ticks, because Ticks per Day is a sampling rate and not a length of life | 🟢 | **Session P, 2026-08-13, with the user in the room**, grilling `01 §4`. `TICKS_PER_DAY` **8192 → 2048**, a Tick is **42.1875 s**, a Day is **2m08s at 1×**, and a twenty-hour campaign goes from **140 Days to 562** — which `01 §4`'s twenty-hour marker needs, because Replacement Rate takes generations and 140 Days contains none. **`adr/0019`'s conclusion falls and its prohibition survives.** Its *"same city, same population, same roads, **twice the vehicles**"* needs a Tick whose duration is fixed independently of the Day, which is what `0082` deleted: a commute is 1.39% of a Day at **both** constants, so the constant cancels and no balance moves. **The rule that decides what rescales is one question — *what is it denominated in?*** Ticks (kept, ×4 coarser in-world), in-world time (unchanged, ×4 more per real second), Days (unchanged, ×4 faster in real time). **Only Goods quantities rescale, ×4.** ⚠ **Two costs and both are large**: routing ×4 takes `0013` to ~305% at 4× and ~76% at 1×, and the sub-step band 21–45 → **84–180** takes the Microscopic Cap to ~3,200–6,800 against ~186,600 demand. **Its finding is that `0082` left two live consequences standing on the premise it deleted** — including a **revisit trigger whose direction is now backwards** — which is `0089`'s *the obligation a deletion creates is a re-derivation* for the third time in two days |
