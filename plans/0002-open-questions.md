...
- **The tabulated `exp`/`log` table resolution has never been stated.** `adr/0003` promoted it to a required core component and was explicit that the resolution is *a stated figure, not an implementation detail*, because it perturbs the effective `μ` and `μ` is what prevents stampedes. The figure appears in **no document**. It is hash-bearing by the `05 §4` test and therefore a world-creation constant, so it cannot be tuned later. Blocks the substrate slice; validation is owed against `adr/0005`'s herding behaviour, and `adr/0005` is 🔴.
